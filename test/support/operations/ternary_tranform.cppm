...
export module dpl.test:ternary_transform;
...
class ternary_transform {
    ...
    using op_result = typename invoke_result_t<Op, dpp::basic_vector<E, A>,
        dpp::basic_vector<E, A>, dpp::basic_vector<E, A>>::value_type;
    template <dpp::simd_element_for<A> E>
    ...
    static constexpr void test_masked(vec_t<E> const lhs, vec_t<E> const mid,
        vec_t<E> const rhs, Op const op, vec_t<op_result<Op, E>> const src,
        Cmp cmp) noexcept {
        ...
        // lhs, mid, rhs)
        {
            assert(dpp::all_of(cmp(op(vzero, all_true, lhs, mid, rhs),
                op(dpp::zero, all_true, lhs, mid, rhs))));
            assert(dpp::all_of(cmp(op(vzero, all_false, lhs, mid, rhs),
                op(dpp::zero, all_false, lhs, mid, rhs))));
            assert(dpp::all_of(cmp(op(vzero, alt_mask, lhs, mid, rhs),
        ...
        if constexpr (dpp::fixed_width_abi<A>) {
            ...
            {
                assert(dpp::all_of(cmp(op(vzero, all_cmask, lhs, mid, rhs),
                    op(dpp::zero, all_cmask, lhs, mid, rhs))));
                assert(dpp::all_of(cmp(op(vzero, none_cmask, lhs, mid, rhs),
                    op(dpp::zero, none_cmask, lhs, mid, rhs))));
                assert(dpp::all_of(cmp(op(vzero, alt_cmask, lhs, mid, rhs),
...
public:
    ...
    static constexpr bool test(span_t<E> lhs, span_t<E> mid, span_t<E> rhs,
        Op op, span_t<op_result<Op, E>> expected,
        ...
        auto const vactual = op(vlhs, vmid, vrhs);
        return dpp::all_of(cmp(vactual, vexpected));
    }

    template <dpp::simd_element_for<A> E, typename Op,
        typename Cmp = decltype(dpp::cmpeq)>
    static constexpr bool test(E lhs, E mid, E rhs, Op op,
        op_result<Op, E> expected, Cmp cmp = dpp::cmpeq) noexcept {
        auto const vlhs = dpp::broadcast<E, A>(lhs);
        auto const vmid = dpp::broadcast<E, A>(mid);
    ...
    static constexpr bool test_masked(span_t<E> lhs, span_t<E> mid,
        span_t<E> rhs, Op op, op_result<Op, E> src,
    ...
    template <dpp::simd_element_for<A> E, typename Op,
        typename Cmp = decltype(test::bitcmp)>
    static constexpr bool test_masked(E lhs, E mid, E rhs, Op op,
