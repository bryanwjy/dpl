...
// Tests for dpp::bwandnot on xmm ABI. Requires SSE4.2 (implied by dpl.xmm).
//
// Forms tested:
//   (1) dpp::bwandnot(lhs, rhs)                  -- unmasked, always available
//   (2) dpp::bwandnot(src, mask, lhs, rhs)       -- merge-masked
//   (3) dpp::bwandnot(dpp::zero, mask, lhs, rhs) -- zero-masked explicit
//   (4) dpp::bwandnot(mask, lhs, rhs)            -- zero-masked alias (== form
//   3)
...
int main() {
    ...
    constexpr auto run = []() {
        ...
        return dpl::test::bitwise<abi_t>::run_all<dpp::bwandnot>(
            types{}, engine);
    };
