...
// Tests for dpp::min on xmm ABI. Requires SSE4.2 (implied by dpl.xmm).
//
// Forms tested:
//   (1) dpp::min(lhs, rhs)                  -- unmasked, always available
//   (2) dpp::min(src, mask, lhs, rhs)       -- merge-masked
//   (3) dpp::min(dpp::zero, mask, lhs, rhs) -- zero-masked explicit
//   (4) dpp::min(mask, lhs, rhs)            -- zero-masked alias (== form 3)
...
int main() {
    ...
    constexpr auto run = []() {
        ...
        return dpl::test::minimization<abi_t>::run_all(types{}, engine);
    };
