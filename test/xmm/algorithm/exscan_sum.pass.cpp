...
// Tests for dpp::exscan_sum on xmm ABI. Requires SSE4.2 (implied by dpl.xmm).
//
// Forms tested:
// (1) dpp::exscan_sum(val, init)
// (2) dpp::exscan_sum(val, mask, init)
...
int main() {
    ...
    constexpr auto run = []() {
        ...
        if consteval {
            ...
            return dpl::test::exscan_sum<abi_t>::run_all(types{}, engine);
        } else {
