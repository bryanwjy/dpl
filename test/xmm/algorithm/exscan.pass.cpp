...
// Tests for dpp::exscan on xmm ABI. Requires SSE4.2 (implied by dpl.xmm).
//
// Forms tested:
// (1) dpp::exscan(val, init, func)
...
int main() {
    ...
    constexpr auto run = []() {
        ...
        if consteval {
            // Reduce number of types to exscan compile time steps
            using types =
                dpl::type_pack<dpl::int8, dpl::int16, dpl::int32, float>;
            return dpl::test::exscan<abi_t>::run_all(types{}, engine);
        } else {
