...
import dpl.xmm;
import dpl.test.harness.algorithm.lookup;

// Tests for dpp::lookup on xmm ABI. Requires SSE4.2 (implied by dpl.xmm).
//
// Forms tested:
// (1) dpp::lookup(oob, lhs, rhs)                  -- unmasked, always
// form 3)
...
int main() {
    ...
    constexpr auto run = []() {
        ...
        if consteval {
            ...
            return dpl::test::lookup<abi_t>::run_all(types{}, engine);
        } else {
