...
// Tests for dpp::reduce_min on xmm ABI. Requires SSE4.2 (implied by dpl.xmm).
//
// Forms tested:
// (1) dpp::reduce_min(val)
// (2) dpp::reduce_min(val, mask)
...
int main() {
    ...
    constexpr auto run = []() {
        ...
        if consteval {
            ...
            return dpl::test::reduce_min<abi_t>::run_all(types{}, engine);
        } else {
