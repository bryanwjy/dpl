...
// Tests for dpp::reduce_max on xmm ABI. Requires SSE4.2 (implied by dpl.xmm).
//
// Forms tested:
// (1) dpp::reduce_max(val)
// (2) dpp::reduce_max(val, mask)
...
int main() {
    ...
    constexpr auto run = []() {
        ...
        if consteval {
            ...
            return dpl::test::reduce_max<abi_t>::run_all(types{}, engine);
        } else {
