...
// Tests for dpp::exscan_max on xmm ABI. Requires SSE4.2 (implied by dpl.xmm).
//
// Forms tested:
// (1) dpp::exscan_max(val, init)
// (2) dpp::exscan_max(val, mask, init)
...
int main() {
    ...
    constexpr auto run = []() {
        ...
        if consteval {
            ...
            return dpl::test::exscan_max<abi_t>::run_all(types{}, engine);
        } else {
