...
JDEP_TARGETS := $(addsuffix .jdep, $(MODULE_SRCS:$(ROOT_DIR)/%=%))
ARG_TARGETS := $(addsuffix .args, $(MODULE_SRCS:$(ROOT_DIR)/%=%))
...
$(ARG_TARGETS):%.cppm.args: $(OUTPUT_DIR)/%.cppm.args
	@

$(JDEP_TARGETS):%.cppm.jdep: $(OUTPUT_DIR)/%.cppm.jdep
	@
