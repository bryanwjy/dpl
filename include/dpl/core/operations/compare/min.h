...
concept unqualified_canonical_immin = requires(S src, L lhs, R rhs) {
    {
        min<const_mask_v<
            canonical_if_zero_t<S, operation_result_t<min_t, L, R>, A>, M>>(
...
template <typename S, typename M, typename L, typename R,
    typename A =
        common_abi_t<canonical_if_zero_t<S, operation_result_t<min_t, L, R>>,
            operation_result_t<min_t, L, R>>>
concept unqualified_extended_immin = requires(S src, L lhs, R rhs) {
    {
        min<const_mask_v<
            canonical_if_zero_t<S, operation_result_t<min_t, L, R>, A>, M>>(
...
public:
    ...
    static constexpr basic_vector<E, SA> operator()(basic_vector<E, SA> src,
        ...
        return min<V>(internal::abi<SA>, src, masked_operation, lhs, rhs);
    }
    ...
    static constexpr auto operator()(S src, Mask mask, L lhs, R rhs) noexcept {
        if constexpr (unqualified_extended_immin<S, Mask, L, R>) {
            ...
            return min<V>(src, masked_operation, lhs, rhs);
        } else {
    ...
    static constexpr basic_vector<E, common_abi_t<LA, RA>> operator()(
        ...
        constexpr auto V = const_mask_v<basic_vector<E, A>, Mask>;
        return min<V>(internal::abi<A>, dx::zero, masked_operation, lhs, rhs);
    }
    ...
    static constexpr auto operator()(Mask mask, L lhs, R rhs) noexcept {
        ...
        if constexpr (unqualified_extended_immin<zero_t, Mask, L, R>) {
            constexpr auto V = const_mask_v<S, Mask>;
            return min<V>(dx::zero, masked_operation, lhs, rhs);
        } else {
