...
public:
    ...
    requires canonical_vector<R> &&
        unqualified_canonical_mmin<source_t<L, R>, mask_t<L, R>, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    requires canonical_vector<R> &&
        unqualified_canonical_mmin<source_t<L, R>, cmask_t<L, R, M>, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(
        source_t<L, R> src, cmask_t<L, R, M> cmask, L lhs, R rhs) noexcept {
        return min(internal::abi<common_abi_t<L, R>>, src, cmask, lhs, rhs);
    }
    ...
    template <canonical_vector L, common_vector_with<L> R, imask_t<L, R> M>
    requires canonical_vector<R> &&
        unqualified_canonical_mmin<dx::zero_t, cmask_t<L, R, M>, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(
        dx::zero_t zero, cmask_t<L, R, M> cmask, L lhs, R rhs) noexcept {
        return min(internal::abi<common_abi_t<L, R>>, zero, cmask, lhs, rhs);
    }
};
...
private:
    ...
    template <typename L, typename R>
    using imask_t DPL_NODEBUG = simask_t<cpo_result_t<min_t, L, R>>;

    template <typename L, typename R, imask_t<L, R> M>
    using cmask_t DPL_NODEBUG = scmask_t<cpo_result_t<min_t, L, R>, M>;
...
public:
    ...
    requires (extended_vector<S> || extended_vector<L> || extended_vector<R>) &&
        unqualified_extended_mmin<S, scmask_t<S, M>, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(
        S&& src, scmask_t<S, M> cmask, L&& lhs, R&& rhs) {
        return min(src, cmask, __DPL forward<L>(lhs), __DPL forward<R>(rhs));
    }
    ...
    requires (extended_vector<L> || extended_vector<R>) &&
        unqualified_extended_mmin<dx::zero_t, cmask_t<L, R, M>, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(
        dx::zero_t zero, cmask_t<L, R, M> cmask, L&& lhs, R&& rhs) {
        return min(zero, cmask, __DPL forward<L>(lhs), __DPL forward<R>(rhs));
    }
};
} // namespace datapar::internal
