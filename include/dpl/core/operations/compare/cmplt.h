...
struct DPL_EMPTY_BASES cmplt_t :
    public comparison_base<cmplt_t>,
    public maskable_predicate_base<cmplt_t> {
...
template <>
struct fallback_impl<cmplt_t> : binary_canonical_broadcaster<cmplt_t> {
    ...
    using binary_canonical_broadcaster<cmplt_t>::operator();
};
...
concept unqualified_canonical_mcmplt =
    cpo_invocable<cmplt_t, L, R> && requires {
        {
            cmplt(internal::abi<T>, internal::declarg<M>(),
...
private:
    template <typename L, typename R>
    using result_t DPL_NODEBUG = cpo_result_t<cmplt_t, L, R>;
...
public:
    ...
    requires canonical_vector<R> && unqualified_canonical_cmplt<L, R>
    ...
    template <unextended_type L, unextended_terminal_of<cmplt_t, L> R>
    requires unqualified_canonical_mcmplt<mask_t<L, R>, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    requires unqualified_canonical_mcmplt<launder_cmask_t<result_t<L, R>, M>, L,
        R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
...
concept unqualified_extended_cmplt = requires(L lhs, R rhs) {
    { cmplt(lhs, rhs) } -> equivalent_mask_with<simd_mask_type_t<T>>;
};

template <typename S, typename L, typename R>
concept unqualified_extended_mcmplt = cpo_invocable<cmplt_t, L, R> && requires {
    {
...
public:
    ...
    requires unqualified_extended_cmplt<L, R, L>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(L&& lhs, R&& rhs) {
        return cmplt(__DPL forward<L>(lhs), __DPL forward<R>(rhs));
    }
    ...
    requires unqualified_extended_cmplt<L, R, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(L&& lhs, R&& rhs) {
        return cmplt(__DPL forward<L>(lhs), __DPL forward<R>(rhs));
    }
    ...
    requires (extended_mask<S> || extended_vector<L> || extended_vector<R>) &&
        ...
        equivalent_mask_with<S, cpo_result_t<cmplt_t, L, R>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    template <typename L, typename R, result_cmask_for<cmplt_t, L, R> M>
    requires (extended_vector<L> || extended_vector<R>) &&
    ...
    static constexpr auto operator()(M mask, L&& lhs, R&& rhs) {
        return cmplt(dx::to_const_mask<cpo_result_t<cmplt_t, L, R>>(mask),
