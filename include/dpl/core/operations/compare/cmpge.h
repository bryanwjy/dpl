...
concept unqualified_extended_cmpge = requires(L lhs, R rhs) {
    { cmpge(lhs, rhs) } -> extended_compare_result<L, R, A>;
};

template <typename L, typename R, typename A = common_abi_t<L, R>>
concept unqualified_cmpge =
    unqualified_canonical_cmpge<L, R> || unqualified_extended_cmpge<L, R> ||
...
concept unqualified_canonical_mcmpge = requires(C mask, L lhs, R rhs) {
    {
...
template <typename C, typename L, typename R,
    typename A = common_abi_t<L, R, C>>
concept unqualified_extended_mcmpge = requires(C mask, L lhs, R rhs) {
    { cmpge(mask, lhs, rhs) } -> extended_compare_result<L, R, A>;
};
...
concept decayable_mcmpge =
    ...
    decayable_vector_for<L, operation_category::lane_agnostic> &&
    decayable_vector_for<R, operation_category::lane_agnostic> &&
    requires(cmpge_t op, canonical_type_t<C> c, canonical_type_t<L> l,
        canonical_type_t<R> r) { op(c, l, r); };

template <typename C, typename L, typename R,
    typename A = common_abi_t<L, R, C>>
concept extended_mcmpge =
    unqualified_extended_mcmpge<C, L, R, A> || decayable_mcmpge<C, L, R, A>;

template <typename M, typename L, typename R, typename A = common_abi_t<L, R>>
concept unqualified_canonical_imcmpge = requires(L lhs, R rhs) {
    {
        cmpge<const_mask_v<operation_result_t<cmpge_t, L, R>, M>>(
            internal::abi<A>, dx::masked_operation, lhs, rhs)
    } -> canonical_compare_result<L, R>;
};

template <typename M, typename L, typename R, typename A = common_abi_t<L, R>>
concept unqualified_extended_imcmpge = requires(L lhs, R rhs) {
    {
        cmpge<const_mask_v<operation_result_t<cmpge_t, L, R>, M>>(
...
concept decayable_imcmpge =
    decayable_vector_for<L, operation_category::lane_agnostic> &&
    decayable_vector_for<R, operation_category::lane_agnostic> &&
    requires(cmpge_t op, M mask, canonical_type_t<L> l, canonical_type_t<R> r) {
        op(mask, l, r);
    };

template <typename M, typename L, typename R, typename A = common_abi_t<L, R>>
concept extended_imcmpge =
    unqualified_extended_imcmpge<M, L, R, A> || decayable_imcmpge<M, L, R, A>;
...
private:
    ...
    requires requires {
        { cmpge(internal::abi<A>, left, right) } -> simd_with_abi<A>;
    }
    {
...
public:
    ...
    static constexpr basic_mask<E, A> operator()(basic_mask<MaskE, A> mask,
        basic_vector<E, A> lhs, basic_vector<E, A> rhs) noexcept {
        if constexpr (unqualified_canonical_mcmpge<basic_mask<MaskE, A>,
    ...
    requires (different_from<SA, common_abi_t<LA, RA>> ||
        ...
        zmaskable_args<basic_mask<MaskE, SA>, basic_vector<E, LA>,
            basic_vector<E, RA>> &&
        unqualified_canonical_mcmpge<basic_mask<MaskE, SA>, basic_vector<E, LA>,
            basic_vector<E, RA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    requires (extended_mask<Mask> || extended_vector<L> ||
                 extended_vector<R>) &&
        zmaskable_args<Mask, L, R> && extended_mcmpge<Mask, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(Mask mask, L lhs, R rhs) noexcept {
        if constexpr (unqualified_extended_mcmpge<Mask, L, R>) {
    ...
    static constexpr basic_mask<E, A> operator()(
        ...
        if constexpr (unqualified_canonical_imcmpge<Mask, basic_vector<E, A>,
                          basic_vector<E, A>>) {
            if consteval {
                return internal::masked<cmpge_t>(mask, lhs, rhs);
            } else {
    ...
    requires (different_from<LA, RA> || scalable_abi<LA> || scalable_abi<RA> ||
        ...
        unqualified_canonical_imcmpge<Mask, basic_vector<E, LA>,
            basic_vector<E, RA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr basic_mask<E, common_abi_t<LA, RA>> operator()(
        ...
        constexpr auto V = const_mask_v<basic_vector<E, A>, Mask>;
        return cmpge<V>(internal::abi<A>, dx::masked_operation, lhs, rhs);
    }

    template <simd_vector L, simd_vector R,
        const_mask_for<operation_result_t<cmpge_t, L, R>> Mask>
    requires (extended_vector<L> || extended_vector<R>) &&
        imm_zmaskable_args<L, R> && extended_imcmpge<Mask, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(Mask mask, L lhs, R rhs) noexcept {
        using S = operation_result_t<cmpge_t, L, R>;
        if constexpr (unqualified_extended_imcmpge<Mask, L, R>) {
            constexpr auto V = const_mask_v<S, Mask>;
            return cmpge<V>(dx::masked_operation, lhs, rhs);
        } else {
