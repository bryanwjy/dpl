...
concept unqualified_canonical_max = requires(L lhs, R rhs) {
    { max(internal::abi<A>, lhs, rhs) } -> canonical_arithmetic_result<L, R, A>;
};

template <typename L, typename R, typename A = common_abi_t<L, R>>
concept unqualified_extended_max = requires(L lhs, R rhs) {
    { max(lhs, rhs) } -> extended_operation_vector<A>;
};

template <typename L, typename R>
concept expression_max = (simd_expression<L> || simd_expression<R>) &&
    invocable<max_t, simd_expression_result_t<L>, simd_expression_result_t<R>>;

template <typename L, typename R, typename A = common_abi_t<L, R>>
concept decayable_max =
...
template <typename L, typename R, typename A = common_abi_t<L, R>>
concept extended_max = unqualified_extended_max<L, R, A> ||
    expression_max<L, R> || decayable_max<L, R, A>;

template <typename S, typename M, typename L, typename R,
    typename A = common_abi_t<L, R, M>>
concept unqualified_canonical_mmax = requires(S src, M mask, L lhs, R rhs) {
    {
        max(internal::abi<A>, src, mask, lhs, rhs)
    } -> equivalent_simd_as<
...
template <typename S, typename M, typename L, typename R,
    typename A = common_abi_t<L, R, M>>
concept unqualified_extended_mmax = requires(S src, M mask, L lhs, R rhs) {
    { max(src, mask, lhs, rhs) } -> extended_operation_vector<A>;
};

template <typename S, typename M, typename L, typename R>
concept expression_mmax = (simd_expression<S> || simd_expression<M> ||
                              simd_expression<L> || simd_expression<R>) &&
    invocable<max_t, expression_result_or_zero_t<S>,
        simd_expression_result_t<M>, simd_expression_result_t<L>,
        simd_expression_result_t<R>>;

template <typename S, typename M, typename L, typename R,
    typename A = common_abi_t<L, R, M>>
concept decayable_mmax =
    ...
    decayable_vector_for<L, operation_category::lane_agnostic> &&
    decayable_vector_for<R, operation_category::lane_agnostic> &&
    regular_invocable<max_t,
        canonical_or_zero_t<S, operation_result_t<max_t, L, R>, A>,
        canonical_type_t<M>, canonical_type_t<L>, canonical_type_t<R>>;

template <typename S, typename M, typename L, typename R,
    typename A = common_abi_t<L, R, M>>
concept extended_mmax = unqualified_extended_mmax<S, M, L, R, A> ||
    expression_mmax<S, M, L, R> || decayable_mmax<S, M, L, R, A>;
...
template <typename S, typename M, typename L, typename R>
concept expression_immax =
    (simd_expression<S> || simd_expression<L> || simd_expression<R>) &&
    invocable<max_t, expression_result_or_zero_t<S>, M,
...
concept decayable_immax =
    ...
    decayable_vector_for<R, operation_category::lane_agnostic> &&
    regular_invocable<max_t,
        canonical_or_zero_t<S, operation_result_t<max_t, L, R>, A>, M,
...
template <typename S, typename M, typename L, typename R,
    typename A =
        common_abi_t<canonical_if_zero_t<S, operation_result_t<max_t, L, R>>,
            operation_result_t<max_t, L, R>>>
concept extended_immax = unqualified_extended_immax<S, M, L, R, A> ||
    expression_immax<S, M, L, R> || decayable_immax<S, M, L, R, A>;
...
private:
    ...
    requires (!simd_class<L> || canonical_vector<L>) &&
        (!simd_class<R> || canonical_vector<R>) && requires(L lhs, R rhs) {
            { max(internal::abi<A>, lhs, rhs) } -> vector_with_abi<A>;
        }
    ...
    requires (!simd_class<L> || extended_vector<L>) &&
        ...
        unqualified_extended_max<L, R, A>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto native(A abi, L lhs, R rhs) noexcept(
        noexcept(max(lhs, rhs))) {
        return max(lhs, rhs);
    }
...
public:
    ...
    static constexpr basic_vector<E, A> operator()(
        basic_vector<E, A> lhs, basic_vector<E, A> rhs) noexcept {
        if constexpr (unqualified_canonical_max<basic_vector<E, A>,
                          basic_vector<E, A>>) {
            if consteval {
    ...
    requires simd_element_for<E, LA> && simd_element_for<E, RA> &&
        (scalable_abi<LA> || scalable_abi<RA> || different_from<LA, RA> ||
            !totally_ordered<E>) &&
        unqualified_canonical_max<basic_vector<E, LA>, basic_vector<E, RA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr basic_vector<E, common_abi_t<LA, RA>> operator()(
        basic_vector<E, LA> lhs, basic_vector<E, RA> rhs) noexcept {
        return max(internal::abi<common_abi_t<LA, RA>>, lhs, rhs);
    }

    template <simd_vector L, simd_vector R>
    requires (extended_vector<L> || extended_vector<R>) && extended_max<L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(L lhs, R rhs) noexcept {
        if constexpr (unqualified_extended_max<L, R>) {
            return max(lhs, rhs);
        } else if constexpr (expression_max<L, R>) {
    ...
    static constexpr basic_vector<E, A> operator()(basic_vector<E, A> src,
        ...
        if constexpr (unqualified_canonical_mmax<basic_vector<E, A>,
                          basic_mask<ME, A>, basic_vector<E, A>,
                          basic_vector<E, A>>) {
            if consteval {
                return internal::masked<max_t>(src, mask, lhs, rhs);
            } else {
                return max(internal::abi<A>, src, mask, lhs, rhs);
            }
        } else {
            return internal::masked<max_t>(src, mask, lhs, rhs);
        }
    ...
    requires (different_from<LA, RA> || scalable_abi<SA> || scalable_abi<LA> ||
        ...
        unqualified_canonical_mmax<basic_vector<E, SA>, basic_mask<ME, SA>,
            basic_vector<E, LA>, basic_vector<E, RA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr basic_vector<E, SA> operator()(basic_vector<E, SA> src,
        ...
        return max(internal::abi<SA>, src, mask, lhs, rhs);
    }
    ...
    requires (extended_vector<S> || extended_mask<M> || extended_vector<L> ||
                 extended_vector<R>) &&
        maskable_args<S, M, L, R> && extended_mmax<S, M, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(S src, M mask, L lhs, R rhs) noexcept {
        if constexpr (unqualified_extended_mmax<S, M, L, R>) {
            return max(src, mask, lhs, rhs);
        } else if constexpr (expression_mmax<S, M, L, R>) {
    ...
    static constexpr basic_vector<E, A> operator()(basic_mask<ME, A> mask,
        basic_vector<E, A> lhs, basic_vector<E, A> rhs) noexcept {
        if constexpr (unqualified_canonical_mmax<zero_t, basic_mask<ME, A>,
                          basic_vector<E, A>, basic_vector<E, A>>) {
            if consteval {
                return internal::masked<max_t>(mask, lhs, rhs);
            } else {
                return max(internal::abi<A>, dx::zero, mask, lhs, rhs);
            }
        } else {
    ...
    requires (different_from<SA, common_abi_t<LA, RA>> ||
        ...
        zmaskable_args<basic_mask<ME, SA>, basic_vector<E, LA>,
            basic_vector<E, RA>> &&
        unqualified_canonical_mmax<zero_t, basic_mask<ME, SA>,
            basic_vector<E, LA>, basic_vector<E, RA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    requires (extended_mask<M> || extended_vector<L> || extended_vector<R>) &&
        zmaskable_args<M, L, R> && extended_mmax<zero_t, M, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(M mask, L lhs, R rhs) noexcept {
        if constexpr (unqualified_extended_mmax<zero_t, M, L, R>) {
            return max(dx::zero, mask, lhs, rhs);
        } else if constexpr (expression_mmax<zero_t, M, L, R>) {
            return operator()(
    ...
    template <simd_mask M, simd_vector L, simd_vector R>
    requires invocable<max_t, M, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    static constexpr basic_vector<E, A> operator()(basic_vector<E, A> src,
        ...
        if constexpr (unqualified_canonical_immax<basic_vector<E, A>, M,
                          basic_vector<E, A>, basic_vector<E, A>>) {
            if consteval {
                return internal::masked<max_t>(src, mask, lhs, rhs);
            } else {
    ...
    requires (different_from<SA, common_abi_t<LA, RA>> ||
        ...
        imm_maskable_args<basic_vector<E, SA>, basic_vector<E, LA>,
            basic_vector<E, RA>> &&
        unqualified_canonical_immax<basic_vector<E, SA>, M, basic_vector<E, LA>,
            basic_vector<E, RA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr basic_vector<E, SA> operator()(basic_vector<E, SA> src,
    ...
    requires (extended_vector<S> || extended_vector<L> || extended_vector<R>) &&
        imm_maskable_args<S, L, R> && extended_immax<S, M, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(S src, M mask, L lhs, R rhs) noexcept {
        if constexpr (unqualified_extended_immax<S, M, L, R>) {
            return max(src, dx::to_compatible_const_mask<S>(mask), lhs, rhs);
        } else if constexpr (expression_immax<S, M, L, R>) {
            return operator()(
    ...
    static constexpr basic_vector<E, A> operator()(
        M mask, basic_vector<E, A> lhs, basic_vector<E, A> rhs) noexcept {
        if constexpr (unqualified_canonical_immax<zero_t, M, basic_vector<E, A>,
                          basic_vector<E, A>>) {
            if consteval {
                return internal::masked<max_t>(mask, lhs, rhs);
            } else {
    ...
    requires (different_from<LA, RA> || scalable_abi<LA> || scalable_abi<RA> ||
        ...
        unqualified_canonical_immax<zero_t, M, basic_vector<E, LA>,
            basic_vector<E, RA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr basic_vector<E, common_abi_t<LA, RA>> operator()(
        M mask, basic_vector<E, LA> lhs, basic_vector<E, RA> rhs) noexcept {
    ...
    template <simd_vector L, simd_vector R,
        const_mask_for<operation_result_t<max_t, L, R>> M>
    requires (extended_vector<L> || extended_vector<R>) &&
        imm_zmaskable_args<L, R> && extended_immax<zero_t, M, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(M mask, L lhs, R rhs) noexcept {
        using S = operation_result_t<max_t, L, R>;
        constexpr auto cmask = dx::to_compatible_const_mask<S>(mask);
        if constexpr (unqualified_extended_immax<zero_t, M, L, R>) {
            return max(dx::zero, cmask, lhs, rhs);
        } else if constexpr (expression_immax<zero_t, M, L, R>) {
