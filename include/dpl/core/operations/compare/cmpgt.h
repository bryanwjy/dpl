...
struct DPL_EMPTY_BASES cmpgt_t :
    ...
    public binary_broadcastable_operation<cmpgt_t> {
...
concept unqualified_canonical_mcmpgt = cpo_invocable<cmpgt_t, L, R> &&
...
public:
    ...
    requires canonical_vector<R> &&
        unqualified_canonical_mcmpgt<launder_cmask_t<mask_t<L, R>, M>, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
...
template <typename S, typename L, typename R>
concept unqualified_extended_mcmpgt = cpo_invocable<cmpgt_t, L, R> &&
...
public:
    ...
    static constexpr auto operator()(M cmask, L&& lhs, R&& rhs) {
        return cmpgt(dx::to_const_mask<cpo_result_t<cmpgt_t, L, R>>(cmask),
