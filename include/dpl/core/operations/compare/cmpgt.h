...
concept unqualified_cmpgt =
    unqualified_canonical_cmpgt<L, R> || unqualified_extended_cmpgt<L, R> ||
...
concept unqualified_canonical_imcmpgt = requires(L lhs, R rhs) {
    {
...
template <typename M, typename L, typename R, typename A = common_abi_t<L, R>>
concept unqualified_extended_imcmpgt = requires(L lhs, R rhs) {
    {
        cmpgt<const_mask_v<operation_result_t<cmpgt_t, L, R>, M>>(
...
public:
    ...
    static constexpr basic_mask<E, common_abi_t<LA, RA>> operator()(
        ...
        constexpr auto V = const_mask_v<basic_vector<E, A>, Mask>;
        return cmpgt<V>(internal::abi<A>, dx::masked_operation, lhs, rhs);
    }
    ...
    static constexpr auto operator()(Mask mask, L lhs, R rhs) noexcept {
        ...
        if constexpr (unqualified_extended_imcmpgt<Mask, L, R>) {
            ...
            return cmpgt<V>(dx::masked_operation, lhs, rhs);
        } else {
