...
template <>
struct fallback_impl<cmpeq_t> : binary_canonical_broadcaster<cmpeq_t> {
...
concept unqualified_canonical_mcmpeq =
    cpo_invocable<cmpeq_t, L, R> && requires {
        {
            cmpeq(internal::abi<T>, internal::declarg<M>(),
...
private:
    template <typename L, typename R>
    using result_t DPL_NODEBUG = cpo_result_t<cmpeq_t, L, R>;
...
public:
    ...
    requires canonical_mask<R> && unqualified_canonical_cmpeq<L, R>
    ...
    requires canonical_vector<R> && unqualified_canonical_cmpeq<L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr main_result_t<L, R> operator()(L lhs, R rhs) noexcept {
        return cmpeq(internal::abi<common_abi_t<L, R>>, lhs, rhs);
    }
    ...
    requires unqualified_canonical_cmpeq<L, R, L>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    requires unqualified_canonical_cmpeq<L, R, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr simd_mask_type_t<R> operator()(L&& lhs, R rhs) noexcept {
        return cmpeq(internal::abi<R>, __DPL forward<L>(lhs), rhs);
    }

    template <unextended_type L, unextended_terminal_of<cmpeq_t, L> R>
    requires unqualified_canonical_mcmpeq<mask_t<L, R>, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    requires unqualified_canonical_mcmpeq<launder_cmask_t<result_t<L, R>, M>, L,
        R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
...
concept unqualified_extended_cmpeq = requires(L lhs, R rhs) {
    { cmpeq(lhs, rhs) } -> equivalent_mask_with<simd_mask_type_t<T>>;
};

template <typename S, typename L, typename R>
concept unqualified_extended_mcmpeq = cpo_invocable<cmpeq_t, L, R> && requires {
    {
...
public:
    ...
    requires unqualified_extended_cmpeq<L, R, L>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(L&& lhs, R&& rhs) {
        return cmpeq(__DPL forward<L>(lhs), __DPL forward<R>(rhs));
    }
    ...
    requires unqualified_extended_cmpeq<L, R, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(L&& lhs, R&& rhs) {
        return cmpeq(__DPL forward<L>(lhs), __DPL forward<R>(rhs));
    }
    ...
    requires (extended_mask<S> || extended_vector<L> || extended_vector<R>) &&
        ...
        equivalent_mask_with<S, cpo_result_t<cmpeq_t, L, R>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    template <typename L, typename R, result_cmask_for<cmpeq_t, L, R> M>
    requires (extended_vector<L> || extended_vector<R>) &&
    ...
    static constexpr auto operator()(M mask, L&& lhs, R&& rhs) {
        return cmpeq(dx::to_const_mask<cpo_result_t<cmpeq_t, L, R>>(mask),
