...
concept unqualified_extended_cmpeq = requires(L lhs, R rhs) {
    { cmpeq(lhs, rhs) } -> extended_compare_result<L, R, A>;
};

template <typename L, typename R, typename A = common_abi_t<L, R>>
concept unqualified_cmpeq =
    unqualified_canonical_cmpeq<L, R> || unqualified_extended_cmpeq<L, R> ||
...
concept unqualified_extended_mask_cmpeq = requires(L lhs, R rhs) {
    { cmpeq(lhs, rhs) } -> extended_compare_mask<L, R, A>;
};

template <typename L, typename R, typename A = common_abi_t<L, R>>
concept unqualified_mask_cmpeq = unqualified_canonical_mask_cmpeq<L, R> ||
    unqualified_extended_mask_cmpeq<L, R> ||
...
concept unqualified_canonical_mcmpeq = requires(C mask, L lhs, R rhs) {
    {
...
template <typename C, typename L, typename R,
    typename A = common_abi_t<L, R, C>>
concept unqualified_extended_mcmpeq = requires(C mask, L lhs, R rhs) {
    { cmpeq(mask, lhs, rhs) } -> extended_compare_result<L, R, A>;
};
...
concept decayable_mcmpeq =
    ...
    decayable_vector_for<L, operation_category::lane_agnostic> &&
    decayable_vector_for<R, operation_category::lane_agnostic> &&
    requires(cmpeq_t op, canonical_type_t<C> c, canonical_type_t<L> l,
        canonical_type_t<R> r) { op(c, l, r); };

template <typename C, typename L, typename R,
    typename A = common_abi_t<L, R, C>>
concept extended_mcmpeq =
    unqualified_extended_mcmpeq<C, L, R, A> || decayable_mcmpeq<C, L, R, A>;

template <typename M, typename L, typename R, typename A = common_abi_t<L, R>>
concept unqualified_canonical_imcmpeq = requires(L lhs, R rhs) {
    {
        cmpeq<const_mask_v<operation_result_t<cmpeq_t, L, R>, M>>(
            internal::abi<A>, dx::masked_operation, lhs, rhs)
    } -> canonical_compare_result<L, R>;
};

template <typename M, typename L, typename R, typename A = common_abi_t<L, R>>
concept unqualified_extended_imcmpeq = requires(L lhs, R rhs) {
    {
        cmpeq<const_mask_v<operation_result_t<cmpeq_t, L, R>, M>>(
...
concept decayable_imcmpeq =
    decayable_vector_for<L, operation_category::lane_agnostic> &&
    decayable_vector_for<R, operation_category::lane_agnostic> &&
    requires(cmpeq_t op, M mask, canonical_type_t<L> l, canonical_type_t<R> r) {
        op(mask, l, r);
    };

template <typename M, typename L, typename R, typename A = common_abi_t<L, R>>
concept extended_imcmpeq =
    unqualified_extended_imcmpeq<M, L, R, A> || decayable_imcmpeq<M, L, R, A>;
...
public:
    ...
    static constexpr basic_mask<E, A> operator()(basic_mask<MaskE, A> mask,
        basic_vector<E, A> lhs, basic_vector<E, A> rhs) noexcept {
        if constexpr (unqualified_canonical_mcmpeq<basic_mask<MaskE, A>,
    ...
    requires (different_from<SA, common_abi_t<LA, RA>> ||
        ...
        zmaskable_args<basic_mask<MaskE, SA>, basic_vector<E, LA>,
            basic_vector<E, RA>> &&
        unqualified_canonical_mcmpeq<basic_mask<MaskE, SA>, basic_vector<E, LA>,
            basic_vector<E, RA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    requires (extended_mask<Mask> || extended_vector<L> ||
                 extended_vector<R>) &&
        zmaskable_args<Mask, L, R> && extended_mcmpeq<Mask, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(Mask mask, L lhs, R rhs) noexcept {
        if constexpr (unqualified_extended_mcmpeq<Mask, L, R>) {
    ...
    static constexpr basic_mask<E, A> operator()(
        ...
        if constexpr (unqualified_canonical_imcmpeq<Mask, basic_vector<E, A>,
                          basic_vector<E, A>>) {
            if consteval {
                return internal::masked<cmpeq_t>(mask, lhs, rhs);
            } else {
    ...
    requires (different_from<LA, RA> || scalable_abi<LA> || scalable_abi<RA> ||
        ...
        unqualified_canonical_imcmpeq<Mask, basic_vector<E, LA>,
            basic_vector<E, RA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr basic_mask<E, common_abi_t<LA, RA>> operator()(
        ...
        constexpr auto V = const_mask_v<basic_vector<E, A>, Mask>;
        return cmpeq<V>(internal::abi<A>, dx::masked_operation, lhs, rhs);
    }

    template <simd_vector L, simd_vector R,
        const_mask_for<operation_result_t<cmpeq_t, L, R>> Mask>
    requires (extended_vector<L> || extended_vector<R>) &&
        imm_zmaskable_args<L, R> && extended_imcmpeq<Mask, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(Mask mask, L lhs, R rhs) noexcept {
        using S = operation_result_t<cmpeq_t, L, R>;
        if constexpr (unqualified_extended_imcmpeq<Mask, L, R>) {
            constexpr auto V = const_mask_v<S, Mask>;
            return cmpeq<V>(dx::masked_operation, lhs, rhs);
        } else {
