...
struct DPL_EMPTY_BASES bwand_t :
    public bitwise_base<bwand_t>,
    public maskable_transform_base<bwand_t> {
...
template <>
struct fallback_impl<bwand_t> : binary_canonical_broadcaster<bwand_t> {
    ...
    using binary_canonical_broadcaster<bwand_t>::operator();
};

template <typename L, typename R, typename T = common_canonical_simd_t<L, R>>
concept unqualified_canonical_bwand = requires {
    {
        bwand(internal::abi<T>, internal::declarg<L>(), internal::declarg<R>())
    } -> same_as<T>;
};

template <typename S, typename M, typename L, typename R, typename T>
concept unqualified_canonical_mbwand_base =
    cpo_invocable<bwand_t, L, R> && requires {
        {
            bwand(internal::abi<T>, internal::declarg<S>(),
...
concept unqualified_canonical_mbwand =
    unqualified_canonical_mbwand_base<T, M, L, R, T>;

template <typename M, typename L, typename R,
    typename T = common_canonical_simd_t<L, R>>
concept unqualified_canonical_zmbwand =
    unqualified_canonical_mbwand_base<dx::zero_t, M, L, R, T>;

template <>
struct canonical_impl<bwand_t> {
private:
    template <typename L, typename R>
    using result_t DPL_NODEBUG = cpo_result_t<bwand_t, L, R>;
    template <typename L, typename R>
    using mask_t DPL_NODEBUG = simd_mask_type_t<result_t<L, R>>;

public:
    template <canonical_vector L, common_vector_with<L> R>
    requires canonical_vector<R> && unqualified_canonical_bwand<L, R>
    ...
    requires canonical_mask<R> && unqualified_canonical_bwand<L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr common_canonical_simd_t<L, R> operator()(
        L lhs, R rhs) noexcept {
        return bwand(internal::abi<common_abi_t<L, R>>, lhs, rhs);
    ...
    requires unqualified_canonical_bwand<L, R, L>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    requires unqualified_canonical_bwand<L, R, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    requires unqualified_canonical_mbwand<mask_t<L, R>, L, R, result_t<L, R>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr result_t<L, R> operator()(
        result_t<L, R> src, mask_t<L, R> mask, L&& lhs, R&& rhs) noexcept {
        return bwand(internal::abi<result_t<L, R>>, src, mask,
            __DPL forward<L>(lhs), __DPL forward<R>(rhs));
    }

    template <unextended_type L, unextended_type R, result_cmask_for<L, R> M>
    requires unqualified_canonical_mbwand<launder_cmask_t<result_t<L, R>, M>, L,
        R, result_t<L, R>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr result_t<L, R> operator()(
        result_t<L, R> src, M mask, L&& lhs, R&& rhs) noexcept {
        using T = result_t<L, R>;
        return bwand(internal::abi<T>, src, dx::to_const_mask<T>(mask),
            __DPL forward<L>(lhs), __DPL forward<R>(rhs));
    }

    template <unextended_type L, unextended_type R>
    requires unqualified_canonical_zmbwand<mask_t<L, R>, L, R, result_t<L, R>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr result_t<L, R> operator()(
        dx::zero_t zero, mask_t<L, R> mask, L&& lhs, R&& rhs) noexcept {
        return bwand(internal::abi<result_t<L, R>>, zero, mask,
            __DPL forward<L>(lhs), __DPL forward<R>(rhs));
    }

    template <unextended_type L, unextended_type R, result_cmask_for<L, R> M>
    requires unqualified_canonical_zmbwand<launder_cmask_t<result_t<L, R>, M>,
        L, R, result_t<L, R>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr result_t<L, R> operator()(
        dx::zero_t zero, M mask, L&& lhs, R&& rhs) noexcept {
        using T = result_t<L, R>;
        return bwand(internal::abi<T>, zero, dx::to_const_mask<T>(mask),
            __DPL forward<L>(lhs), __DPL forward<R>(rhs));
    }
};
...
concept unqualified_extended_mbwand_base =
    cpo_invocable<bwand_t, L, R> && requires {
        {
...
concept unqualified_extended_mbwand =
    equivalent_vector_with<S, cpo_result_t<bwand_t, L, R>> &&
    unqualified_extended_mbwand_base<S, M, L, R>;

template <typename M, typename L, typename R>
concept unqualified_extended_zmbwand =
    unqualified_extended_mbwand_base<dx::zero_t, M, L, R>;
...
public:
    ...
    requires (extended_vector<L> || extended_vector<R>) &&
        unqualified_extended_bwand<L, R, common_canonical_simd_t<L, R>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(L&& lhs, R&& rhs) {
    ...
    requires unqualified_extended_bwand<L, R, L>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(L&& lhs, R&& rhs) {
        return bwand(__DPL forward<L>(lhs), __DPL forward<R>(rhs));
    }
    ...
    requires unqualified_extended_bwand<L, R, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(L&& lhs, R&& rhs) {
        return bwand(__DPL forward<L>(lhs), __DPL forward<R>(rhs));
    }
    ...
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(S&& src, M mask, L&& lhs, R&& rhs) {
        return bwand(src, dx::to_const_mask<S>(mask), __DPL forward<L>(lhs),
            __DPL forward<R>(rhs));
    }

    template <typename L, typename R, result_mask_for<bwand_t, L, R> M>
    requires (extended_mask<M> || extended_vector<L> || extended_vector<R>) &&
        unqualified_extended_zmbwand<M, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    template <typename L, typename R, result_cmask_for<bwand_t, L, R> M>
    requires (extended_vector<L> || extended_vector<R>) &&
        unqualified_extended_zmbwand<
            launder_cmask_t<cpo_result_t<bwand_t, L, R>, M>, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(
        dx::zero_t zero, M mask, L&& lhs, R&& rhs) {
        return bwand(zero, dx::to_const_mask<cpo_result_t<bwand_t, L, R>>(mask),
            __DPL forward<L>(lhs), __DPL forward<R>(rhs));
    }
};
...
inline constexpr internal::bwand_t bwand{};
} // namespace cpo
} // namespace datapar
