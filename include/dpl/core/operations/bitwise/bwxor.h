...
template <typename L, typename R, typename A = common_abi_t<L, R>>
concept unqualified_extended_bwxor = requires(L lhs, R rhs) {
    { bwxor(lhs, rhs) } -> extended_operation_vector<A>;
};

template <typename L, typename R>
concept expression_bwxor = (simd_expression<L> || simd_expression<R>) &&
    invocable<abs_t, simd_expression_result_t<L>, simd_expression_result_t<R>>;

template <typename L, typename R, typename A = common_abi_t<L, R>>
concept decayable_bwxor =
...
template <typename L, typename R, typename A = common_abi_t<L, R>>
concept extended_bwxor = unqualified_extended_bwxor<L, R, A> ||
    expression_bwxor<L, R> || decayable_bwxor<L, R, A>;

template <typename S, typename M, typename L, typename R,
    typename A = common_abi_t<L, R, M>>
concept unqualified_canonical_mbwxor = requires(S src, M mask, L lhs, R rhs) {
    {
...
template <typename S, typename M, typename L, typename R,
    typename A = common_abi_t<L, R, M>>
concept unqualified_extended_mbwxor = requires(S src, M mask, L lhs, R rhs) {
    { bwxor(src, mask, lhs, rhs) } -> extended_operation_vector<A>;
};

template <typename S, typename M, typename L, typename R>
concept expression_mbwxor = (simd_expression<S> || simd_expression<M> ||
                                simd_expression<L> || simd_expression<R>) &&
    invocable<bwxor_t, expression_result_or_zero_t<S>,
...
concept decayable_mbwxor =
    ...
    decayable_vector_for<L, operation_category::lane_agnostic> &&
    decayable_vector_for<R, operation_category::lane_agnostic> &&
    regular_invocable<bwxor_t,
        canonical_if_zero_t<S, operation_result_t<bwxor_t, L, R>, A>,
        canonical_type_t<M>, canonical_type_t<L>, canonical_type_t<R>>;

template <typename S, typename M, typename L, typename R,
    typename A = common_abi_t<L, R, M>>
concept extended_mbwxor = unqualified_extended_mbwxor<S, M, L, R, A> ||
    expression_mbwxor<S, M, L, R> || decayable_mbwxor<S, M, L, R, A>;

template <typename S, typename M, typename L, typename R,
    typename A =
        common_abi_t<canonical_if_zero_t<S, operation_result_t<bwxor_t, L, R>>,
            operation_result_t<bwxor_t, L, R>>>
concept unqualified_canonical_imbwxor = requires(S src, M mask, L lhs, R rhs) {
    {
        bwxor(internal::abi<A>, src,
            internal::to_const_mask<A, bwxor_t, S, L, R>(mask), lhs, rhs)
    } -> equivalent_simd_as<
        canonical_if_zero_t<S, operation_result_t<bwxor_t, L, R>, A>>;
};
...
template <typename S, typename M, typename L, typename R,
    typename A =
        common_abi_t<canonical_if_zero_t<S, operation_result_t<bwxor_t, L, R>>,
            operation_result_t<bwxor_t, L, R>>>
concept unqualified_extended_imbwxor = requires(S src, M mask, L lhs, R rhs) {
    {
        bwxor(src, internal::to_const_mask<A, bwxor_t, S, L, R>(mask), lhs, rhs)
    } -> extended_operation_vector<A>;
};

template <typename S, typename M, typename L, typename R>
concept expression_imbwxor =
    (simd_expression<S> || simd_expression<L> || simd_expression<R>) &&
    invocable<bwxor_t, expression_result_or_zero_t<S>, M,
...
concept extended_imbwxor = unqualified_extended_imbwxor<S, M, L, R, A> ||
    expression_imbwxor<S, M, L, R> || decayable_imbwxor<S, M, L, R, A>;
...
concept unqualified_extended_mask_bwxor = requires(L lhs, R rhs) {
    { bwxor(lhs, rhs) } -> extended_operation_mask<A>;
};

template <typename L, typename R>
concept expression_mask_bwxor = (mask_expression<L> || mask_expression<R>) &&
    invocable<bwxor_t, simd_expression_result_t<L>,
        simd_expression_result_t<R>>;

template <typename L, typename R, typename A = common_abi_t<L, R>>
concept decayable_mask_bwxor =
    decayable_mask_for<L, operation_category::lane_agnostic> &&
    decayable_mask_for<R, operation_category::lane_agnostic> &&
    regular_invocable<bwxor_t, canonical_type_t<L>, canonical_type_t<R>>;

template <typename L, typename R, typename A = common_abi_t<L, R>>
concept extended_mask_bwxor = unqualified_extended_mask_bwxor<L, R, A> ||
    decayable_mask_bwxor<L, R> || decayable_mask_bwxor<L, R, A>;

struct bwxor_t : private binary_operation_base<bwxor_t> {
private:
    ...
    requires (!simd_class<L> || canonical_vector<L>) &&
        (!simd_class<R> || canonical_vector<R>) && requires(L lhs, R rhs) {
            { bwxor(internal::abi<A>, lhs, rhs) } -> vector_with_abi<A>;
        }
    ...
    requires (!simd_class<L> || canonical_mask<L>) &&
        (!simd_class<R> || canonical_mask<R>) && requires(L lhs, R rhs) {
            { bwxor(internal::abi<A>, lhs, rhs) } -> mask_with_abi<A>;
        }
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto native(A abi, L lhs, R rhs) noexcept {
        return bwxor(internal::abi<A>, lhs, rhs);
...
public:
    ...
    static constexpr basic_vector<E, A> operator()(
        ...
        if constexpr (unqualified_canonical_bwxor<basic_vector<E, A>,
                          basic_vector<E, A>>) {
            if consteval {
    ...
    requires simd_element_for<E, LA> && simd_element_for<E, RA> &&
        ...
        unqualified_canonical_bwxor<basic_vector<E, LA>, basic_vector<E, RA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    requires (extended_vector<L> || extended_vector<R>) && extended_bwxor<L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(L lhs, R rhs) noexcept {
        if constexpr (unqualified_extended_bwxor<L, R>) {
            return bwxor(lhs, rhs);
        } else if constexpr (expression_bwxor<L, R>) {
    ...
    static constexpr basic_vector<E, A> operator()(basic_vector<E, A> src,
        ...
        if constexpr (unqualified_canonical_mbwxor<basic_vector<E, A>,
            ...
            if consteval {
                ...
                return bwxor(internal::abi<A>, src, mask, lhs, rhs);
            }
        } else {
            return internal::masked<bwxor_t>(src, mask, lhs, rhs);
        }
    ...
    requires (different_from<LA, RA> || scalable_abi<SA> || scalable_abi<LA> ||
        ...
        unqualified_canonical_mbwxor<basic_vector<E, SA>, basic_mask<ME, SA>,
            basic_vector<E, LA>, basic_vector<E, RA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr basic_vector<E, SA> operator()(basic_vector<E, SA> src,
        ...
        return bwxor(internal::abi<SA>, src, mask, lhs, rhs);
    }
    ...
    requires (extended_vector<S> || extended_mask<M> || extended_vector<L> ||
                 extended_vector<R>) &&
        maskable_args<S, M, L, R> && extended_mbwxor<S, M, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(S src, M mask, L lhs, R rhs) noexcept {
        if constexpr (unqualified_extended_mbwxor<S, M, L, R>) {
            return bwxor(src, mask, lhs, rhs);
        } else if constexpr (expression_mbwxor<S, M, L, R>) {
    ...
    static constexpr basic_vector<E, A> operator()(basic_mask<ME, A> mask,
        basic_vector<E, A> lhs, basic_vector<E, A> rhs) noexcept {
        if constexpr (unqualified_canonical_mbwxor<zero_t, basic_mask<ME, A>,
                          basic_vector<E, A>, basic_vector<E, A>>) {
            if consteval {
                return internal::masked<bwxor_t>(mask, lhs, rhs);
            } else {
                return bwxor(internal::abi<A>, dx::zero, mask, lhs, rhs);
            }
        } else {
    ...
    requires (different_from<SA, common_abi_t<LA, RA>> ||
        ...
        zmaskable_args<basic_mask<ME, SA>, basic_vector<E, LA>,
            basic_vector<E, RA>> &&
        unqualified_canonical_mbwxor<zero_t, basic_mask<ME, SA>,
            basic_vector<E, LA>, basic_vector<E, RA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    requires (extended_mask<M> || extended_vector<L> || extended_vector<R>) &&
        zmaskable_args<M, L, R> && extended_mbwxor<zero_t, M, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(M mask, L lhs, R rhs) noexcept {
        if constexpr (unqualified_extended_mbwxor<zero_t, M, L, R>) {
            return bwxor(dx::zero, mask, lhs, rhs);
        } else if constexpr (expression_mbwxor<zero_t, M, L, R>) {
            return operator()(
    ...
    template <simd_mask M, simd_vector L, simd_vector R>
    requires invocable<bwxor_t, M, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    static constexpr basic_vector<E, A> operator()(basic_vector<E, A> src,
        ...
        if constexpr (unqualified_canonical_imbwxor<basic_vector<E, A>, M,
                          basic_vector<E, A>, basic_vector<E, A>>) {
            if consteval {
                return internal::masked<bwxor_t>(src, mask, lhs, rhs);
            } else {
                return bwxor(internal::abi<A>, src,
    ...
    requires (different_from<SA, common_abi_t<LA, RA>> ||
        ...
        imm_maskable_args<basic_vector<E, SA>, basic_vector<E, LA>,
            basic_vector<E, RA>> &&
        unqualified_canonical_imbwxor<basic_vector<E, SA>, M,
            basic_vector<E, LA>, basic_vector<E, RA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr basic_vector<E, SA> operator()(basic_vector<E, SA> src,
        M mask, basic_vector<E, LA> lhs, basic_vector<E, RA> rhs) noexcept {
        return bwxor(internal::abi<SA>, src,
    ...
    requires (extended_vector<S> || extended_vector<L> || extended_vector<R>) &&
        imm_maskable_args<S, L, R> && extended_imbwxor<S, M, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(S src, M mask, L lhs, R rhs) noexcept {
        if constexpr (unqualified_extended_imbwxor<S, M, L, R>) {
            return bwxor(src, dx::to_compatible_const_mask<S>(mask), lhs, rhs);
        } else if constexpr (expression_imbwxor<S, M, L, R>) {
            return operator()(
    ...
    static constexpr basic_vector<E, A> operator()(
        M mask, basic_vector<E, A> lhs, basic_vector<E, A> rhs) noexcept {
        if constexpr (unqualified_canonical_imbwxor<zero_t, M,
                          basic_vector<E, A>, basic_vector<E, A>>) {
            if consteval {
                return internal::masked<bwxor_t>(mask, lhs, rhs);
            } else {
                return bwxor(internal::abi<A>, dx::zero,
                    dx::to_compatible_const_mask<basic_vector<E, A>>(mask), lhs,
                    rhs);
            }
        } else {
    ...
    requires (different_from<LA, RA> || scalable_abi<LA> || scalable_abi<RA>) &&
        ...
        unqualified_canonical_imbwxor<zero_t, M, basic_vector<E, LA>,
            basic_vector<E, RA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr basic_vector<E, common_abi_t<LA, RA>> operator()(
        M mask, basic_vector<E, LA> lhs, basic_vector<E, RA> rhs) noexcept {
    ...
    template <simd_vector L, simd_vector R,
        const_mask_for<operation_result_t<bwxor_t, L, R>> M>
    requires (extended_vector<L> || extended_vector<R>) &&
        imm_zmaskable_args<L, R> && extended_imbwxor<zero_t, M, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(M mask, L lhs, R rhs) noexcept {
        using S = operation_result_t<bwxor_t, L, R>;
        constexpr auto cmask = dx::to_compatible_const_mask<S>(mask);
        if constexpr (unqualified_extended_imbwxor<zero_t, M, L, R>) {
            return bwxor(dx::zero, cmask, lhs, rhs);
        } else if constexpr (expression_imbwxor<zero_t, M, L, R>) {
    ...
    requires (extended_mask<L> || extended_mask<R>) && extended_mask_bwxor<L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(L lhs, R rhs) noexcept {
        if constexpr (unqualified_extended_mask_bwxor<L, R>) {
            return bwxor(lhs, rhs);
        } else if constexpr (expression_mask_bwxor<L, R>) {
            return operator()(dx::evaluate(lhs), dx::evaluate(rhs));
        } else {
            return operator()(dx::to_canonical(lhs), dx::to_canonical(rhs));
        }
