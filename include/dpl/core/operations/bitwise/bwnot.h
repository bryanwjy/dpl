...
concept unqualified_bwnot =
    ...
    (decayable_vector_for<T, operation_category::lane_agnostic> &&
        unqualified_canonical_bwnot<canonical_type_t<T>>);

template <typename T, typename A = typename T::abi_type>
concept unqualified_canonical_mask_not = requires(T val) {
    { bwnot(internal::abi<A>, val) } -> canonical_mask_result<T, T, A>;
};

template <typename T, typename A = typename T::abi_type>
concept unqualified_extended_mask_not = requires(T val) {
    { bwnot(val) } -> extended_mask_result<T, T, A>;
};

template <typename T, typename A = typename T::abi_type>
concept unqualified_mask_not = unqualified_canonical_mask_not<T, A> ||
    unqualified_extended_mask_not<T, A> ||
    (decayable_mask_for<T, operation_category::lane_agnostic> &&
        unqualified_canonical_mask_not<canonical_type_t<T>, A>);
...
concept extended_mbwnot =
    unqualified_extended_mbwnot<S, C, T, A> || decayable_mbwnot<Op, S, C, T, A>;
...
public:
    ...
    static constexpr basic_mask<E, A> operator()(
        basic_mask<E, A> val) noexcept {
        if constexpr (unqualified_canonical_mask_not<basic_mask<E, A>>) {
            if consteval {
                return fallback(val);
            } else {
                return bwnot(internal::abi<A>, val);
    ...
    template <simd_abi A, simd_element_for<A> E>
    requires scalable_abi<A> && unqualified_canonical_mask_not<basic_mask<E, A>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr basic_mask<E, A> operator()(
        basic_mask<E, A> val) noexcept {
        return bwnot(internal::abi<A>, val);
    ...
    requires unqualified_mask_not<T>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(T val) noexcept {
        if constexpr (unqualified_extended_mask_not<T>) {
            return bwnot(val);
        } else {
            return operator()(dx::to_canonical(val));
        }
    ...
    requires (extended_vector<Pass> || extended_mask<Mask> ||
        ...
        extended_mbwnot<bwnot_t, Pass, Mask, Arg>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    static constexpr basic_vector<E, A> operator()(
        basic_mask<MaskE, A> mask, basic_vector<E, A> val) noexcept {
        if constexpr (unqualified_canonical_mbwnot<zero_t, basic_mask<MaskE, A>,
                          basic_vector<E, A>>) {
            if consteval {
    ...
    requires (extended_mask<Mask> || extended_vector<Arg>) &&
        zmaskable_args<Mask, Arg> && extended_mbwnot<bwnot_t, zero_t, Mask, Arg>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    requires (extended_vector<Pass> || extended_vector<Arg>) &&
        ...
        extended_imbwnot<bwnot_t, Pass, Mask, Arg>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(Pass pass, Mask mask, Arg arg) noexcept {
        if constexpr (unqualified_extended_mbwnot<Pass, Mask, Arg>) {
    ...
    static constexpr basic_vector<E, A> operator()(
        ...
        if constexpr (unqualified_canonical_imbwnot<zero_t, Mask,
                          basic_vector<E, A>>) {
            if consteval {
                return internal::masked<bwnot_t>(mask, val);
            } else {
    ...
    requires scalable_abi<InA> && simd_element_for<E, InA> &&
        ...
        unqualified_canonical_imbwnot<zero_t, Mask, basic_vector<E, InA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    requires imm_zmaskable_args<Arg> &&
        extended_imbwnot<bwnot_t, zero_t, Mask, Arg>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(Mask mask, Arg arg) noexcept {
        if constexpr (unqualified_extended_imbwnot<zero_t, Mask, Arg>) {
