...
public:
    ...
    requires (extended_vector<S> || extended_mask<Mask> ||
                 extended_vector<T>) &&
        maskable_args<S, Mask, T> && extended_mbwsr<S, Mask, T>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(
        S pass, Mask mask, T arg, size_t shift) noexcept {
        if constexpr (unqualified_extended_mbwsr<S, Mask, T>) {
    ...
    requires (extended_mask<Mask> || extended_vector<T>) &&
        zmaskable_args<Mask, T> && extended_mbwsr<zero_t, Mask, T>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(Mask mask, T arg, size_t shift) noexcept {
        if constexpr (unqualified_extended_mbwsr<dx::zero_t, Mask, T>) {
    ...
    template <simd_mask Mask, simd_vector T>
    requires requires(Mask mask, T arg, size_t shift) {
        bwshift_right_t::operator()(mask, arg, shift);
    }
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(
        dx::zero_t, Mask mask, T arg, size_t shift) noexcept {
    ...
    requires (different_from<SA, InA> || scalable_abi<SA> ||
        ...
        unqualified_canonical_imbwsr<basic_vector<E, SA>, Mask,
            basic_vector<E, InA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    requires (extended_vector<S> || extended_vector<T>) &&
        imm_maskable_args<S, T> && extended_imbwsr<S, Mask, T>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(
        S pass, Mask mask, T arg, size_t shift) noexcept {
        if constexpr (unqualified_extended_mbwsr<S, Mask, T>) {
    ...
    requires imm_zmaskable_args<T> && extended_imbwsr<zero_t, Mask, T>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(Mask mask, T arg, size_t shift) noexcept {
        if constexpr (unqualified_extended_mbwsr<zero_t, Mask, T>) {
            return bwshift_right(
    ...
    requires (extended_vector<S> || extended_mask<Mask> ||
                 extended_vector<T>) &&
        maskable_args<S, Mask, T> && extended_mbwsri<S, Mask, T, Shift>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(
        S pass, Mask mask, T arg, Shift shift) noexcept {
        if constexpr (unqualified_extended_mbwsri<S, Mask, T, Shift>) {
            return bwshift_right(pass, mask, arg, shift);
        } else {
            return operator()(dx::to_canonical(pass), dx::to_canonical(mask),
                dx::to_canonical(arg), shift);
    ...
    template <simd_mask Mask, simd_vector T, integral_constant_like Shift>
    requires (extended_mask<Mask> || extended_vector<T>) &&
        zmaskable_args<Mask, T> && extended_mbwsri<zero_t, Mask, T, Shift>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(Mask mask, T arg, Shift shift) noexcept {
        if constexpr (unqualified_extended_mbwsri<dx::zero_t, Mask, T, Shift>) {
            return bwshift_right(mask, arg, shift);
        } else {
            return operator()(
                dx::to_canonical(mask), dx::to_canonical(arg), shift);
    ...
    template <simd_mask Mask, simd_vector T, integral_constant_like Shift>
    requires requires(Mask mask, T arg, Shift shift) {
        bwshift_right_t::operator()(mask, arg, shift);
    }
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(
        dx::zero_t, Mask mask, T arg, Shift shift) noexcept {
        return operator()(mask, arg, shift);
    }
    ...
    requires (different_from<SA, InA> || scalable_abi<SA> ||
        ...
        simd_element_for<E, InA> &&
        imm_maskable_args<basic_vector<E, SA>, basic_vector<E, InA>> &&
        unqualified_canonical_imbwsri<basic_vector<E, SA>, Mask,
    ...
    template <simd_vector S, const_mask_for<S> Mask, simd_vector T,
        integral_constant_like Shift>
    requires (extended_vector<S> || extended_vector<T>) &&
        imm_maskable_args<S, T> && extended_imbwsri<S, Mask, T, Shift>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(
        S pass, Mask mask, T arg, Shift shift) noexcept {
        if constexpr (unqualified_extended_mbwsri<S, Mask, T, Shift>) {
    ...
    template <extended_vector T, const_mask_for<T> Mask,
        integral_constant_like Shift>
    requires imm_zmaskable_args<T> && extended_imbwsri<zero_t, Mask, T, Shift>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(Mask mask, T arg, Shift shift) noexcept {
        if constexpr (unqualified_extended_mbwsri<zero_t, Mask, T, Shift>) {
            return bwshift_right(
                dx::zero, dx::to_compatible_const_mask<T>(mask), arg, shift);
        } else {
            return operator()(dx::to_compatible_const_mask<T>(mask),
