...
template <typename T>
concept unqualified_bwsr =
    unqualified_canonical_bwsr<T> || unqualified_extended_bwsr<T> ||
    (decayable_vector_for<T, operation_category::lane_agnostic> &&
        unqualified_canonical_bwsr<canonical_type_t<T>>);

template <typename T>
concept unqualified_masksr =
    unqualified_canonical_bwsr<T> || unqualified_extended_bwsr<T> ||
...
template <typename T, typename V>
concept unqualified_bwsri =
    unqualified_canonical_bwsri<T, V> || unqualified_extended_bwsri<T, V> ||
    (decayable_vector_for<T, operation_category::lane_agnostic> &&
        unqualified_canonical_bwsri<canonical_type_t<T>, V>);

template <typename T, typename V>
concept unqualified_masksri =
    unqualified_canonical_bwsri<T, V> || unqualified_extended_bwsri<T, V> ||
    (decayable_mask_for<T, operation_category::lane_permutation> &&
        unqualified_canonical_bwsri<canonical_type_t<T>, V>);
...
concept unqualified_bwsrv =
    ...
    (decayable_vector_for<L, operation_category::lane_agnostic> &&
        ...
        unqualified_canonical_bwsrv<canonical_type_t<L>, canonical_type_t<R>>);
...
concept extended_mbwsr =
    unqualified_extended_mbwsr<S, C, T, A> || decayable_mbwsr<Op, S, C, T, A>;

template <typename S, typename M, typename T,
    typename A = common_abi_t<canonical_if_zero_t<S, T>, T>>
concept unqualified_canonical_imbwsr = requires(S src, T val, size_t shift) {
    {
        bwshift_right<const_mask_v<canonical_if_zero_t<S, T>, M>>(
...
template <typename S, typename M, typename T,
    typename A = common_abi_t<canonical_if_zero_t<S, T>, T>>
concept unqualified_extended_imbwsr = requires(S src, T val, size_t shift) {
    {
        bwshift_right<const_mask_v<canonical_if_zero_t<S, T>, M>>(
...
concept extended_imbwsr =
    unqualified_extended_imbwsr<S, M, T, A> || decayable_imbwsr<Op, S, M, T, A>;
...
concept extended_mbwsri = unqualified_extended_mbwsri<S, C, L, R, A> ||
    decayable_mbwsri<Op, S, C, L, R, A>;

template <typename S, typename M, typename L, typename R,
    typename A = common_abi_t<canonical_if_zero_t<S, L>, L>>
concept unqualified_canonical_imbwsri = requires(S src, L val, R shift) {
    {
        bwshift_right<const_mask_v<canonical_if_zero_t<S, L>, M>>(
            internal::abi<A>, src, dx::masked_operation, val, shift)
    } -> equivalent_simd_as<canonical_if_zero_t<S, L, A>>;
};
...
template <typename S, typename M, typename L, typename R,
    typename A = common_abi_t<canonical_if_zero_t<S, L>, L>>
concept unqualified_extended_imbwsri = requires(S src, L val, R shift) {
    {
        bwshift_right<const_mask_v<canonical_if_zero_t<S, L>, M>>(
            src, dx::masked_operation, val, shift)
    } -> equivalent_simd_as<canonical_if_zero_t<S, L, A>>;
...
concept extended_imbwsri = unqualified_extended_imbwsri<S, M, L, R, A> ||
    decayable_imbwsri<Op, S, M, L, R, A>;
...
template <typename Op, typename S, typename C, typename L, typename R,
    typename A = common_abi_t<L, R, C>>
concept extended_mbwsrv = unqualified_extended_mbwsrv<Op, S, C, L, R, A> ||
    decayable_mbwsrv<Op, S, C, L, R, A>;

template <typename Op, typename S, typename M, typename L, typename R,
    typename A =
        common_abi_t<canonical_if_zero_t<S, operation_result_t<Op, L, R>>,
            operation_result_t<Op, L, R>>>
concept unqualified_canonical_imbwsrv = requires(S src, L lhs, R rhs) {
    {
        bwshift_right<const_mask_v<
...
template <typename Op, typename S, typename M, typename L, typename R,
    typename A =
        common_abi_t<canonical_if_zero_t<S, operation_result_t<Op, L, R>>,
            operation_result_t<Op, L, R>>>
concept unqualified_extended_imbwsrv = requires(S src, L lhs, R rhs) {
    {
        bwshift_right<const_mask_v<
            canonical_if_zero_t<S, operation_result_t<Op, L, R>, A>, M>>(
...
template <typename Op, typename S, typename M, typename L, typename R,
    typename A =
        common_abi_t<canonical_if_zero_t<S, operation_result_t<Op, L, R>>,
            operation_result_t<Op, L, R>>>
concept extended_imbwsrv = unqualified_extended_imbwsrv<Op, S, M, L, R, A> ||
    decayable_imbwsrv<Op, S, M, L, R, A>;
...
public:
    ...
    requires (different_from<A1, A2> || scalable_abi<A1> || scalable_abi<A2>) &&
        ...
        maskable_args<basic_vector<E, A1>, basic_mask<MaskE, A1>,
            basic_vector<E, A2>> &&
        unqualified_canonical_mbwsr<basic_vector<E, A1>, basic_mask<MaskE, A1>,
            basic_vector<E, A2>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr basic_vector<E, A1> operator()(basic_vector<E, A1> pass,
        basic_mask<MaskE, A1> mask, basic_vector<E, A2> val,
        size_t shift) noexcept {
        return bwshift_right(internal::abi<A1>, pass, mask, val, shift);
    }
    ...
    requires (extended_vector<Pass> || extended_mask<Mask> ||
        ...
        extended_mbwsr<bwshift_right_t, Pass, Mask, Arg>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(
        Pass pass, Mask mask, Arg arg, size_t shift) noexcept {
        if constexpr (unqualified_extended_mbwsr<Pass, Mask, Arg>) {
    ...
    static constexpr basic_vector<E, A> operator()(basic_mask<MaskE, A> mask,
        basic_vector<E, A> val, size_t shift) noexcept {
        if constexpr (unqualified_canonical_mbwsr<zero_t, basic_mask<MaskE, A>,
                          basic_vector<E, A>>) {
            if consteval {
    ...
    static constexpr basic_vector<E, A> operator()(basic_vector<E, A> pass,
        ...
        if constexpr (unqualified_canonical_imbwsr<basic_vector<E, A>, Mask,
                          basic_vector<E, A>>) {
            if consteval {
                return internal::masked<bwshift_right_t>(
                    pass, mask, val, shift);
    ...
    requires (different_from<PassA, InA> || scalable_abi<PassA> ||
        ...
        unqualified_canonical_imbwsr<basic_vector<E, PassA>, Mask,
            basic_vector<E, InA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    requires (extended_vector<Pass> || extended_vector<Arg>) &&
        ...
        extended_imbwsr<bwshift_right_t, Pass, Mask, Arg>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(
        Pass pass, Mask mask, Arg arg, size_t shift) noexcept {
        if constexpr (unqualified_extended_mbwsr<Pass, Mask, Arg>) {
            constexpr auto V = const_mask_v<Pass, Mask>;
            return bwshift_right<V>(pass, masked_operation, arg, shift);
        } else {
    ...
    static constexpr basic_vector<E, A> operator()(
        Mask mask, basic_vector<E, A> val, size_t shift) noexcept {
        if constexpr (unqualified_canonical_imbwsr<zero_t, Mask,
                          basic_vector<E, A>>) {
            if consteval {
                return internal::masked<bwshift_right_t>(mask, val, shift);
            } else {
    ...
    requires scalable_abi<InA> && simd_element_for<E, InA> &&
        ...
        unqualified_canonical_imbwsr<zero_t, Mask, basic_vector<E, InA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    requires imm_zmaskable_args<Arg> &&
        extended_imbwsr<bwshift_right_t, zero_t, Mask, Arg>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(
        Mask mask, Arg arg, size_t shift) noexcept {
        if constexpr (unqualified_extended_mbwsr<zero_t, Mask, Arg>) {
            constexpr auto V = const_mask_v<Arg, Mask>;
            return bwshift_right<V>(dx::zero, masked_operation, arg, shift);
        } else {
    ...
    requires (different_from<A1, A2> || scalable_abi<A1> || scalable_abi<A2>) &&
        ...
        maskable_args<basic_vector<E, A1>, basic_mask<MaskE, A1>,
            basic_vector<E, A2>> &&
        unqualified_canonical_mbwsri<basic_vector<E, A1>, basic_mask<MaskE, A1>,
    ...
    requires (extended_vector<Pass> || extended_mask<Mask> ||
                 extended_vector<Arg>) &&
        maskable_args<Pass, Mask, Arg> &&
        extended_mbwsri<bwshift_right_t, Pass, Mask, Arg, Shift>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(
        Pass pass, Mask mask, Arg arg, Shift shift) noexcept {
        if constexpr (unqualified_extended_mbwsri<Pass, Mask, Arg, Shift>) {
            return bwshift_right(pass, mask, arg, shift);
        } else {
            return operator()(dx::to_canonical(pass), dx::to_canonical(mask),
                dx::to_canonical(arg), shift);
    ...
    static constexpr basic_vector<E, A> operator()(basic_mask<MaskE, A> mask,
        basic_vector<E, A> val, Shift shift) noexcept {
        if constexpr (unqualified_canonical_mbwsri<zero_t, basic_mask<MaskE, A>,
    ...
    requires (different_from<A1, A2> || scalable_abi<A1> || scalable_abi<A2>) &&
        simd_element_for<E, A2> &&
        zmaskable_args<basic_mask<MaskE, A1>, basic_vector<E, A2>> &&
        unqualified_canonical_mbwsri<dx::zero_t, basic_mask<MaskE, A1>,
            basic_vector<E, A2>, Shift>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(basic_mask<MaskE, A1> mask,
    ...
    requires (extended_mask<Mask> || extended_vector<Arg>) &&
        zmaskable_args<Mask, Arg> &&
        extended_mbwsri<bwshift_right_t, zero_t, Mask, Arg, Shift>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(Mask mask, Arg arg, Shift shift) noexcept {
        if constexpr (unqualified_extended_mbwsri<dx::zero_t, Mask, Arg,
                          Shift>) {
            return bwshift_right(mask, arg, shift);
        } else {
            return operator()(
    ...
    static constexpr basic_vector<E, A> operator()(basic_vector<E, A> pass,
        ...
        if constexpr (unqualified_canonical_imbwsri<basic_vector<E, A>, Mask,
                          basic_vector<E, A>, Shift>) {
            if consteval {
                return internal::masked<bwshift_right_t>(
                    pass, mask, val, shift);
    ...
    requires (different_from<PassA, InA> || scalable_abi<PassA> ||
        ...
        simd_element_for<E, InA> &&
        imm_maskable_args<basic_vector<E, PassA>, basic_vector<E, InA>> &&
        unqualified_canonical_imbwsri<basic_vector<E, PassA>, Mask,
    ...
    requires (extended_vector<Pass> || extended_vector<Arg>) &&
        imm_maskable_args<Pass, Arg> &&
        extended_imbwsri<bwshift_right_t, Pass, Mask, Arg, Shift>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(
        Pass pass, Mask mask, Arg arg, Shift shift) noexcept {
        if constexpr (unqualified_extended_mbwsri<Pass, Mask, Arg, Shift>) {
    ...
    static constexpr basic_vector<E, A> operator()(
        Mask mask, basic_vector<E, A> val, Shift shift) noexcept {
        if constexpr (unqualified_canonical_imbwsri<zero_t, Mask,
                          basic_vector<E, A>, Shift>) {
            if consteval {
                return internal::masked<bwshift_right_t>(mask, val, shift);
            } else {
    ...
    requires scalable_abi<InA> && simd_element_for<E, InA> &&
        imm_zmaskable_args<basic_vector<E, InA>> &&
        unqualified_canonical_imbwsri<zero_t, Mask, basic_vector<E, InA>, Shift>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr basic_vector<E, InA> operator()(
    ...
    template <extended_vector Arg, const_mask_for<Arg> Mask,
        integral_constant_like Shift>
    requires imm_zmaskable_args<Arg> &&
        extended_imbwsri<bwshift_right_t, zero_t, Mask, Arg, Shift>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(Mask mask, Arg arg, Shift shift) noexcept {
        if constexpr (unqualified_extended_mbwsri<zero_t, Mask, Arg, Shift>) {
            constexpr auto V = const_mask_v<Arg, Mask>;
            return bwshift_right<V>(dx::zero, masked_operation, arg, shift);
        } else {
            return operator()(dx::to_compatible_const_mask<Arg>(mask),
    ...
    requires integral<typename R::value_type> &&
        ...
        extended_mbwsrv<bwshift_right_t, S, Mask, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(S src, Mask mask, L lhs, R rhs) noexcept {
        if constexpr (unqualified_extended_mbwsrv<bwshift_right_t, S, Mask, L,
                          R>) {
    ...
    requires (extended_vector<S> || extended_mask<Mask> || extended_vector<L> ||
        ...
        extended_mbwsrv<bwshift_right_t, zero_t, Mask, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(Mask mask, L lhs, R rhs) noexcept {
        if constexpr (unqualified_extended_mbwsrv<bwshift_right_t, zero_t, Mask,
                          L, R>) {
    ...
    requires requires(Mask mask, L lhs, R rhs) {
        bwshift_right_t::operator()(mask, lhs, rhs);
    }
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(
    ...
    requires (different_from<SA, common_abi_t<LA, RA>> ||
        ...
        imm_maskable_args<basic_vector<E, SA>, basic_vector<E, LA>,
            basic_vector<E, RA>> &&
        unqualified_canonical_imbwsrv<bwshift_right_t, basic_vector<E, SA>,
            Mask, basic_vector<E, LA>, basic_vector<E, RA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    requires (extended_vector<S> || extended_vector<L> || extended_vector<R>) &&
        ...
        extended_imbwsrv<bwshift_right_t, S, Mask, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(S src, Mask mask, L lhs, R rhs) noexcept {
        if constexpr (unqualified_extended_imbwsrv<bwshift_right_t, S, Mask, L,
                          R>) {
            constexpr auto V = const_mask_v<S, Mask>;
            return bwshift_right<V>(src, masked_operation, lhs, rhs);
        } else {
    ...
    static constexpr basic_vector<E, A> operator()(
        Mask mask, basic_vector<E, A> lhs, basic_vector<E, A> rhs) noexcept {
        if constexpr (unqualified_canonical_imbwsrv<bwshift_right_t, zero_t,
    ...
    requires (different_from<LA, RA> || scalable_abi<LA> || scalable_abi<RA>) &&
        ...
        unqualified_canonical_imbwsrv<bwshift_right_t, zero_t, Mask,
    ...
    template <simd_vector L, simd_vector R,
        const_mask_for<operation_result_t<bwshift_right_t, L, R>> Mask>
    requires (extended_vector<L> || extended_vector<R>) &&
        imm_zmaskable_args<L, R> &&
        extended_imbwsrv<bwshift_right_t, zero_t, Mask, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(Mask mask, L lhs, R rhs) noexcept {
        using S = operation_result_t<bwshift_right_t, L, R>;
        if constexpr (unqualified_extended_imbwsrv<bwshift_right_t, zero_t,
                          Mask, L, R>) {
            constexpr auto V = const_mask_v<S, Mask>;
            return bwshift_right<V>(dx::zero, masked_operation, lhs, rhs);
        } else {
