...
concept unqualified_canonical_bwandnot = requires(L lhs, R rhs) {
    {
        bwandnot(internal::abi<A>, lhs, rhs)
    } -> canonical_bitwise_result<L, R, A>;
};

template <typename L, typename R, typename A = common_abi_t<L, R>>
concept unqualified_extended_bwandnot = requires(L lhs, R rhs) {
    { bwandnot(lhs, rhs) } -> extended_operation_vector<A>;
};

template <typename L, typename R>
concept expression_bwandnot = (simd_expression<L> || simd_expression<R>) &&
    invocable<abs_t, simd_expression_result_t<L>, simd_expression_result_t<R>>;

template <typename L, typename R, typename A = common_abi_t<L, R>>
concept decayable_bwandnot = (decayable_vector_for<L,
                                  operation_category::lane_agnostic> &&
    decayable_vector_for<R, operation_category::lane_agnostic> &&
    regular_invocable<bwandnot_t, canonical_type_t<L>, canonical_type_t<R>>);

template <typename L, typename R, typename A = common_abi_t<L, R>>
concept extended_bwandnot = unqualified_extended_bwandnot<L, R, A> ||
    expression_bwandnot<L, R> || decayable_bwandnot<L, R, A>;

template <typename S, typename M, typename L, typename R,
    typename A = common_abi_t<L, R, M>>
concept unqualified_canonical_mbwandnot =
    requires(S src, M mask, L lhs, R rhs) {
        {
            bwandnot(internal::abi<A>, src, mask, lhs, rhs)
        } -> equivalent_simd_as<
            canonical_if_zero_t<S, operation_result_t<bwandnot_t, L, R>, A>>;
    };

template <typename S, typename M, typename L, typename R,
    typename A = common_abi_t<L, R, M>>
concept unqualified_extended_mbwandnot = requires(S src, M mask, L lhs, R rhs) {
    { bwandnot(src, mask, lhs, rhs) } -> extended_operation_vector<A>;
};

template <typename S, typename M, typename L, typename R>
concept expression_mbwandnot = (simd_expression<S> || simd_expression<M> ||
                                   simd_expression<L> || simd_expression<R>) &&
    invocable<bwandnot_t, expression_result_or_zero_t<S>,
        simd_expression_result_t<M>, simd_expression_result_t<L>,
        simd_expression_result_t<R>>;

template <typename S, typename M, typename L, typename R,
    typename A = common_abi_t<L, R, M>>
concept decayable_mbwandnot =
    ...
    decayable_vector_for<R, operation_category::lane_agnostic> &&
    regular_invocable<bwandnot_t,
        canonical_or_zero_t<S, operation_result_t<bwandnot_t, L, R>, A>,
        canonical_type_t<M>, canonical_type_t<L>, canonical_type_t<R>>;

template <typename S, typename M, typename L, typename R,
    typename A = common_abi_t<L, R, M>>
concept extended_mbwandnot = unqualified_extended_mbwandnot<S, M, L, R, A> ||
    expression_mbwandnot<S, M, L, R> || decayable_mbwandnot<S, M, L, R, A>;

template <typename S, typename M, typename L, typename R,
    typename A = common_abi_t<
        canonical_if_zero_t<S, operation_result_t<bwandnot_t, L, R>>,
        operation_result_t<bwandnot_t, L, R>>>
concept unqualified_canonical_imbwandnot =
    requires(S src, M mask, L lhs, R rhs) {
        {
            bwandnot(internal::abi<A>, src,
                internal::to_const_mask<A, bwandnot_t, S, L, R>(mask), lhs, rhs)
        } -> equivalent_simd_as<
            canonical_if_zero_t<S, operation_result_t<bwandnot_t, L, R>, A>>;
    };
...
template <typename S, typename M, typename L, typename R,
    typename A = common_abi_t<
        canonical_if_zero_t<S, operation_result_t<bwandnot_t, L, R>>,
        operation_result_t<bwandnot_t, L, R>>>
concept unqualified_extended_imbwandnot =
    requires(S src, M mask, L lhs, R rhs) {
        {
            bwandnot(src, internal::to_const_mask<A, bwandnot_t, S, L, R>(mask),
                lhs, rhs)
        } -> extended_operation_vector<A>;
    };

template <typename S, typename M, typename L, typename R>
concept expression_imbwandnot =
    (simd_expression<S> || simd_expression<L> || simd_expression<R>) &&
    invocable<bwandnot_t, expression_result_or_zero_t<S>, M,
...
template <typename S, typename M, typename L, typename R,
    typename A = common_abi_t<
        canonical_if_zero_t<S, operation_result_t<bwandnot_t, L, R>>,
        operation_result_t<bwandnot_t, L, R>>>
concept decayable_imbwandnot =
    decayable_vector_for<
        canonical_if_zero_t<S, operation_result_t<bwandnot_t, L, R>, A>,
        operation_category::lane_agnostic> &&
    decayable_vector_for<L, operation_category::lane_agnostic> &&
    decayable_vector_for<R, operation_category::lane_agnostic> &&
    requires(bwandnot_t op,
        canonical_if_zero_t<S, operation_result_t<bwandnot_t, L, R>, A> s,
...
template <typename S, typename M, typename L, typename R,
    typename A = common_abi_t<
        canonical_if_zero_t<S, operation_result_t<bwandnot_t, L, R>>,
        operation_result_t<bwandnot_t, L, R>>>
concept extended_imbwandnot = unqualified_extended_imbwandnot<S, M, L, R, A> ||
    expression_imbwandnot<S, M, L, R> || decayable_imbwandnot<S, M, L, R, A>;

template <typename L, typename R, typename A = common_abi_t<L, R>>
concept unqualified_canonical_mask_bwandnot = requires(L lhs, R rhs) {
    { bwandnot(internal::abi<A>, lhs, rhs) } -> canonical_bitwise_mask<L, R, A>;
};

template <typename L, typename R, typename A = common_abi_t<L, R>>
concept unqualified_extended_mask_bwandnot = requires(L lhs, R rhs) {
    { bwandnot(lhs, rhs) } -> extended_operation_mask<A>;
};

template <typename L, typename R>
concept expression_mask_bwandnot = (mask_expression<L> || mask_expression<R>) &&
    invocable<bwandnot_t, simd_expression_result_t<L>,
        simd_expression_result_t<R>>;

template <typename L, typename R, typename A = common_abi_t<L, R>>
concept decayable_mask_bwandnot =
    decayable_mask_for<L, operation_category::lane_agnostic> &&
    decayable_mask_for<R, operation_category::lane_agnostic> &&
    regular_invocable<bwandnot_t, canonical_type_t<L>, canonical_type_t<R>>;

template <typename L, typename R, typename A = common_abi_t<L, R>>
concept extended_mask_bwandnot = unqualified_extended_mask_bwandnot<L, R, A> ||
    decayable_mask_bwandnot<L, R> || decayable_mask_bwandnot<L, R, A>;

struct bwandnot_t : private binary_operation_base<bwandnot_t> {
private:
    ...
    requires (!simd_class<L> || canonical_vector<L>) &&
        (!simd_class<R> || canonical_vector<R>) && requires(L lhs, R rhs) {
            { bwandnot(internal::abi<A>, lhs, rhs) } -> vector_with_abi<A>;
        }
    ...
    static constexpr auto native(A, L lhs, R rhs) noexcept(
        noexcept(bwandnot(lhs, rhs))) {
        return bwandnot(lhs, rhs);
    }

    template <simd_abi A, typename L, typename R>
    requires (!simd_class<L> || canonical_mask<L>) &&
        (!simd_class<R> || canonical_mask<R>) && requires(L lhs, R rhs) {
            { bwandnot(internal::abi<A>, lhs, rhs) } -> mask_with_abi<A>;
        }
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto native(A abi, L lhs, R rhs) noexcept {
        return bwandnot(internal::abi<A>, lhs, rhs);
    ...
    requires (!simd_class<L> || extended_mask<L>) &&
        ...
        unqualified_extended_mask_bwandnot<L, R, A>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto native(A, L lhs, R rhs) noexcept(
        noexcept(bwandnot(lhs, rhs))) {
        return bwandnot(lhs, rhs);
...
public:
    ...
    static constexpr basic_vector<E, A> operator()(
        ...
        if constexpr (unqualified_canonical_bwandnot<basic_vector<E, A>,
                          basic_vector<E, A>>) {
            if consteval {
    ...
    requires simd_element_for<E, LA> && simd_element_for<E, RA> &&
        ...
        unqualified_canonical_bwandnot<basic_vector<E, LA>, basic_vector<E, RA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr basic_vector<E, common_abi_t<LA, RA>> operator()(
        basic_vector<E, LA> lhs, basic_vector<E, RA> rhs) noexcept {
        return bwandnot(internal::abi<common_abi_t<LA, RA>>, lhs, rhs);
    }

    template <simd_vector L, simd_vector R>
    requires (extended_vector<L> || extended_vector<R>) &&
        extended_bwandnot<L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(L lhs, R rhs) noexcept {
        if constexpr (unqualified_extended_bwandnot<L, R>) {
            return bwandnot(lhs, rhs);
        } else if constexpr (expression_bwandnot<L, R>) {
    ...
    static constexpr basic_vector<E, A> operator()(basic_vector<E, A> src,
        ...
        if constexpr (unqualified_canonical_mbwandnot<basic_vector<E, A>,
                          basic_mask<ME, A>, basic_vector<E, A>,
                          basic_vector<E, A>>) {
            if consteval {
                return internal::masked<bwandnot_t>(src, mask, lhs, rhs);
            } else {
                return bwandnot(internal::abi<A>, src, mask, lhs, rhs);
            }
        } else {
            return internal::masked<bwandnot_t>(src, mask, lhs, rhs);
        }
    ...
    requires (different_from<LA, RA> || scalable_abi<SA> || scalable_abi<LA> ||
        ...
        unqualified_canonical_mbwandnot<basic_vector<E, SA>, basic_mask<ME, SA>,
            basic_vector<E, LA>, basic_vector<E, RA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr basic_vector<E, SA> operator()(basic_vector<E, SA> src,
        ...
        return bwandnot(internal::abi<SA>, src, mask, lhs, rhs);
    }
    ...
    requires (extended_vector<S> || extended_mask<M> || extended_vector<L> ||
                 extended_vector<R>) &&
        maskable_args<S, M, L, R> && extended_mbwandnot<S, M, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(S src, M mask, L lhs, R rhs) noexcept {
        if constexpr (unqualified_extended_mbwandnot<S, M, L, R>) {
            return bwandnot(src, mask, lhs, rhs);
        } else if constexpr (expression_mbwandnot<S, M, L, R>) {
    ...
    static constexpr basic_vector<E, A> operator()(basic_mask<ME, A> mask,
        basic_vector<E, A> lhs, basic_vector<E, A> rhs) noexcept {
        if constexpr (unqualified_canonical_mbwandnot<zero_t, basic_mask<ME, A>,
                          basic_vector<E, A>, basic_vector<E, A>>) {
            if consteval {
                return internal::masked<bwandnot_t>(mask, lhs, rhs);
            } else {
                return bwandnot(internal::abi<A>, dx::zero, mask, lhs, rhs);
            }
        } else {
    ...
    requires (different_from<SA, common_abi_t<LA, RA>> ||
        ...
        zmaskable_args<basic_mask<ME, SA>, basic_vector<E, LA>,
            basic_vector<E, RA>> &&
        unqualified_canonical_mbwandnot<zero_t, basic_mask<ME, SA>,
            basic_vector<E, LA>, basic_vector<E, RA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    requires (extended_mask<M> || extended_vector<L> || extended_vector<R>) &&
        zmaskable_args<M, L, R> && extended_mbwandnot<zero_t, M, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(M mask, L lhs, R rhs) noexcept {
        if constexpr (unqualified_extended_mbwandnot<zero_t, M, L, R>) {
            return bwandnot(dx::zero, mask, lhs, rhs);
        } else if constexpr (expression_mbwandnot<zero_t, M, L, R>) {
            return operator()(
    ...
    template <simd_mask M, simd_vector L, simd_vector R>
    requires invocable<bwandnot_t, M, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    static constexpr basic_vector<E, A> operator()(basic_vector<E, A> src,
        ...
        if constexpr (unqualified_canonical_imbwandnot<basic_vector<E, A>, M,
                          basic_vector<E, A>, basic_vector<E, A>>) {
            if consteval {
                return internal::masked<bwandnot_t>(src, mask, lhs, rhs);
            } else {
                return bwandnot(internal::abi<A>, src,
    ...
    requires (different_from<SA, common_abi_t<LA, RA>> ||
        ...
        imm_maskable_args<basic_vector<E, SA>, basic_vector<E, LA>,
            basic_vector<E, RA>> &&
        unqualified_canonical_imbwandnot<basic_vector<E, SA>, M,
            basic_vector<E, LA>, basic_vector<E, RA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr basic_vector<E, SA> operator()(basic_vector<E, SA> src,
        M mask, basic_vector<E, LA> lhs, basic_vector<E, RA> rhs) noexcept {
        return bwandnot(internal::abi<SA>, src,
    ...
    requires (extended_vector<S> || extended_vector<L> || extended_vector<R>) &&
        imm_maskable_args<S, L, R> && extended_imbwandnot<S, M, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(S src, M mask, L lhs, R rhs) noexcept {
        if constexpr (unqualified_extended_imbwandnot<S, M, L, R>) {
            return bwandnot(
                src, dx::to_compatible_const_mask<S>(mask), lhs, rhs);
        } else if constexpr (expression_imbwandnot<S, M, L, R>) {
            return operator()(
    ...
    static constexpr basic_vector<E, A> operator()(
        M mask, basic_vector<E, A> lhs, basic_vector<E, A> rhs) noexcept {
        if constexpr (unqualified_canonical_imbwandnot<zero_t, M,
                          basic_vector<E, A>, basic_vector<E, A>>) {
            if consteval {
                return internal::masked<bwandnot_t>(mask, lhs, rhs);
            } else {
                return bwandnot(internal::abi<A>, dx::zero,
                    dx::to_compatible_const_mask<basic_vector<E, A>>(mask), lhs,
                    rhs);
            }
        } else {
    ...
    requires (different_from<LA, RA> || scalable_abi<LA> || scalable_abi<RA>) &&
        ...
        unqualified_canonical_imbwandnot<zero_t, M, basic_vector<E, LA>,
            basic_vector<E, RA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr basic_vector<E, common_abi_t<LA, RA>> operator()(
        M mask, basic_vector<E, LA> lhs, basic_vector<E, RA> rhs) noexcept {
    ...
    template <simd_vector L, simd_vector R,
        const_mask_for<operation_result_t<bwandnot_t, L, R>> M>
    requires (extended_vector<L> || extended_vector<R>) &&
        imm_zmaskable_args<L, R> && extended_imbwandnot<zero_t, M, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(M mask, L lhs, R rhs) noexcept {
        using S = operation_result_t<bwandnot_t, L, R>;
        constexpr auto cmask = dx::to_compatible_const_mask<S>(mask);
        if constexpr (unqualified_extended_imbwandnot<zero_t, M, L, R>) {
            return bwandnot(dx::zero, cmask, lhs, rhs);
        } else if constexpr (expression_imbwandnot<zero_t, M, L, R>) {
    ...
    static constexpr basic_mask<common_size_type_t<LE, RE>, A> operator()(
        basic_mask<LE, A> lhs, basic_mask<RE, A> rhs) noexcept {
        if constexpr (unqualified_canonical_mask_bwandnot<basic_mask<LE, A>,
                          basic_mask<RE, A>>) {
            if consteval {
                return fallback(lhs, rhs);
            } else {
    ...
    requires simd_element_for<LE, LA> && simd_element_for<RE, RA> &&
        (scalable_abi<LA> || scalable_abi<RA> || different_from<LA, RA>) &&
        unqualified_canonical_mask_bwandnot<basic_mask<LE, LA>,
            basic_mask<RE, RA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    requires (extended_mask<L> || extended_mask<R>) &&
        extended_mask_bwandnot<L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(L lhs, R rhs) noexcept {
        if constexpr (unqualified_extended_mask_bwandnot<L, R>) {
            return bwandnot(lhs, rhs);
        } else if constexpr (expression_mask_bwandnot<L, R>) {
            return operator()(dx::evaluate(lhs), dx::evaluate(rhs));
        } else {
            return operator()(dx::to_canonical(lhs), dx::to_canonical(rhs));
        }
