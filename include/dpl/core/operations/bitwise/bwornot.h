...
concept unqualified_bwornot =
    ...
    (decayable_vector_for<L, operation_category::lane_agnostic> &&
        ...
        unqualified_canonical_bwornot<canonical_type_t<L>,
            canonical_type_t<R>>);

template <typename L, typename R, typename A = common_abi_t<L, R>>
concept unqualified_canonical_mask_ornot = requires(L lhs, R rhs) {
    {
        bwornot(internal::abi<A>, lhs, rhs)
    } -> canonical_bitwise_result<L, R, A>;
};
...
template <typename L, typename R, typename A = common_abi_t<L, R>>
concept unqualified_extended_mask_ornot = requires(L lhs, R rhs) {
    { bwornot(lhs, rhs) } -> extended_bitwise_result<L, R, A>;
};

template <typename L, typename R, typename A = common_abi_t<L, R>>
concept unqualified_mask_ornot = unqualified_canonical_mask_ornot<L, R> ||
    unqualified_extended_mask_ornot<L, R> ||
    (decayable_mask_for<L, operation_category::lane_agnostic> &&
        decayable_mask_for<R, operation_category::lane_agnostic> &&
        unqualified_canonical_mask_ornot<canonical_type_t<L>,
            canonical_type_t<R>>);
...
template <typename Op, typename S, typename C, typename L, typename R,
    typename A = common_abi_t<L, R, C>>
concept extended_mbwornot = unqualified_extended_mbwornot<Op, S, C, L, R, A> ||
    decayable_mbwornot<Op, S, C, L, R, A>;
...
template <typename Op, typename S, typename M, typename L, typename R,
    typename A =
        common_abi_t<canonical_if_zero_t<S, operation_result_t<Op, L, R>>,
            operation_result_t<Op, L, R>>>
concept extended_imbwornot =
    unqualified_extended_imbwornot<Op, S, M, L, R, A> ||
    decayable_imbwornot<Op, S, M, L, R, A>;
...
public:
    ...
    static constexpr basic_mask<E, A> operator()(
        basic_mask<E, A> lhs, basic_mask<E, A> rhs) noexcept {
        if constexpr (unqualified_canonical_mask_ornot<basic_mask<E, A>,
                          basic_mask<E, A>>) {
            if consteval {
                return fallback(lhs, rhs);
            } else {
    ...
    requires simd_element_for<E, LA> && simd_element_for<E, RA> &&
        (scalable_abi<LA> || scalable_abi<RA> || different_from<LA, RA>) &&
        unqualified_canonical_mask_ornot<basic_mask<E, LA>, basic_mask<E, RA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    requires (extended_mask<L> || extended_mask<R>) &&
        unqualified_mask_ornot<L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(L lhs, R rhs) noexcept {
        if constexpr (unqualified_extended_mask_ornot<L, R>) {
            return bwornot(lhs, rhs);
        } else {
            return operator()(dx::to_canonical(lhs), dx::to_canonical(rhs));
        }
    ...
    static constexpr basic_vector<E, A> operator()(basic_vector<E, A> src,
        ...
        if constexpr (unqualified_canonical_mbwornot<bwornot_t,
    ...
    requires (different_from<LA, RA> || scalable_abi<SA> || scalable_abi<LA> ||
        ...
        unqualified_canonical_mbwornot<bwornot_t, basic_vector<E, SA>,
    ...
    requires (extended_vector<S> || extended_mask<Mask> || extended_vector<L> ||
        ...
        extended_mbwornot<bwornot_t, S, Mask, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(S src, Mask mask, L lhs, R rhs) noexcept {
        if constexpr (unqualified_extended_mbwornot<bwornot_t, S, Mask, L, R>) {
    ...
    static constexpr basic_vector<E, A> operator()(basic_mask<MaskE, A> mask,
        basic_vector<E, A> lhs, basic_vector<E, A> rhs) noexcept {
        if constexpr (unqualified_canonical_mbwornot<bwornot_t, zero_t,
    ...
    requires (different_from<SA, common_abi_t<LA, RA>> ||
        ...
        zmaskable_args<basic_mask<MaskE, SA>, basic_vector<E, LA>,
            basic_vector<E, RA>> &&
        unqualified_canonical_mbwornot<bwornot_t, zero_t, basic_mask<MaskE, SA>,
            basic_vector<E, LA>, basic_vector<E, RA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    requires (extended_vector<S> || extended_mask<Mask> || extended_vector<L> ||
        ...
        extended_mbwornot<bwornot_t, zero_t, Mask, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(Mask mask, L lhs, R rhs) noexcept {
        if constexpr (unqualified_extended_mbwornot<bwornot_t, zero_t, Mask, L,
                          R>) {
    ...
    static constexpr basic_vector<E, A> operator()(basic_vector<E, A> src,
        ...
        if constexpr (unqualified_canonical_imbwornot<bwornot_t,
    ...
    requires (different_from<SA, common_abi_t<LA, RA>> ||
        ...
        imm_maskable_args<basic_vector<E, SA>, basic_vector<E, LA>,
            basic_vector<E, RA>> &&
        unqualified_canonical_imbwornot<bwornot_t, basic_vector<E, SA>, Mask,
            basic_vector<E, LA>, basic_vector<E, RA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr basic_vector<E, SA> operator()(basic_vector<E, SA> src,
    ...
    requires (extended_vector<S> || extended_vector<L> || extended_vector<R>) &&
        ...
        extended_imbwornot<bwornot_t, S, Mask, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(S src, Mask mask, L lhs, R rhs) noexcept {
        if constexpr (unqualified_extended_imbwornot<bwornot_t, S, Mask, L,
                          R>) {
    ...
    static constexpr basic_vector<E, A> operator()(
        Mask mask, basic_vector<E, A> lhs, basic_vector<E, A> rhs) noexcept {
        if constexpr (unqualified_canonical_imbwornot<bwornot_t, zero_t, Mask,
                          basic_vector<E, A>, basic_vector<E, A>>) {
            if consteval {
                return internal::masked<bwornot_t>(mask, lhs, rhs);
            } else {
    ...
    requires (different_from<LA, RA> || scalable_abi<LA> || scalable_abi<RA>) &&
        ...
        unqualified_canonical_imbwornot<bwornot_t, zero_t, Mask,
            basic_vector<E, LA>, basic_vector<E, RA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr basic_vector<E, common_abi_t<LA, RA>> operator()(
        Mask mask, basic_vector<E, LA> lhs, basic_vector<E, RA> rhs) noexcept {
    ...
    requires (extended_vector<L> || extended_vector<R>) &&
        ...
        extended_imbwornot<bwornot_t, zero_t, Mask, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(Mask mask, L lhs, R rhs) noexcept {
        using S = operation_result_t<bwornot_t, L, R>;
        if constexpr (unqualified_extended_imbwornot<bwornot_t, zero_t, Mask, L,
                          R>) {
            constexpr auto V = const_mask_v<S, Mask>;
