...
struct DPL_EMPTY_BASES bwornot_t :
    private bitwise_base<bwornot_t>,
    private maskable_transform_base<bwornot_t>,
    private binary_broadcastable_operation<bwornot_t> {
...
template <>
struct fallback_impl<bwornot_t> : binary_broadcasting_fallback<bwornot_t> {
    ...
    using binary_broadcasting_fallback<bwornot_t>::operator();
};
...
concept unqualified_canonical_mbwornot = cpo_invocable<bwornot_t, L, R> &&
    ...
    requires {
        {
            bwornot(internal::abi<A>, internal::declarg<S>(),
...
public:
    ...
    requires requires { bwornot(internal::abi<A>, lhs, rhs); }
    {
...
public:
    ...
    requires (extended_vector<L> || extended_vector<R>) &&
        unqualified_extended_bwornot<L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(L&& lhs, R&& rhs) {
        return bwornot(__DPL forward<L>(lhs), __DPL forward<L>(rhs));
    }
    ...
    requires (extended_vector<L> &&
                 ...
                 unqualified_extended_bwornot<L, R, simd_abi_type_t<L>>) ||
        (extended_vector<R> && broadcastable_to<L, result_or_decayed_t<R>> &&
            unqualified_extended_bwornot<L, R, simd_abi_type_t<R>>)
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(L&& lhs, R&& rhs) {
