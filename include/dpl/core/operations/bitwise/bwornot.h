...
concept unqualified_extended_bwornot = requires(L lhs, R rhs) {
    { bwornot(lhs, rhs) } -> vector_with_common_abi<A>;
};
...
concept unqualified_extended_mbwornot = requires(S src, M mask, L lhs, R rhs) {
    { bwornot(src, mask, lhs, rhs) } -> vector_with_common_abi<A>;
};
...
concept unqualified_extended_mask_bwornot = requires(L lhs, R rhs) {
    { bwornot(lhs, rhs) } -> mask_with_common_abi<A>;
};
...
private:
    ...
    requires (!simd_type<L> || canonical_vector<L>) &&
        (!simd_type<R> || canonical_vector<R>) && requires(L lhs, R rhs) {
            { bwornot(internal::abi<A>, lhs, rhs) } -> vector_with_abi<A>;
        }
    ...
    requires (!simd_type<L> || canonical_mask<L>) &&
        (!simd_type<R> || canonical_mask<R>) && requires(L lhs, R rhs) {
            { bwornot(internal::abi<A>, lhs, rhs) } -> mask_with_abi<A>;
        }
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto native(A abi, L lhs, R rhs) noexcept {
        return bwornot(internal::abi<A>, lhs, rhs);
