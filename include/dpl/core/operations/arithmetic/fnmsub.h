...
concept unqualified_canonical_fnmsub = requires(AT a, BT b, CT c) {
    {
        fnmsub(internal::abi<A>, a, b, c)
    } -> canonical_fma_result<AT, BT, CT, A>;
};

template <typename AT, typename BT, typename CT,
    typename A = common_abi_t<AT, BT, CT>>
concept unqualified_extended_fnmsub = requires(AT a, BT b, CT c) {
    { fnmsub(a, b, c) } -> extended_operation_vector<A>;
};

template <typename AT, typename BT, typename CT>
concept expression_fnmsub =
    (simd_expression<AT> || simd_expression<BT> || simd_expression<CT>) &&
    invocable<fnmsub_t, simd_expression_result_t<AT>,
        simd_expression_result_t<BT>, simd_expression_result_t<CT>>;

template <typename AT, typename BT, typename CT,
    typename A = common_abi_t<AT, BT, CT>>
concept decayable_fnmsub =
    ...
    regular_invocable<fnmsub_t, canonical_type_t<AT>, canonical_type_t<BT>,
        canonical_type_t<CT>>;

template <typename AT, typename BT, typename CT,
    typename A = common_abi_t<AT, BT, CT>>
concept unqualified_fnmsub = unqualified_extended_fnmsub<AT, BT, CT, A> ||
    expression_fnmsub<AT, BT, CT> || decayable_fnmsub<AT, BT, CT, A>;

template <typename S, typename M, typename AT, typename BT, typename CT,
    typename A = common_abi_t<AT, BT, CT, M>>
concept unqualified_canonical_mfnmsub = requires(
    S src, M mask, AT a, BT b, CT c) {
    {
        fnmsub(internal::abi<A>, src, mask, a, b, c)
    } -> equivalent_simd_as<
        canonical_if_zero_t<S, operation_result_t<fnmsub_t, AT, BT, CT>, A>>;
};

template <typename S, typename M, typename AT, typename BT, typename CT,
    typename A = common_abi_t<AT, BT, CT, M>>
concept unqualified_extended_mfnmsub =
    requires(S src, M mask, AT a, BT b, CT c) {
        { fnmsub(src, mask, a, b, c) } -> extended_operation_vector<A>;
    };
template <typename S, typename M, typename AT, typename BT, typename CT>
concept expression_mfnmsub =
    (simd_expression<S> || simd_expression<M> || simd_expression<AT> ||
        simd_expression<BT> || simd_expression<CT>) &&
    invocable<fnmsub_t, expression_result_or_zero_t<S>,
        simd_expression_result_t<M>, simd_expression_result_t<AT>,
        simd_expression_result_t<BT>, simd_expression_result_t<CT>>;

template <typename S, typename M, typename AT, typename BT, typename CT,
    typename A = common_abi_t<AT, BT, CT, M>>
concept decayable_mfnmsub =
    ...
    decayable_vector_for<BT, operation_category::lane_agnostic> &&
    decayable_vector_for<CT, operation_category::lane_agnostic> &&
    regular_invocable<fnmsub_t,
        canonical_or_zero_t<S, operation_result_t<fnmsub_t, AT, BT, CT>, A>,
        canonical_type_t<M>, canonical_type_t<AT>, canonical_type_t<BT>,
        canonical_type_t<CT>>;

template <typename S, typename M, typename AT, typename BT, typename CT,
    typename A = common_abi_t<AT, BT, CT, M>>
concept extended_mfnmsub = unqualified_extended_mfnmsub<S, M, AT, BT, CT, A> ||
    expression_mfnmsub<S, M, AT, BT, CT> ||
    decayable_mfnmsub<S, M, AT, BT, CT, A>;

template <typename S, typename M, typename AT, typename BT, typename CT,
    typename A = common_abi_t<
        canonical_if_zero_t<S, operation_result_t<fnmsub_t, AT, BT, CT>>,
        operation_result_t<fnmsub_t, AT, BT, CT>>>
concept unqualified_canonical_imfnmsub = requires(
    S src, M mask, AT a, BT b, CT c) {
    {
        fnmsub(internal::abi<A>, src,
            internal::to_const_mask<A, fnmsub_t, S, AT, BT, CT>(mask), a, b, c)
    } -> equivalent_simd_as<
        canonical_if_zero_t<S, operation_result_t<fnmsub_t, AT, BT, CT>, A>>;
};
...
template <typename S, typename M, typename AT, typename BT, typename CT,
    typename A = common_abi_t<
        canonical_if_zero_t<S, operation_result_t<fnmsub_t, AT, BT, CT>>,
        operation_result_t<fnmsub_t, AT, BT, CT>>>
concept unqualified_extended_imfnmsub = requires(
    S src, M mask, AT a, BT b, CT c) {
    {
        fnmsub(src, internal::to_const_mask<A, fnmsub_t, S, AT, BT, CT>(mask),
            a, b, c)
    } -> extended_operation_vector<A>;
};

template <typename S, typename M, typename AT, typename BT, typename CT>
concept expression_imfnmsub = (simd_expression<S> || simd_expression<AT> ||
                                  simd_expression<BT> || simd_expression<CT>) &&
    invocable<fnmsub_t, expression_result_or_zero_t<S>, M,
...
concept decayable_imfnmsub =
    ...
    decayable_vector_for<CT, operation_category::lane_agnostic> &&
    regular_invocable<fnmsub_t,
        canonical_or_zero_t<S, operation_result_t<fnmsub_t, AT, BT, CT>, A>, M,
...
template <typename S, typename M, typename AT, typename BT, typename CT,
    typename A = common_abi_t<
        canonical_if_zero_t<S, operation_result_t<fnmsub_t, AT, BT, CT>>,
        operation_result_t<fnmsub_t, AT, BT, CT>>>
concept extended_imfnmsub =
    unqualified_extended_imfnmsub<S, M, AT, BT, CT, A> ||
    expression_imfnmsub<S, M, AT, BT, CT> ||
    decayable_imfnmsub<S, M, AT, BT, CT, A>;

struct fnmsub_t : private ternary_operation_base<fnmsub_t> {
private:
    friend ternary_operation_base<fnmsub_t>;
    ...
    requires (!simd_class<AT> || canonical_vector<AT>) &&
        ...
        requires(AT a, BT b, CT c) {
            { fnmsub(internal::abi<A>, a, b, c) } -> vector_with_abi<A>;
        }
    ...
    static constexpr auto DPL_VECTORCALL fallback(basic_vector<E, A> a,
        ...
        return dx::negate(dx::fmadd(a, b, c));
    }
...
public:
    ...
    static constexpr basic_vector<E, A> operator()(basic_vector<E, A> a,
        basic_vector<E, A> b, basic_vector<E, A> c) noexcept {
        if constexpr (unqualified_canonical_fnmsub<basic_vector<E, A>,
    ...
    requires common_abi_with<common_abi_t<AA, BA>, CA> &&
        ...
        unqualified_canonical_fnmsub<basic_vector<E, AA>, basic_vector<E, BA>,
            basic_vector<E, CA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr basic_vector<E, common_abi_t<AA, BA, CA>> operator()(
        ...
        return fnmsub(internal::abi<common_abi_t<AA, BA, CA>>, a, b, c);
    }

    template <simd_vector AT, simd_vector BT, simd_vector CT>
    requires (extended_vector<AT> || extended_vector<BT> ||
                 extended_vector<CT>) &&
        unqualified_fnmsub<AT, BT, CT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(AT a, BT b, CT c) noexcept {
        if constexpr (unqualified_extended_fnmsub<AT, BT, CT>) {
            return fnmsub(a, b, c);
        } else if constexpr (expression_fnmsub<AT, BT, CT>) {
            return operator()(
    ...
    using ternary_operation_base<fnmsub_t>::operator();
    ...
    static constexpr basic_vector<E, A> operator()(basic_vector<E, A> src,
        ...
        if constexpr (unqualified_canonical_mfnmsub<basic_vector<E, A>,
                          basic_mask<ME, A>, basic_vector<E, A>,
                          basic_vector<E, A>, basic_vector<E, A>>) {
            if consteval {
                return internal::masked<fnmsub_t>(src, mask, a, b, c);
            } else {
                return fnmsub(internal::abi<A>, src, mask, a, b, c);
            }
        } else {
            return internal::masked<fnmsub_t>(src, mask, a, b, c);
        }
    ...
    requires common_abi_with<common_abi_t<AA, BA>, CA> &&
        ...
        unqualified_canonical_mfnmsub<basic_vector<E, SA>, basic_mask<ME, SA>,
            basic_vector<E, AA>, basic_vector<E, BA>, basic_vector<E, CA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr basic_vector<E, SA> operator()(basic_vector<E, SA> src,
        ...
        basic_vector<E, CA> c) noexcept {
        return fnmsub(internal::abi<SA>, src, mask, a, b, c);
    }
    ...
    requires (extended_vector<S> || extended_mask<M> || extended_vector<AT> ||
        ...
        maskable_args<S, M, AT, BT, CT> && extended_mfnmsub<S, M, AT, BT, CT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(S src, M mask, AT a, BT b, CT c) noexcept {
        if constexpr (unqualified_extended_mfnmsub<S, M, AT, BT, CT>) {
            return fnmsub(src, mask, a, b, c);
        } else if constexpr (expression_mfnmsub<S, M, AT, BT, CT>) {
    ...
    static constexpr basic_vector<E, A> operator()(basic_mask<ME, A> mask,
        ...
        basic_vector<E, A> c) noexcept {
        if constexpr (unqualified_canonical_mfnmsub<zero_t, basic_mask<ME, A>,
                          basic_vector<E, A>, basic_vector<E, A>,
                          basic_vector<E, A>>) {
            if consteval {
                return internal::masked<fnmsub_t>(mask, a, b, c);
            } else {
                return fnmsub(internal::abi<A>, dx::zero, mask, a, b, c);
            }
        } else {
    ...
    requires common_abi_with<common_abi_t<AA, BA>, CA> &&
        ...
        unqualified_canonical_mfnmsub<zero_t, basic_mask<ME, SA>,
            basic_vector<E, AA>, basic_vector<E, BA>, basic_vector<E, CA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    requires (extended_mask<M> || extended_vector<AT> || extended_vector<BT> ||
                 extended_vector<CT>) &&
        zmaskable_args<M, AT, BT, CT> && extended_mfnmsub<zero_t, M, AT, BT, CT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(M mask, AT a, BT b, CT c) noexcept {
        if constexpr (unqualified_extended_mfnmsub<zero_t, M, AT, BT, CT>) {
            return fnmsub(dx::zero, mask, a, b, c);
        } else if constexpr (expression_mfnmsub<zero_t, M, AT, BT, CT>) {
    ...
    template <simd_mask M, simd_vector AT, simd_vector BT, simd_vector CT>
    requires invocable<fnmsub_t, M, AT, BT, CT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    static constexpr basic_vector<E, A> operator()(basic_vector<E, A> src,
        ...
        basic_vector<E, A> c) noexcept {
        if constexpr (unqualified_canonical_imfnmsub<basic_vector<E, A>, M,
                          basic_vector<E, A>, basic_vector<E, A>,
                          basic_vector<E, A>>) {
            if consteval {
                return internal::masked<fnmsub_t>(src, mask, a, b, c);
    ...
    requires common_abi_with<common_abi_t<AA, BA>, CA> &&
        ...
        imm_maskable_args<basic_vector<E, SA>, basic_vector<E, AA>,
            basic_vector<E, BA>, basic_vector<E, CA>> &&
        unqualified_canonical_imfnmsub<basic_vector<E, SA>, M,
            basic_vector<E, AA>, basic_vector<E, BA>, basic_vector<E, CA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr basic_vector<E, SA> operator()(basic_vector<E, SA> src,
        M mask, basic_vector<E, AA> a, basic_vector<E, BA> b,
        basic_vector<E, CA> c) noexcept {
        return fnmsub(internal::abi<SA>, src,
    ...
    requires (extended_vector<S> || extended_vector<AT> ||
                 extended_vector<BT> || extended_vector<CT>) &&
        imm_maskable_args<S, AT, BT, CT> && extended_imfnmsub<S, M, AT, BT, CT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(S src, M mask, AT a, BT b, CT c) noexcept {
        if constexpr (unqualified_extended_imfnmsub<S, M, AT, BT, CT>) {
            return fnmsub(src, dx::to_compatible_const_mask<S>(mask), a, b, c);
        } else if constexpr (expression_imfnmsub<S, M, AT, BT, CT>) {
    ...
    static constexpr basic_vector<E, A> operator()(M mask, basic_vector<E, A> a,
        basic_vector<E, A> b, basic_vector<E, A> c) noexcept {
        if constexpr (unqualified_canonical_imfnmsub<zero_t, M,
                          basic_vector<E, A>, basic_vector<E, A>,
                          basic_vector<E, A>>) {
            if consteval {
                return internal::masked<fnmsub_t>(mask, a, b, c);
            } else {
                return fnmsub(internal::abi<A>, dx::zero,
                    dx::to_compatible_const_mask<basic_vector<E, A>>(mask), a,
                    b, c);
            }
        } else {
    ...
    requires common_abi_with<common_abi_t<AA, BA>, CA> &&
        ...
        imm_zmaskable_args<basic_vector<E, AA>, basic_vector<E, BA>,
            basic_vector<E, CA>> &&
        unqualified_canonical_imfnmsub<zero_t, M, basic_vector<E, AA>,
            basic_vector<E, BA>, basic_vector<E, CA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr basic_vector<E, common_abi_t<AA, BA, CA>> operator()(
        M mask, basic_vector<E, AA> a, basic_vector<E, BA> b,
    ...
    template <simd_vector AT, simd_vector BT, simd_vector CT,
        const_mask_for<operation_result_t<fnmsub_t, AT, BT, CT>> M>
    requires (extended_vector<AT> || extended_vector<BT> ||
                 extended_vector<CT>) &&
        imm_zmaskable_args<AT, BT, CT> &&
        extended_imfnmsub<zero_t, M, AT, BT, CT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(M mask, AT a, BT b, CT c) noexcept {
        using S = operation_result_t<fnmsub_t, AT, BT, CT>;
        if constexpr (unqualified_extended_imfnmsub<zero_t, M, AT, BT, CT>) {
            return fnmsub(
                dx::zero, dx::to_compatible_const_mask<S>(mask), a, b, c);
        } else if constexpr (expression_imfnmsub<zero_t, M, AT, BT, CT>) {
            return operator()(
