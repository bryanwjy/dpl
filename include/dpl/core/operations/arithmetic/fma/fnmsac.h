...
#include "dpl/core/operations/arithmetic/fma/fnmsub.h"
...
struct DPL_EMPTY_BASES fnmsac_t :
    public arithmetic_base<fnmsac_t>,
    public maskable_accumulation_base<fnmsac_t>,
    public ternary_broadcastable_operation<fnmsac_t> {
    using operation_base<fnmsac_t>::operator();
    using maskable_accumulation_base<fnmsac_t>::operator();
    using ternary_broadcastable_operation<fnmsac_t>::operator();
};
...
template <>
struct fallback_impl<fnmsac_t> : ternary_broadcasting_fallback<fnmsac_t> {
    ...
    requires floating_point_like<simd_element_type_t<CT>> &&
        ...
        simd_invocable<fmacc_t, CT, AT, BT>
    ...
    using ternary_broadcasting_fallback<fnmsac_t>::operator();
};

template <typename AT, typename BT, typename CT,
    typename A = simd_abi_type_t<AT>>
concept unqualified_canonical_fnmsac =
    unqualified_canonical_fnmsub<BT, CT, AT, A>;

template <typename M, typename AT, typename BT, typename CT,
    typename A = simd_abi_type_t<AT>>
concept unqualified_canonical_zmfnmsac =
    unqualified_canonical_zmfnmsub<M, BT, CT, AT, A>;

template <typename AT, typename M, typename BT, typename CT,
    typename A = simd_abi_type_t<AT>>
concept unqualified_canonical_mfnmsac = cpo_invocable<fnmsac_t, AT, BT, CT> &&
    equivalent_vector_with<AT, cpo_result_t<fnmsac_t, AT, BT, CT>> && requires {
        {
            fnmsub(internal::abi<A>, internal::declarg<BT>(),
...
public:
    ...
    requires canonical_vector<BT> && canonical_vector<CT> &&
        unqualified_canonical_fnmsac<AT, BT, CT, A>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr AT operator()(AT aval, BT bval, CT cval) noexcept {
        return fnmsub(internal::abi<A>, bval, cval, aval);
    }

    template <canonical_vector AT, vector_subsumed_by<AT> BT,
        broadcastable_to<AT> CT, typename A = simd_abi_type_t<AT>>
    requires canonical_vector<BT> && unqualified_canonical_fnmsac<AT, BT, CT, A>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr AT operator()(AT aval, BT bval, CT&& cval) noexcept {
        return fnmsub(internal::abi<A>, bval, __DPL forward<CT>(cval), aval);
    }

    template <canonical_vector AT, vector_subsumed_by<AT> CT,
        broadcastable_to<AT> BT, typename A = simd_abi_type_t<AT>>
    requires canonical_vector<CT> && unqualified_canonical_fnmsac<AT, BT, CT, A>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr AT operator()(AT aval, BT&& bval, CT cval) noexcept {
        return fnmsub(internal::abi<A>, __DPL forward<BT>(bval), cval, aval);
    }
    ...
    static constexpr vector_t<BT, CT> operator()(
        ...
        return fnmsub(internal::abi<A>, bval, cval, __DPL forward<AT>(aval));
    }

    template <canonical_vector AT, broadcastable_to<AT> BT,
        broadcastable_to<AT> CT, typename A = simd_abi_type_t<AT>>
    requires unqualified_canonical_fnmsac<AT, BT, CT, A>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr AT operator()(AT aval, BT&& bval, CT&& cval) noexcept {
        return fnmsub(internal::abi<A>, __DPL forward<BT>(bval),
    ...
    static constexpr BT operator()(AT&& aval, BT bval, CT&& cval) noexcept {
        return fnmsub(internal::abi<A>, bval, __DPL forward<CT>(cval),
    ...
    static constexpr CT operator()(AT&& aval, BT&& bval, CT cval) noexcept {
        return fnmsub(internal::abi<A>, __DPL forward<BT>(bval), cval,
            __DPL forward<AT>(aval));
    }
    ...
    requires canonical_vector<BT> && canonical_vector<CT> &&
        unqualified_canonical_mfnmsac<AT, mask_t<AT>, BT, CT, A>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr AT operator()(
        AT aval, mask_t<AT> mask, BT bval, CT cval) noexcept {
        return fnmsub(internal::abi<A>, bval, cval, aval, mask);
    }
    ...
    template <canonical_vector AT, const_mask_for<AT> M,
        ...
        typename A = simd_abi_type_t<AT>>
    requires canonical_vector<BT> && canonical_vector<CT> &&
        unqualified_canonical_mfnmsac<AT, launder_cmask_t<AT, M>, BT, CT, A>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr AT operator()(
    ...
    template <canonical_vector AT, vector_subsumed_by<AT> BT,
        vector_subsumed_by<AT> CT, typename A = simd_abi_type_t<AT>>
    requires canonical_vector<BT> && canonical_vector<CT> &&
        unqualified_canonical_zmfnmsac<mask_t<AT, BT, CT>, AT, BT, CT, A>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr AT operator()(dx::zero_t zero, mask_t<AT, BT, CT> mask,
        AT aval, BT bval, CT cval) noexcept {
        return fnmsub(internal::abi<A>, zero, mask, bval, cval, aval);
    }
    ...
    template <canonical_vector AT, vector_subsumed_by<AT> BT,
        ...
        typename A = simd_abi_type_t<AT>>
    requires canonical_vector<BT> && canonical_vector<CT> &&
        unqualified_canonical_zmfnmsac<launder_cmask_t<AT, M>, AT, BT, CT, A>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr AT operator()(
...
concept unqualified_extended_fnmsac =
    unqualified_extended_fnmsub<BT, CT, AT, A>;

template <typename AT, typename M, typename BT, typename CT>
concept unqualified_extended_mfnmsac = cpo_invocable<fnmsac_t, AT, BT, CT> &&
    equivalent_vector_with<AT, cpo_result_t<fnmsac_t, AT, BT, CT>> && requires {
        {
...
concept unqualified_extended_zmfnmsac =
    unqualified_extended_zmfnmsub<M, BT, CT, AT>;
...
public:
    ...
    requires (extended_vector<AT> || extended_vector<BT> ||
                 extended_vector<CT>) &&
        unqualified_extended_fnmsac<AT, BT, CT, A>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    template <extended_vector AT, broadcastable_to<AT> BT,
        broadcastable_to<AT> CT, typename A = simd_abi_type_t<AT>>
    requires unqualified_extended_fnmsac<AT, BT, CT, A>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(AT&& aval, BT&& bval, CT&& cval) {
        return fnmsub(__DPL forward<BT>(bval), __DPL forward<CT>(cval),
            __DPL forward<AT>(aval));
    ...
    template <simd_vector AT, exact_mask_for<AT> M, common_vector_with<AT> BT,
        common_vector_with<AT> CT, typename A = common_abi_t<AT, BT, CT>>
    ...
    requires (extended_vector<AT> || extended_vector<BT> ||
                 extended_vector<CT>) &&
        unqualified_extended_mfnmsac<AT, launder_cmask_t<AT, M>, BT, CT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    requires (extended_vector<AT> || extended_vector<BT> ||
                 extended_vector<CT>) &&
        unqualified_extended_zmfnmsac<launder_cmask_t<AT, M>, AT, BT, CT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(
