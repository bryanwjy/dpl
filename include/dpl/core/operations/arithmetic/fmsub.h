...
concept unqualified_extended_fmsub = requires(AT a, BT b, CT c) {
    { fmsub(a, b, c) } -> extended_fma_result<AT, BT, CT, A>;
};
...
concept decayable_mfmsub =
    ...
    decayable_vector_for<CT, operation_category::lane_agnostic> &&
    requires(fmsub_t op,
        canonical_if_zero_t<S, operation_result_t<fmsub_t, AT, BT, CT>, A> s,
...
template <typename S, typename C, typename AT, typename BT, typename CT,
    typename A = common_abi_t<AT, BT, CT, C>>
concept extended_mfmsub = unqualified_extended_mfmsub<S, C, AT, BT, CT, A> ||
    decayable_mfmsub<S, C, AT, BT, CT, A>;
...
struct fmsub_t : ternary_operation_base<fmsub_t> {
private:
    ...
    static constexpr auto native(A abi, AT a, BT b, CT c) noexcept
    requires requires {
        { fmsub(a, b, c) } -> broadcasting_arithmetic_result<A>;
    }
    {
        return fmsub(a, b, c);
    }
...
public:
    ...
    requires common_abi_with<common_abi_t<AA, BA>, CA> &&
        ...
        unqualified_canonical_mfmsub<basic_vector<E, SA>, basic_mask<MaskE, SA>,
            basic_vector<E, AA>, basic_vector<E, BA>, basic_vector<E, CA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    requires (extended_vector<S> || extended_mask<Mask> ||
        ...
        extended_mfmsub<S, Mask, AT, BT, CT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(
        S src, Mask mask, AT a, BT b, CT c) noexcept {
        if constexpr (unqualified_extended_mfmsub<S, Mask, AT, BT, CT>) {
    ...
    static constexpr basic_vector<E, A> operator()(basic_mask<MaskE, A> mask,
        ...
        basic_vector<E, A> c) noexcept {
        if constexpr (unqualified_canonical_mfmsub<zero_t, basic_mask<MaskE, A>,
                          basic_vector<E, A>, basic_vector<E, A>,
                          basic_vector<E, A>>) {
            if consteval {
    ...
    requires common_abi_with<common_abi_t<AA, BA>, CA> &&
        ...
        unqualified_canonical_mfmsub<zero_t, basic_mask<MaskE, SA>,
            basic_vector<E, AA>, basic_vector<E, BA>, basic_vector<E, CA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    requires (extended_mask<Mask> || extended_vector<AT> ||
        ...
        extended_mfmsub<zero_t, Mask, AT, BT, CT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(Mask mask, AT a, BT b, CT c) noexcept {
        if constexpr (unqualified_extended_mfmsub<zero_t, Mask, AT, BT, CT>) {
            return fmsub(mask, a, b, c);
        } else {
    ...
    template <simd_mask Mask, simd_vector AT, simd_vector BT, simd_vector CT>
    requires requires(
        Mask mask, AT a, BT b, CT c) { fmsub_t::operator()(mask, a, b, c); }
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(
    ...
    static constexpr basic_vector<E, A> operator()(basic_vector<E, A> src,
        ...
        basic_vector<E, A> c) noexcept {
        if constexpr (unqualified_canonical_imfmsub<basic_vector<E, A>, Mask,
                          basic_vector<E, A>, basic_vector<E, A>,
                          basic_vector<E, A>>) {
            if consteval {
                return internal::masked<fmsub_t>(src, mask, a, b, c);
    ...
    requires common_abi_with<common_abi_t<AA, BA>, CA> &&
        ...
        imm_maskable_args<basic_vector<E, SA>, basic_vector<E, AA>,
            basic_vector<E, BA>, basic_vector<E, CA>> &&
        unqualified_canonical_imfmsub<basic_vector<E, SA>, Mask,
            basic_vector<E, AA>, basic_vector<E, BA>, basic_vector<E, CA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr basic_vector<E, SA> operator()(basic_vector<E, SA> src,
    ...
    requires (extended_vector<S> || extended_vector<AT> ||
        ...
        extended_imfmsub<S, Mask, AT, BT, CT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(
        S src, Mask mask, AT a, BT b, CT c) noexcept {
        if constexpr (unqualified_extended_imfmsub<S, Mask, AT, BT, CT>) {
    ...
    static constexpr basic_vector<E, A> operator()(Mask mask,
        basic_vector<E, A> a, basic_vector<E, A> b,
        basic_vector<E, A> c) noexcept {
        if constexpr (unqualified_canonical_imfmsub<zero_t, Mask,
                          basic_vector<E, A>, basic_vector<E, A>,
                          basic_vector<E, A>>) {
            if consteval {
                return internal::masked<fmsub_t>(mask, a, b, c);
    ...
    requires common_abi_with<common_abi_t<AA, BA>, CA> &&
        ...
        imm_zmaskable_args<basic_vector<E, AA>, basic_vector<E, BA>,
            basic_vector<E, CA>> &&
        unqualified_canonical_imfmsub<zero_t, Mask, basic_vector<E, AA>,
            basic_vector<E, BA>, basic_vector<E, CA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr basic_vector<E, common_abi_t<AA, BA, CA>> operator()(
        Mask mask, basic_vector<E, AA> a, basic_vector<E, BA> b,
    ...
    template <simd_vector AT, simd_vector BT, simd_vector CT,
        const_mask_for<operation_result_t<fmsub_t, AT, BT, CT>> Mask>
    requires (extended_vector<AT> || extended_vector<BT> ||
                 extended_vector<CT>) &&
        imm_zmaskable_args<AT, BT, CT> &&
        extended_imfmsub<zero_t, Mask, AT, BT, CT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(Mask mask, AT a, BT b, CT c) noexcept {
        using S = operation_result_t<fmsub_t, AT, BT, CT>;
        if constexpr (unqualified_extended_imfmsub<zero_t, Mask, AT, BT, CT>) {
            return fmsub(
