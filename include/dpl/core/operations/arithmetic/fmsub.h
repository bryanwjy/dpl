...
concept unqualified_canonical_fmsub = requires(AT a, BT b, CT c) {
    { fmsub(internal::abi<A>, a, b, c) } -> canonical_fma_result<AT, BT, CT, A>;
};

template <typename AT, typename BT, typename CT,
    typename A = common_abi_t<AT, BT, CT>>
concept unqualified_extended_fmsub = requires(AT a, BT b, CT c) {
    { fmsub(a, b, c) } -> extended_operation_vector<A>;
};

template <typename AT, typename BT, typename CT>
concept expression_fmsub =
    (simd_expression<AT> || simd_expression<BT> || simd_expression<CT>) &&
    invocable<fmsub_t, simd_expression_result_t<AT>,
        simd_expression_result_t<BT>, simd_expression_result_t<CT>>;

template <typename AT, typename BT, typename CT,
    typename A = common_abi_t<AT, BT, CT>>
concept decayable_fmsub =
    ...
    regular_invocable<fmsub_t, canonical_type_t<AT>, canonical_type_t<BT>,
        canonical_type_t<CT>>;

template <typename AT, typename BT, typename CT,
    typename A = common_abi_t<AT, BT, CT>>
concept unqualified_fmsub = unqualified_extended_fmsub<AT, BT, CT, A> ||
    expression_fmsub<AT, BT, CT> || decayable_fmsub<AT, BT, CT, A>;

template <typename S, typename M, typename AT, typename BT, typename CT,
    typename A = common_abi_t<AT, BT, CT, M>>
concept unqualified_canonical_mfmsub =
    requires(S src, M mask, AT a, BT b, CT c) {
        {
            fmsub(internal::abi<A>, src, mask, a, b, c)
        } -> equivalent_simd_as<
            canonical_if_zero_t<S, operation_result_t<fmsub_t, AT, BT, CT>, A>>;
    };

template <typename S, typename M, typename AT, typename BT, typename CT,
    typename A = common_abi_t<AT, BT, CT, M>>
concept unqualified_extended_mfmsub =
    requires(S src, M mask, AT a, BT b, CT c) {
        { fmsub(src, mask, a, b, c) } -> extended_operation_vector<A>;
    };
template <typename S, typename M, typename AT, typename BT, typename CT>
concept expression_mfmsub =
    (simd_expression<S> || simd_expression<M> || simd_expression<AT> ||
        simd_expression<BT> || simd_expression<CT>) &&
    invocable<fmsub_t, expression_result_or_zero_t<S>,
        simd_expression_result_t<M>, simd_expression_result_t<AT>,
        simd_expression_result_t<BT>, simd_expression_result_t<CT>>;

template <typename S, typename M, typename AT, typename BT, typename CT,
    typename A = common_abi_t<AT, BT, CT, M>>
concept decayable_mfmsub =
    ...
    decayable_vector_for<BT, operation_category::lane_agnostic> &&
    decayable_vector_for<CT, operation_category::lane_agnostic> &&
    regular_invocable<fmsub_t,
        canonical_or_zero_t<S, operation_result_t<fmsub_t, AT, BT, CT>, A>,
        canonical_type_t<M>, canonical_type_t<AT>, canonical_type_t<BT>,
        canonical_type_t<CT>>;

template <typename S, typename M, typename AT, typename BT, typename CT,
    typename A = common_abi_t<AT, BT, CT, M>>
concept extended_mfmsub = unqualified_extended_mfmsub<S, M, AT, BT, CT, A> ||
    expression_mfmsub<S, M, AT, BT, CT> ||
    decayable_mfmsub<S, M, AT, BT, CT, A>;

template <typename S, typename M, typename AT, typename BT, typename CT,
    typename A = common_abi_t<
        canonical_if_zero_t<S, operation_result_t<fmsub_t, AT, BT, CT>>,
        operation_result_t<fmsub_t, AT, BT, CT>>>
concept unqualified_canonical_imfmsub = requires(
    S src, M mask, AT a, BT b, CT c) {
    {
        fmsub(internal::abi<A>, src,
            internal::to_const_mask<A, fmsub_t, S, AT, BT, CT>(mask), a, b, c)
    } -> equivalent_simd_as<
        canonical_if_zero_t<S, operation_result_t<fmsub_t, AT, BT, CT>, A>>;
};
...
template <typename S, typename M, typename AT, typename BT, typename CT,
    typename A = common_abi_t<
        canonical_if_zero_t<S, operation_result_t<fmsub_t, AT, BT, CT>>,
        operation_result_t<fmsub_t, AT, BT, CT>>>
concept unqualified_extended_imfmsub =
    requires(S src, M mask, AT a, BT b, CT c) {
        {
            fmsub(src, internal::to_const_mask<A, fmsub_t, S, AT, BT, CT>(mask),
                a, b, c)
        } -> extended_operation_vector<A>;
    };

template <typename S, typename M, typename AT, typename BT, typename CT>
concept expression_imfmsub = (simd_expression<S> || simd_expression<AT> ||
                                 simd_expression<BT> || simd_expression<CT>) &&
    invocable<fmsub_t, expression_result_or_zero_t<S>, M,
...
concept decayable_imfmsub =
    ...
    decayable_vector_for<CT, operation_category::lane_agnostic> &&
    regular_invocable<fmsub_t,
        canonical_or_zero_t<S, operation_result_t<fmsub_t, AT, BT, CT>, A>, M,
...
template <typename S, typename M, typename AT, typename BT, typename CT,
    typename A = common_abi_t<
        canonical_if_zero_t<S, operation_result_t<fmsub_t, AT, BT, CT>>,
        operation_result_t<fmsub_t, AT, BT, CT>>>
concept extended_imfmsub = unqualified_extended_imfmsub<S, M, AT, BT, CT, A> ||
    expression_imfmsub<S, M, AT, BT, CT> ||
    decayable_imfmsub<S, M, AT, BT, CT, A>;

struct fmsub_t : private ternary_operation_base<fmsub_t> {
private:
    friend ternary_operation_base<fmsub_t>;
    ...
    requires (!simd_class<AT> || canonical_vector<AT>) &&
        ...
        requires(AT a, BT b, CT c) {
            { fmsub(internal::abi<A>, a, b, c) } -> vector_with_abi<A>;
        }
...
public:
    ...
    static constexpr basic_vector<E, A> operator()(basic_vector<E, A> a,
        basic_vector<E, A> b, basic_vector<E, A> c) noexcept {
        if constexpr (unqualified_canonical_fmsub<basic_vector<E, A>,
    ...
    requires common_abi_with<common_abi_t<AA, BA>, CA> &&
        ...
        unqualified_canonical_fmsub<basic_vector<E, AA>, basic_vector<E, BA>,
            basic_vector<E, CA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr basic_vector<E, common_abi_t<AA, BA, CA>> operator()(
        ...
        return fmsub(internal::abi<common_abi_t<AA, BA, CA>>, a, b, c);
    }

    template <simd_vector AT, simd_vector BT, simd_vector CT>
    requires (extended_vector<AT> || extended_vector<BT> ||
                 extended_vector<CT>) &&
        unqualified_fmsub<AT, BT, CT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(AT a, BT b, CT c) noexcept {
        if constexpr (unqualified_extended_fmsub<AT, BT, CT>) {
            return fmsub(a, b, c);
        } else if constexpr (expression_fmsub<AT, BT, CT>) {
            return operator()(
    ...
    using ternary_operation_base<fmsub_t>::operator();
    ...
    static constexpr basic_vector<E, A> operator()(basic_vector<E, A> src,
        ...
        if constexpr (unqualified_canonical_mfmsub<basic_vector<E, A>,
                          basic_mask<ME, A>, basic_vector<E, A>,
                          basic_vector<E, A>, basic_vector<E, A>>) {
            if consteval {
                return internal::masked<fmsub_t>(src, mask, a, b, c);
            } else {
                return fmsub(internal::abi<A>, src, mask, a, b, c);
            }
        } else {
            return internal::masked<fmsub_t>(src, mask, a, b, c);
        }
    ...
    requires common_abi_with<common_abi_t<AA, BA>, CA> &&
        ...
        unqualified_canonical_mfmsub<basic_vector<E, SA>, basic_mask<ME, SA>,
            basic_vector<E, AA>, basic_vector<E, BA>, basic_vector<E, CA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr basic_vector<E, SA> operator()(basic_vector<E, SA> src,
        ...
        basic_vector<E, CA> c) noexcept {
        return fmsub(internal::abi<SA>, src, mask, a, b, c);
    }
    ...
    requires (extended_vector<S> || extended_mask<M> || extended_vector<AT> ||
        ...
        maskable_args<S, M, AT, BT, CT> && extended_mfmsub<S, M, AT, BT, CT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(S src, M mask, AT a, BT b, CT c) noexcept {
        if constexpr (unqualified_extended_mfmsub<S, M, AT, BT, CT>) {
            return fmsub(src, mask, a, b, c);
        } else if constexpr (expression_mfmsub<S, M, AT, BT, CT>) {
    ...
    static constexpr basic_vector<E, A> operator()(basic_mask<ME, A> mask,
        ...
        basic_vector<E, A> c) noexcept {
        if constexpr (unqualified_canonical_mfmsub<zero_t, basic_mask<ME, A>,
                          basic_vector<E, A>, basic_vector<E, A>,
                          basic_vector<E, A>>) {
            if consteval {
                return internal::masked<fmsub_t>(mask, a, b, c);
            } else {
                return fmsub(internal::abi<A>, dx::zero, mask, a, b, c);
            }
        } else {
    ...
    requires common_abi_with<common_abi_t<AA, BA>, CA> &&
        ...
        unqualified_canonical_mfmsub<zero_t, basic_mask<ME, SA>,
            basic_vector<E, AA>, basic_vector<E, BA>, basic_vector<E, CA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    requires (extended_mask<M> || extended_vector<AT> || extended_vector<BT> ||
                 extended_vector<CT>) &&
        zmaskable_args<M, AT, BT, CT> && extended_mfmsub<zero_t, M, AT, BT, CT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(M mask, AT a, BT b, CT c) noexcept {
        if constexpr (unqualified_extended_mfmsub<zero_t, M, AT, BT, CT>) {
            return fmsub(dx::zero, mask, a, b, c);
        } else if constexpr (expression_mfmsub<zero_t, M, AT, BT, CT>) {
    ...
    template <simd_mask M, simd_vector AT, simd_vector BT, simd_vector CT>
    requires invocable<fmsub_t, M, AT, BT, CT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    static constexpr basic_vector<E, A> operator()(basic_vector<E, A> src,
        ...
        basic_vector<E, A> c) noexcept {
        if constexpr (unqualified_canonical_imfmsub<basic_vector<E, A>, M,
                          basic_vector<E, A>, basic_vector<E, A>,
                          basic_vector<E, A>>) {
            if consteval {
                return internal::masked<fmsub_t>(src, mask, a, b, c);
    ...
    requires common_abi_with<common_abi_t<AA, BA>, CA> &&
        ...
        imm_maskable_args<basic_vector<E, SA>, basic_vector<E, AA>,
            basic_vector<E, BA>, basic_vector<E, CA>> &&
        unqualified_canonical_imfmsub<basic_vector<E, SA>, M,
            basic_vector<E, AA>, basic_vector<E, BA>, basic_vector<E, CA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr basic_vector<E, SA> operator()(basic_vector<E, SA> src,
        M mask, basic_vector<E, AA> a, basic_vector<E, BA> b,
        basic_vector<E, CA> c) noexcept {
        return fmsub(internal::abi<SA>, src,
    ...
    requires (extended_vector<S> || extended_vector<AT> ||
                 extended_vector<BT> || extended_vector<CT>) &&
        imm_maskable_args<S, AT, BT, CT> && extended_imfmsub<S, M, AT, BT, CT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(S src, M mask, AT a, BT b, CT c) noexcept {
        if constexpr (unqualified_extended_imfmsub<S, M, AT, BT, CT>) {
            return fmsub(src, dx::to_compatible_const_mask<S>(mask), a, b, c);
        } else if constexpr (expression_imfmsub<S, M, AT, BT, CT>) {
    ...
    static constexpr basic_vector<E, A> operator()(M mask, basic_vector<E, A> a,
        basic_vector<E, A> b, basic_vector<E, A> c) noexcept {
        if constexpr (unqualified_canonical_imfmsub<zero_t, M,
                          basic_vector<E, A>, basic_vector<E, A>,
                          basic_vector<E, A>>) {
            if consteval {
                return internal::masked<fmsub_t>(mask, a, b, c);
            } else {
                return fmsub(internal::abi<A>, dx::zero,
                    dx::to_compatible_const_mask<basic_vector<E, A>>(mask), a,
                    b, c);
            }
        } else {
    ...
    requires common_abi_with<common_abi_t<AA, BA>, CA> &&
        ...
        imm_zmaskable_args<basic_vector<E, AA>, basic_vector<E, BA>,
            basic_vector<E, CA>> &&
        unqualified_canonical_imfmsub<zero_t, M, basic_vector<E, AA>,
            basic_vector<E, BA>, basic_vector<E, CA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr basic_vector<E, common_abi_t<AA, BA, CA>> operator()(
        M mask, basic_vector<E, AA> a, basic_vector<E, BA> b,
    ...
    template <simd_vector AT, simd_vector BT, simd_vector CT,
        const_mask_for<operation_result_t<fmsub_t, AT, BT, CT>> M>
    requires (extended_vector<AT> || extended_vector<BT> ||
                 extended_vector<CT>) &&
        imm_zmaskable_args<AT, BT, CT> &&
        extended_imfmsub<zero_t, M, AT, BT, CT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(M mask, AT a, BT b, CT c) noexcept {
        using S = operation_result_t<fmsub_t, AT, BT, CT>;
        if constexpr (unqualified_extended_imfmsub<zero_t, M, AT, BT, CT>) {
            return fmsub(
                dx::zero, dx::to_compatible_const_mask<S>(mask), a, b, c);
        } else if constexpr (expression_imfmsub<zero_t, M, AT, BT, CT>) {
            return operator()(
