...
concept decayable_fmaddsub =
    ...
    regular_invocable<fmaddsub_t, canonical_type_t<AT>, canonical_type_t<BT>,
        canonical_type_t<CT>>;

template <typename AT, typename BT, typename CT,
    typename A = common_abi_t<AT, BT, CT>>
concept unqualified_fmaddsub = unqualified_extended_fmaddsub<AT, BT, CT, A> ||
    expression_fmaddsub<AT, BT, CT> || decayable_fmaddsub<AT, BT, CT, A>;
...
concept expression_mfmaddsub =
    ...
    invocable<fmaddsub_t, expression_result_or_zero_t<S>,
...
concept decayable_mfmaddsub =
    ...
    decayable_vector_for<BT, operation_category::lane_agnostic> &&
    decayable_vector_for<CT, operation_category::lane_agnostic> &&
    regular_invocable<fmaddsub_t,
        canonical_or_zero_t<S, operation_result_t<fmaddsub_t, AT, BT, CT>, A>,
...
concept extended_mfmaddsub =
    unqualified_extended_mfmaddsub<S, M, AT, BT, CT, A> ||
    expression_mfmaddsub<S, M, AT, BT, CT> ||
    decayable_mfmaddsub<S, M, AT, BT, CT, A>;
...
concept unqualified_canonical_imfmaddsub = requires(
    S src, M mask, AT a, BT b, CT c) {
    {
...
template <typename S, typename M, typename AT, typename BT, typename CT,
    typename A = common_abi_t<
        canonical_if_zero_t<S, operation_result_t<fmaddsub_t, AT, BT, CT>>,
        operation_result_t<fmaddsub_t, AT, BT, CT>>>
concept unqualified_extended_imfmaddsub =
    requires(S src, M mask, AT a, BT b, CT c) {
        {
            fmaddsub(src,
...
template <typename S, typename M, typename AT, typename BT, typename CT>
concept expression_imfmaddsub =
    (simd_expression<S> || simd_expression<AT> || simd_expression<BT> ||
        simd_expression<CT>) &&
    invocable<fmaddsub_t, expression_result_or_zero_t<S>, M,
...
concept extended_imfmaddsub =
    ...
    expression_imfmaddsub<S, M, AT, BT, CT> ||
    decayable_imfmaddsub<S, M, AT, BT, CT, A>;

struct fmaddsub_t : private ternary_operation_base<fmaddsub_t> {
private:
...
public:
    ...
    static constexpr auto operator()(AT a, BT b, CT c) noexcept {
        if constexpr (unqualified_extended_fmaddsub<AT, BT, CT>) {
            return fmaddsub(a, b, c);
        } else if constexpr (expression_fmaddsub<AT, BT, CT>) {
            return operator()(
    ...
    requires (extended_vector<S> || extended_mask<M> || extended_vector<AT> ||
        ...
        maskable_args<S, M, AT, BT, CT> && extended_mfmaddsub<S, M, AT, BT, CT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(S src, M mask, AT a, BT b, CT c) noexcept {
        if constexpr (unqualified_extended_mfmaddsub<S, M, AT, BT, CT>) {
            return fmaddsub(src, mask, a, b, c);
        } else if constexpr (expression_mfmaddsub<S, M, AT, BT, CT>) {
    ...
    static constexpr basic_vector<E, A> operator()(basic_mask<ME, A> mask,
        ...
        basic_vector<E, A> c) noexcept {
        if constexpr (unqualified_canonical_mfmaddsub<zero_t, basic_mask<ME, A>,
                          basic_vector<E, A>, basic_vector<E, A>,
                          basic_vector<E, A>>) {
            if consteval {
    ...
    requires (extended_mask<M> || extended_vector<AT> || extended_vector<BT> ||
        ...
        extended_mfmaddsub<zero_t, M, AT, BT, CT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(M mask, AT a, BT b, CT c) noexcept {
        if constexpr (unqualified_extended_mfmaddsub<zero_t, M, AT, BT, CT>) {
            return fmaddsub(dx::zero, mask, a, b, c);
        } else if constexpr (expression_mfmaddsub<zero_t, M, AT, BT, CT>) {
    ...
    template <simd_mask M, simd_vector AT, simd_vector BT, simd_vector CT>
    requires invocable<fmaddsub_t, M, AT, BT, CT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    static constexpr basic_vector<E, A> operator()(basic_vector<E, A> src,
        ...
        basic_vector<E, A> c) noexcept {
        if constexpr (unqualified_canonical_imfmaddsub<basic_vector<E, A>, M,
                          basic_vector<E, A>, basic_vector<E, A>,
                          basic_vector<E, A>>) {
            if consteval {
                return internal::masked<fmaddsub_t>(src, mask, a, b, c);
    ...
    static constexpr basic_vector<E, SA> operator()(basic_vector<E, SA> src,
        ...
        basic_vector<E, CA> c) noexcept {
        return fmaddsub(internal::abi<SA>, src,
    ...
    requires (extended_vector<S> || extended_vector<AT> ||
        ...
        extended_imfmaddsub<S, M, AT, BT, CT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(S src, M mask, AT a, BT b, CT c) noexcept {
        if constexpr (unqualified_extended_imfmaddsub<S, M, AT, BT, CT>) {
            return fmaddsub(
                src, dx::to_compatible_const_mask<S>(mask), a, b, c);
        } else if constexpr (expression_imfmaddsub<S, M, AT, BT, CT>) {
    ...
    static constexpr basic_vector<E, A> operator()(M mask, basic_vector<E, A> a,
        basic_vector<E, A> b, basic_vector<E, A> c) noexcept {
        if constexpr (unqualified_canonical_imfmaddsub<zero_t, M,
                          basic_vector<E, A>, basic_vector<E, A>,
                          basic_vector<E, A>>) {
            if consteval {
                return internal::masked<fmaddsub_t>(mask, a, b, c);
            } else {
                return fmaddsub(internal::abi<A>, dx::zero,
                    dx::to_compatible_const_mask<basic_vector<E, A>>(mask), a,
                    b, c);
            }
        } else {
    ...
    template <simd_vector AT, simd_vector BT, simd_vector CT,
        const_mask_for<operation_result_t<fmaddsub_t, AT, BT, CT>> M>
    requires (extended_vector<AT> || extended_vector<BT> ||
                 extended_vector<CT>) &&
        imm_zmaskable_args<AT, BT, CT> &&
        extended_imfmaddsub<zero_t, M, AT, BT, CT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(M mask, AT a, BT b, CT c) noexcept {
        using S = operation_result_t<fmaddsub_t, AT, BT, CT>;
        if constexpr (unqualified_extended_imfmaddsub<zero_t, M, AT, BT, CT>) {
            return fmaddsub(
                dx::zero, dx::to_compatible_const_mask<S>(mask), a, b, c);
        } else if constexpr (expression_imfmaddsub<zero_t, M, AT, BT, CT>) {
            return operator()(
