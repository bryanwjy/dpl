...
concept unqualified_canonical_mdivide = cpo_invocable<divide_t, L, R> &&
    (!simd_type<S> || same_as<S, cpo_result_t<divide_t, L, R>>) && requires {
        {
            divide(internal::abi<A>, internal::declarg<S>(),
                internal::declarg<M>(), internal::declarg<L>(),
                internal::declarg<R>())
        } -> same_as<cpo_result_t<divide_t, L, R>>;
    };
...
public:
    ...
    requires canonical_vector<R> &&
        unqualified_canonical_mdivide<result_t<L, R>, mask_t<L, R>, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    requires canonical_vector<R> &&
        unqualified_canonical_mdivide<result_t<L, R>,
            launder_cmask_t<result_t<L, R>, M>, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr result_t<L, R> operator()(
        result_t<L, R> src, M cmask, L lhs, R rhs) noexcept {
        return divide(internal::abi<common_abi_t<L, R>>, src,
    ...
    template <canonical_vector L, common_vector_with<L> R,
        const_mask_for<result_t<L, R>> M>
    requires canonical_vector<R> &&
        unqualified_canonical_mdivide<dx::zero_t,
            launder_cmask_t<result_t<L, R>, M>, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr result_t<L, R> operator()(
        dx::zero_t zero, M cmask, L lhs, R rhs) noexcept {
        return divide(internal::abi<common_abi_t<L, R>>, zero,
            dx::to_const_mask<result_t<L, R>>(cmask), lhs, rhs);
    }
};
...
template <typename S, typename M, typename L, typename R>
concept unqualified_extended_mdivide = cpo_invocable<divide_t, L, R> &&
    (!simd_type<S> ||
...
public:
    ...
    static constexpr auto operator()(S&& src, M cmask, L&& lhs, R&& rhs) {
        return divide(src, dx::to_const_mask<S>(cmask), __DPL forward<L>(lhs),
            __DPL forward<R>(rhs));
    }

    template <simd_vector L, common_vector_with<L> R,
        result_mask_for<divide_t, L, R> M>
    ...
    template <simd_vector L, common_vector_with<L> R,
        result_cmask_for<divide_t, L, R> M>
    requires (extended_vector<L> || extended_vector<R>) &&
        unqualified_extended_mdivide<dx::zero_t,
            launder_cmask_t<cpo_result_t<divide_t, L, R>, M>, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(
        dx::zero_t zero, M cmask, L&& lhs, R&& rhs) {
        return divide(zero,
            dx::to_const_mask<cpo_result_t<divide_t, L, R>>(cmask),
            __DPL forward<L>(lhs), __DPL forward<R>(rhs));
    }
};
} // namespace datapar::internal
