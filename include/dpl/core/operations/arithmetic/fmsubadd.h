...
template <typename AT, typename BT, typename CT,
    typename A = common_abi_t<AT, BT, CT>>
concept unqualified_fmsubadd = unqualified_canonical_fmsubadd<AT, BT, CT, A> ||
...
concept unqualified_canonical_imfmsubadd = requires(S src, AT a, BT b, CT c) {
    {
        fmsubadd<
            const_mask_v<canonical_if_zero_t<S,
                             operation_result_t<fmsubadd_t, AT, BT, CT>, A>,
...
template <typename S, typename M, typename AT, typename BT, typename CT,
    typename A = common_abi_t<
        canonical_if_zero_t<S, operation_result_t<fmsubadd_t, AT, BT, CT>>,
        operation_result_t<fmsubadd_t, AT, BT, CT>>>
concept unqualified_extended_imfmsubadd = requires(S src, AT a, BT b, CT c) {
    {
        fmsubadd<
            const_mask_v<canonical_if_zero_t<S,
                             operation_result_t<fmsubadd_t, AT, BT, CT>, A>,
...
public:
    ...
    static constexpr auto operator()(
        ...
        if constexpr (unqualified_extended_imfmsubadd<S, Mask, AT, BT, CT>) {
            ...
            return fmsubadd<V>(src, dx::masked_operation, a, b, c);
        } else {
    ...
    static constexpr auto operator()(Mask mask, AT a, BT b, CT c) noexcept {
        ...
        if constexpr (unqualified_extended_imfmsubadd<zero_t, Mask, AT, BT,
                          CT>) {
            constexpr auto V = const_mask_v<S, Mask>;
            return fmsubadd<V>(dx::zero, masked_operation, a, b, c);
        } else {
