...
concept unqualified_extended_add = requires(L lhs, R rhs) {
    { add(lhs, rhs) } -> vector_with_common_abi<A>;
};

template <typename L, typename R>
concept expression_add = (simd_expression<L> || simd_expression<R>) &&
    invocable<add_t, simd_expression_result_t<L>, simd_expression_result_t<R>>;

template <typename L, typename R, typename A = common_abi_t<L, R>>
concept decayable_add =
...
template <typename L, typename R, typename A = common_abi_t<L, R>>
concept extended_add = unqualified_extended_add<L, R, A> ||
    expression_add<L, R> || decayable_add<L, R, A>;

template <typename S, typename M, typename L, typename R,
    typename A = common_abi_t<L, R, M>>
concept unqualified_canonical_madd = requires(S src, M mask, L lhs, R rhs) {
    {
...
template <typename S, typename M, typename L, typename R,
    typename A = common_abi_t<L, R, M>>
concept unqualified_extended_madd = requires(S src, M mask, L lhs, R rhs) {
    { add(src, mask, lhs, rhs) } -> vector_with_common_abi<A>;
};

template <typename S, typename M, typename L, typename R>
concept expression_madd = (simd_expression<S> || simd_expression<M> ||
                              simd_expression<L> || simd_expression<R>) &&
    invocable<add_t, expression_result_or_zero_t<S>,
...
concept decayable_madd =
    ...
    decayable_vector_for<L, operation_category::lane_agnostic> &&
    decayable_vector_for<R, operation_category::lane_agnostic> &&
    regular_invocable<add_t,
        canonical_or_zero_t<S, operation_result_t<add_t, L, R>, A>,
        canonical_type_t<M>, canonical_type_t<L>, canonical_type_t<R>>;

template <typename S, typename M, typename L, typename R,
    typename A = common_abi_t<L, R, M>>
concept extended_madd = unqualified_extended_madd<S, M, L, R, A> ||
    expression_madd<S, M, L, R> || decayable_madd<S, M, L, R, A>;

template <typename S, typename M, typename L, typename R,
    typename A =
        common_abi_t<canonical_if_zero_t<S, operation_result_t<add_t, L, R>>,
            operation_result_t<add_t, L, R>>>
concept unqualified_canonical_imadd = requires(S src, M mask, L lhs, R rhs) {
    {
        add(internal::abi<A>, src,
            internal::to_const_mask<A, add_t, S, L, R>(mask), lhs, rhs)
    } -> equivalent_simd_type_with<
        canonical_if_zero_t<S, operation_result_t<add_t, L, R>, A>>;
};
...
template <typename S, typename M, typename L, typename R,
    typename A =
        common_abi_t<canonical_if_zero_t<S, operation_result_t<add_t, L, R>>,
            operation_result_t<add_t, L, R>>>
concept unqualified_extended_imadd = requires(S src, M mask, L lhs, R rhs) {
    {
        add(src, internal::to_const_mask<A, add_t, S, L, R>(mask), lhs, rhs)
    } -> vector_with_common_abi<A>;
};

template <typename S, typename M, typename L, typename R>
concept expression_imadd =
    (simd_expression<S> || simd_expression<L> || simd_expression<R>) &&
    invocable<add_t, expression_result_or_zero_t<S>, M,
...
concept decayable_imadd =
    ...
    decayable_vector_for<R, operation_category::lane_agnostic> &&
    regular_invocable<add_t,
        canonical_or_zero_t<S, operation_result_t<add_t, L, R>, A>, M,
...
template <typename S, typename M, typename L, typename R,
    typename A =
        common_abi_t<canonical_if_zero_t<S, operation_result_t<add_t, L, R>>,
            operation_result_t<add_t, L, R>>>
concept extended_imadd = unqualified_extended_imadd<S, M, L, R, A> ||
    expression_imadd<S, M, L, R> || decayable_imadd<S, M, L, R, A>;

struct add_t : private binary_operation_base<add_t> {
private:
...
public:
    ...
    requires (extended_vector<L> || extended_vector<R>) && extended_add<L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(L lhs, R rhs) noexcept {
        if constexpr (unqualified_extended_add<L, R>) {
            return add(lhs, rhs);
        } else if constexpr (expression_add<L, R>) {
    ...
    requires (different_from<LA, RA> || scalable_abi<SA> || scalable_abi<LA> ||
        ...
        unqualified_canonical_madd<basic_vector<E, SA>, basic_mask<ME, SA>,
            basic_vector<E, LA>, basic_vector<E, RA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    requires (extended_vector<S> || extended_mask<M> || extended_vector<L> ||
                 extended_vector<R>) &&
        maskable_args<S, M, L, R> && extended_madd<S, M, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(S src, M mask, L lhs, R rhs) noexcept {
        if constexpr (unqualified_extended_madd<S, M, L, R>) {
            return add(src, mask, lhs, rhs);
        } else if constexpr (expression_madd<S, M, L, R>) {
    ...
    static constexpr basic_vector<E, A> operator()(basic_mask<ME, A> mask,
        basic_vector<E, A> lhs, basic_vector<E, A> rhs) noexcept {
        if constexpr (unqualified_canonical_madd<zero_t, basic_mask<ME, A>,
    ...
    requires (different_from<SA, common_abi_t<LA, RA>> ||
        ...
        zmaskable_args<basic_mask<ME, SA>, basic_vector<E, LA>,
            basic_vector<E, RA>> &&
        unqualified_canonical_madd<zero_t, basic_mask<ME, SA>,
            basic_vector<E, LA>, basic_vector<E, RA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    requires (extended_mask<M> || extended_vector<L> || extended_vector<R>) &&
        zmaskable_args<M, L, R> && extended_madd<zero_t, M, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(M mask, L lhs, R rhs) noexcept {
        if constexpr (unqualified_extended_madd<zero_t, M, L, R>) {
            return add(dx::zero, mask, lhs, rhs);
        } else if constexpr (expression_madd<zero_t, M, L, R>) {
            return operator()(
    ...
    static constexpr basic_vector<E, A> operator()(basic_vector<E, A> src,
        ...
        if constexpr (unqualified_canonical_imadd<basic_vector<E, A>, M,
                          basic_vector<E, A>, basic_vector<E, A>>) {
            if consteval {
                return internal::masked<add_t>(src, mask, lhs, rhs);
            } else {
                return add(internal::abi<A>, src,
    ...
    requires (different_from<SA, common_abi_t<LA, RA>> ||
        ...
        imm_maskable_args<basic_vector<E, SA>, basic_vector<E, LA>,
            basic_vector<E, RA>> &&
        unqualified_canonical_imadd<basic_vector<E, SA>, M, basic_vector<E, LA>,
            basic_vector<E, RA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr basic_vector<E, SA> operator()(basic_vector<E, SA> src,
        M mask, basic_vector<E, LA> lhs, basic_vector<E, RA> rhs) noexcept {
        return add(internal::abi<SA>, src,
    ...
    requires (extended_vector<S> || extended_vector<L> || extended_vector<R>) &&
        imm_maskable_args<S, L, R> && extended_imadd<S, M, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(S src, M mask, L lhs, R rhs) noexcept {
        if constexpr (unqualified_extended_imadd<S, M, L, R>) {
            return add(src, dx::to_compatible_const_mask<S>(mask), lhs, rhs);
        } else if constexpr (expression_imadd<S, M, L, R>) {
            return operator()(
    ...
    static constexpr basic_vector<E, A> operator()(
        M mask, basic_vector<E, A> lhs, basic_vector<E, A> rhs) noexcept {
        if constexpr (unqualified_canonical_imadd<zero_t, M, basic_vector<E, A>,
                          basic_vector<E, A>>) {
            if consteval {
                return internal::masked<add_t>(mask, lhs, rhs);
            } else {
                return add(internal::abi<A>, dx::zero,
                    dx::to_compatible_const_mask<basic_vector<E, A>>(mask), lhs,
                    rhs);
            }
        } else {
    ...
    requires (different_from<LA, RA> || scalable_abi<LA> || scalable_abi<RA> ||
        ...
        unqualified_canonical_imadd<zero_t, M, basic_vector<E, LA>,
            basic_vector<E, RA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr basic_vector<E, common_abi_t<LA, RA>> operator()(
        M mask, basic_vector<E, LA> lhs, basic_vector<E, RA> rhs) noexcept {
    ...
    requires (extended_vector<L> || extended_vector<R>) &&
        imm_zmaskable_args<L, R> && extended_imadd<zero_t, M, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(M mask, L lhs, R rhs) noexcept {
        using S = operation_result_t<add_t, L, R>;
        constexpr auto cmask = dx::to_compatible_const_mask<S>(mask);
        if constexpr (unqualified_extended_imadd<zero_t, M, L, R>) {
            return add(dx::zero, cmask, lhs, rhs);
        } else if constexpr (expression_imadd<zero_t, M, L, R>) {
