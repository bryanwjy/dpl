...
concept unqualified_canonical_subtract = requires(L lhs, R rhs) {
    {
...
template <typename L, typename R, typename A = common_abi_t<L, R>>
concept unqualified_extended_subtract = requires(L lhs, R rhs) {
    { subtract(lhs, rhs) } -> vector_with_common_abi<A>;
};

template <typename L, typename R>
concept expression_subtract = (simd_expression<L> || simd_expression<R>) &&
    invocable<subtract_t, simd_expression_result_t<L>,
        simd_expression_result_t<R>>;

template <typename L, typename R, typename A = common_abi_t<L, R>>
concept decayable_subtract = (decayable_vector_for<L,
                                  operation_category::lane_agnostic> &&
    decayable_vector_for<R, operation_category::lane_agnostic> &&
    regular_invocable<subtract_t, canonical_type_t<L>, canonical_type_t<R>>);

template <typename L, typename R, typename A = common_abi_t<L, R>>
concept extended_subtract = unqualified_extended_subtract<L, R, A> ||
    expression_subtract<L, R> || decayable_subtract<L, R, A>;
...
template <typename S, typename M, typename L, typename R,
    typename A = common_abi_t<L, R, M>>
concept unqualified_extended_msubtract = requires(S src, M mask, L lhs, R rhs) {
    { subtract(src, mask, lhs, rhs) } -> vector_with_common_abi<A>;
};

template <typename S, typename M, typename L, typename R>
concept expression_msubtract = (simd_expression<S> || simd_expression<M> ||
                                   simd_expression<L> || simd_expression<R>) &&
    invocable<subtract_t, expression_result_or_zero_t<S>,
...
template <typename S, typename M, typename L, typename R,
    typename A = common_abi_t<L, R, M>>
concept extended_msubtract = unqualified_extended_msubtract<S, M, L, R, A> ||
    expression_msubtract<S, M, L, R> || decayable_msubtract<S, M, L, R, A>;
...
template <typename S, typename M, typename L, typename R>
concept expression_imsubtract =
    (simd_expression<S> || simd_expression<L> || simd_expression<R>) &&
    invocable<subtract_t, expression_result_or_zero_t<S>, M,
...
concept extended_imsubtract = unqualified_extended_imsubtract<S, M, L, R, A> ||
    expression_imsubtract<S, M, L, R> || decayable_imsubtract<S, M, L, R, A>;

struct subtract_t : private binary_operation_base<subtract_t> {
private:
...
public:
    ...
    static constexpr basic_vector<E, A> operator()(
        basic_vector<E, A> lhs, basic_vector<E, A> rhs) noexcept {
        if constexpr (unqualified_canonical_subtract<basic_vector<E, A>,
                          basic_vector<E, A>>) {
            if consteval {
    ...
    requires simd_element_for<E, LA> && simd_element_for<E, RA> &&
        (scalable_abi<LA> || scalable_abi<RA> || different_from<LA, RA> ||
            !basic_element<E>) &&
        unqualified_canonical_subtract<basic_vector<E, LA>, basic_vector<E, RA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr basic_vector<E, common_abi_t<LA, RA>> operator()(
        basic_vector<E, LA> lhs, basic_vector<E, RA> rhs) noexcept {
        return subtract(internal::abi<common_abi_t<LA, RA>>, lhs, rhs);
    }

    template <simd_vector L, simd_vector R>
    requires (extended_vector<L> || extended_vector<R>) &&
        extended_subtract<L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(L lhs, R rhs) noexcept {
        if constexpr (unqualified_extended_subtract<L, R>) {
            return subtract(lhs, rhs);
        } else if constexpr (expression_subtract<L, R>) {
    ...
    static constexpr basic_vector<E, A> operator()(basic_vector<E, A> src,
        ...
        if constexpr (unqualified_canonical_msubtract<basic_vector<E, A>,
            ...
            if consteval {
                ...
                return subtract(internal::abi<A>, src, mask, lhs, rhs);
            }
        } else {
            return internal::masked<subtract_t>(src, mask, lhs, rhs);
        }
    ...
    requires (different_from<LA, RA> || scalable_abi<SA> || scalable_abi<LA> ||
        ...
        unqualified_canonical_msubtract<basic_vector<E, SA>, basic_mask<ME, SA>,
            basic_vector<E, LA>, basic_vector<E, RA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr basic_vector<E, SA> operator()(basic_vector<E, SA> src,
        ...
        return subtract(internal::abi<SA>, src, mask, lhs, rhs);
    }
    ...
    requires (extended_vector<S> || extended_mask<M> || extended_vector<L> ||
                 extended_vector<R>) &&
        maskable_args<S, M, L, R> && extended_msubtract<S, M, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(S src, M mask, L lhs, R rhs) noexcept {
        if constexpr (unqualified_extended_msubtract<S, M, L, R>) {
            return subtract(src, mask, lhs, rhs);
        } else if constexpr (expression_msubtract<S, M, L, R>) {
    ...
    static constexpr basic_vector<E, A> operator()(basic_mask<ME, A> mask,
        basic_vector<E, A> lhs, basic_vector<E, A> rhs) noexcept {
        if constexpr (unqualified_canonical_msubtract<zero_t, basic_mask<ME, A>,
                          basic_vector<E, A>, basic_vector<E, A>>) {
            if consteval {
                return internal::masked<subtract_t>(mask, lhs, rhs);
            } else {
                return subtract(internal::abi<A>, dx::zero, mask, lhs, rhs);
            }
        } else {
    ...
    requires (different_from<SA, common_abi_t<LA, RA>> ||
        ...
        zmaskable_args<basic_mask<ME, SA>, basic_vector<E, LA>,
            basic_vector<E, RA>> &&
        unqualified_canonical_msubtract<zero_t, basic_mask<ME, SA>,
            basic_vector<E, LA>, basic_vector<E, RA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    requires (extended_mask<M> || extended_vector<L> || extended_vector<R>) &&
        zmaskable_args<M, L, R> && extended_msubtract<zero_t, M, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(M mask, L lhs, R rhs) noexcept {
        if constexpr (unqualified_extended_msubtract<zero_t, M, L, R>) {
            return subtract(dx::zero, mask, lhs, rhs);
        } else if constexpr (expression_msubtract<zero_t, M, L, R>) {
            return operator()(
    ...
    static constexpr basic_vector<E, A> operator()(basic_vector<E, A> src,
        ...
        if constexpr (unqualified_canonical_imsubtract<basic_vector<E, A>, M,
                          basic_vector<E, A>, basic_vector<E, A>>) {
            if consteval {
                return internal::masked<subtract_t>(src, mask, lhs, rhs);
            } else {
    ...
    requires (different_from<SA, common_abi_t<LA, RA>> ||
        ...
        imm_maskable_args<basic_vector<E, SA>, basic_vector<E, LA>,
            basic_vector<E, RA>> &&
        unqualified_canonical_imsubtract<basic_vector<E, SA>, M,
            basic_vector<E, LA>, basic_vector<E, RA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr basic_vector<E, SA> operator()(basic_vector<E, SA> src,
    ...
    requires (extended_vector<S> || extended_vector<L> || extended_vector<R>) &&
        imm_maskable_args<S, L, R> && extended_imsubtract<S, M, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(S src, M mask, L lhs, R rhs) noexcept {
        if constexpr (unqualified_extended_imsubtract<S, M, L, R>) {
            return subtract(
                src, dx::to_compatible_const_mask<S>(mask), lhs, rhs);
        } else if constexpr (expression_imsubtract<S, M, L, R>) {
            return operator()(
    ...
    static constexpr basic_vector<E, A> operator()(
        M mask, basic_vector<E, A> lhs, basic_vector<E, A> rhs) noexcept {
        if constexpr (unqualified_canonical_imsubtract<zero_t, M,
                          basic_vector<E, A>, basic_vector<E, A>>) {
            if consteval {
                return internal::masked<subtract_t>(mask, lhs, rhs);
            } else {
    ...
    requires (different_from<LA, RA> || scalable_abi<LA> || scalable_abi<RA> ||
        ...
        unqualified_canonical_imsubtract<zero_t, M, basic_vector<E, LA>,
            basic_vector<E, RA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr basic_vector<E, common_abi_t<LA, RA>> operator()(
        M mask, basic_vector<E, LA> lhs, basic_vector<E, RA> rhs) noexcept {
    ...
    requires (extended_vector<L> || extended_vector<R>) &&
        imm_zmaskable_args<L, R> && extended_imsubtract<zero_t, M, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(M mask, L lhs, R rhs) noexcept {
        using S = operation_result_t<subtract_t, L, R>;
        constexpr auto cmask = dx::to_compatible_const_mask<S>(mask);
        if constexpr (unqualified_extended_imsubtract<zero_t, M, L, R>) {
            return subtract(dx::zero, cmask, lhs, rhs);
        } else if constexpr (expression_imsubtract<zero_t, M, L, R>) {
