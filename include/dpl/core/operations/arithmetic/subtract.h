...
concept unqualified_extended_subtract = requires(L lhs, R rhs) {
    { subtract(lhs, rhs) } -> extended_arithmetic_result<L, R, A>;
};

template <typename L, typename R, typename A = common_abi_t<L, R>>
concept unqualified_subtract = unqualified_canonical_subtract<L, R> ||
    unqualified_extended_subtract<L, R> ||
    (decayable_vector_for<L, operation_category::lane_agnostic> &&
        decayable_vector_for<R, operation_category::lane_agnostic> &&
        unqualified_canonical_subtract<canonical_type_t<L>,
            canonical_type_t<R>>);
...
concept unqualified_canonical_subtracti = requires(S src, L lhs, R rhs) {
    {
...
template <typename Op, typename S, typename M, typename L, typename R,
    typename A =
        common_abi_t<canonical_if_zero_t<S, operation_result_t<Op, L, R>>,
            operation_result_t<Op, L, R>>>
concept unqualified_extended_subtracti = requires(S src, L lhs, R rhs) {
    {
        subtract<const_mask_v<
            canonical_if_zero_t<S, operation_result_t<Op, L, R>, A>, M>>(
...
template <typename Op, typename S, typename M, typename L, typename R,
    typename A =
        common_abi_t<canonical_if_zero_t<S, operation_result_t<Op, L, R>>,
            operation_result_t<Op, L, R>>>
concept extended_subtracti = unqualified_extended_subtracti<S, M, L, R, A> ||
    decayable_subtracti<Op, S, M, L, R, A>;
...
public:
    ...
    requires requires(
        Mask mask, L lhs, R rhs) { subtract_t::operator()(mask, lhs, rhs); }
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    static constexpr basic_vector<E, A> operator()(basic_vector<E, A> src,
        ...
        if constexpr (unqualified_canonical_subtracti<subtract_t,
    ...
    requires (different_from<SA, common_abi_t<LA, RA>> ||
        ...
        imm_maskable_args<basic_vector<E, SA>, basic_vector<E, LA>,
            basic_vector<E, RA>> &&
        unqualified_canonical_subtracti<subtract_t, basic_vector<E, SA>, Mask,
            basic_vector<E, LA>, basic_vector<E, RA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr basic_vector<E, SA> operator()(basic_vector<E, SA> src,
    ...
    requires (extended_vector<S> || extended_vector<L> || extended_vector<R>) &&
        ...
        extended_subtracti<subtract_t, S, Mask, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(S src, Mask mask, L lhs, R rhs) noexcept {
        if constexpr (unqualified_extended_subtracti<subtract_t, S, Mask, L,
                          R>) {
    ...
    static constexpr basic_vector<E, A> operator()(
        Mask mask, basic_vector<E, A> lhs, basic_vector<E, A> rhs) noexcept {
        if constexpr (unqualified_canonical_subtracti<subtract_t, zero_t, Mask,
                          basic_vector<E, A>, basic_vector<E, A>>) {
            if consteval {
                return internal::masked<subtract_t>(mask, lhs, rhs);
            } else {
    ...
    requires (different_from<LA, RA> || scalable_abi<LA> || scalable_abi<RA> ||
        ...
        unqualified_canonical_subtracti<subtract_t, zero_t, Mask,
            basic_vector<E, LA>, basic_vector<E, RA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr basic_vector<E, common_abi_t<LA, RA>> operator()(
        Mask mask, basic_vector<E, LA> lhs, basic_vector<E, RA> rhs) noexcept {
    ...
    requires (extended_vector<L> || extended_vector<R>) &&
        ...
        extended_subtracti<subtract_t, zero_t, Mask, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(Mask mask, L lhs, R rhs) noexcept {
