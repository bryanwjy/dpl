...
concept unqualified_extended_fnmadd = requires(AT a, BT b, CT c) {
    { fnmadd(a, b, c) } -> vector_with_common_abi<A>;
};
...
concept unqualified_extended_mfnmadd =
    requires(S src, M mask, AT a, BT b, CT c) {
        { fnmadd(src, mask, a, b, c) } -> vector_with_common_abi<A>;
    };
template <typename S, typename M, typename AT, typename BT, typename CT>
...
private:
    ...
    requires (!simd_type<AT> || canonical_vector<AT>) &&
        ...
        (!simd_type<CT> || canonical_vector<CT>) && requires(AT a, BT b, CT c) {
            { fnmadd(internal::abi<A>, a, b, c) } -> vector_with_abi<A>;
        }
...
public:
    ...
    template <simd_mask M, simd_vector AT, simd_vector BT, simd_vector CT>
    requires invocable<fnmadd_t, M, AT, BT, CT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
