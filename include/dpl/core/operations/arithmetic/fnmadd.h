...
struct DPL_EMPTY_BASES fnmadd_t :
    private arithmetic_base<fnmadd_t>,
    private maskable_transform_base<fnmadd_t>,
    private ternary_broadcastable_operation<fnmadd_t> {
    using operation_base<fnmadd_t>::operator();
    using maskable_transform_base<fnmadd_t>::operator();
...
struct fallback_impl<fnmadd_t> : ternary_broadcasting_fallback<fnmadd_t> {
    ...
    requires floating_point<simd_element_type_t<AT>> &&
        ...
        cpo_invocable<subtract_t, CT, cpo_result_t<multiply_t, AT, BT>>
...
concept unqualified_canonical_mfnmadd = cpo_invocable<fnmadd_t, AT, BT, CT> &&
    (!simd_type<S> ||
        equivalent_vector_with<S, cpo_result_t<fnmadd_t, AT, BT, CT>>) &&
    requires {
        {
            fnmadd(internal::abi<A>, internal::declarg<S>(),
...
public:
    ...
    template <canonical_vector AT, common_vector_with<AT> BT,
        ...
        typename A = common_abi_t<AT, BT, CT>>
    requires canonical_vector<BT> && canonical_vector<CT> &&
        unqualified_canonical_mfnmadd<result_t<AT, BT, CT>,
    ...
    template <canonical_vector AT, common_vector_with<AT> BT,
        common_vector_with<BT> CT, typename A = common_abi_t<AT, BT, CT>>
    requires canonical_vector<BT> && canonical_vector<CT> &&
        unqualified_canonical_mfnmadd<dx::zero_t, mask_t<AT, BT, CT>, AT, BT,
            CT, A>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
...
concept unqualified_extended_mfnmadd = cpo_invocable<fnmadd_t, AT, BT, CT> &&
    ...
    requires {
        {
            fnmadd(internal::declarg<S>(), internal::declarg<M>(),
...
public:
    ...
    requires (extended_vector<S> || extended_mask<M> || extended_vector<AT> ||
        ...
        unqualified_extended_mfnmadd<S, M, AT, BT, CT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(
        S&& src, M&& mask, AT&& aval, BT&& bval, CT&& cval) {
        return fnmadd(__DPL forward<S>(src), __DPL forward<M>(mask),
    ...
    requires (extended_vector<S> || extended_vector<AT> ||
                 extended_vector<BT> || extended_vector<CT>) &&
        unqualified_extended_mfnmadd<S, launder_cmask_t<S, M>, AT, BT, CT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(
        S&& src, M cmask, AT&& aval, BT&& bval, CT&& cval) {
        return fnmadd(__DPL forward<S>(src), dx::to_const_mask<S>(cmask),
            __DPL forward<AT>(aval), __DPL forward<BT>(bval),
            __DPL forward<CT>(cval));
    }

    template <simd_vector AT, common_vector_with<AT> BT,
        common_vector_with<BT> CT, result_mask_for<fnmadd_t, AT, BT, CT> M>
    requires (extended_mask<M> || extended_vector<AT> || extended_vector<BT> ||
                 extended_vector<CT>) &&
        unqualified_extended_mfnmadd<dx::zero_t, M, AT, BT, CT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(
    ...
    template <simd_vector AT, common_vector_with<AT> BT,
        common_vector_with<BT> CT, result_cmask_for<fnmadd_t, AT, BT, CT> M>
    requires (extended_vector<AT> || extended_vector<BT> ||
                 extended_vector<CT>) &&
        unqualified_extended_mfnmadd<dx::zero_t,
            launder_cmask_t<cpo_result_t<fnmadd_t, AT, BT, CT>, M>, AT, BT, CT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(
        dx::zero_t zero, M cmask, AT&& aval, BT&& bval, CT&& cval) {
        return fnmadd(zero,
            dx::to_const_mask<cpo_result_t<fnmadd_t, AT, BT, CT>>(cmask),
            __DPL forward<AT>(aval), __DPL forward<BT>(bval),
            __DPL forward<CT>(cval));
    }
};
