...
template <typename L, typename R, typename A = common_abi_t<L, R>>
concept extended_subadd = unqualified_extended_subadd<L, R, A> ||
    expression_subadd<L, R> || decayable_subadd<L, R, A>;
...
template <typename S, typename M, typename L, typename R>
concept expression_msubadd = (simd_expression<S> || simd_expression<M> ||
                                 simd_expression<L> || simd_expression<R>) &&
    invocable<subadd_t, expression_result_or_zero_t<S>,
...
template <typename S, typename M, typename L, typename R,
    typename A = common_abi_t<L, R, M>>
concept extended_msubadd = unqualified_extended_msubadd<S, M, L, R, A> ||
    expression_msubadd<S, M, L, R> || decayable_msubadd<S, M, L, R, A>;
...
template <typename S, typename M, typename L, typename R>
concept expression_imsubadd =
    (simd_expression<S> || simd_expression<L> || simd_expression<R>) &&
    invocable<subadd_t, expression_result_or_zero_t<S>, M,
...
concept extended_imsubadd = unqualified_extended_imsubadd<S, M, L, R, A> ||
    expression_imsubadd<S, M, L, R> || decayable_imsubadd<S, M, L, R, A>;
...
public:
    ...
    requires (extended_vector<L> || extended_vector<R>) && extended_subadd<L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(L lhs, R rhs) noexcept {
        if constexpr (unqualified_extended_subadd<L, R>) {
            return subadd(lhs, rhs);
        } else if constexpr (expression_subadd<L, R>) {
    ...
    requires (different_from<LA, RA> || scalable_abi<SA> || scalable_abi<LA> ||
        ...
        unqualified_canonical_msubadd<basic_vector<E, SA>, basic_mask<ME, SA>,
            basic_vector<E, LA>, basic_vector<E, RA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    requires (extended_vector<S> || extended_mask<M> || extended_vector<L> ||
                 extended_vector<R>) &&
        maskable_args<S, M, L, R> && extended_msubadd<S, M, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(S src, M mask, L lhs, R rhs) noexcept {
        if constexpr (unqualified_extended_msubadd<S, M, L, R>) {
            return subadd(src, mask, lhs, rhs);
        } else if constexpr (expression_msubadd<S, M, L, R>) {
    ...
    static constexpr basic_vector<E, A> operator()(basic_mask<ME, A> mask,
        basic_vector<E, A> lhs, basic_vector<E, A> rhs) noexcept {
        if constexpr (unqualified_canonical_msubadd<zero_t, basic_mask<ME, A>,
    ...
    requires (different_from<SA, common_abi_t<LA, RA>> ||
        ...
        zmaskable_args<basic_mask<ME, SA>, basic_vector<E, LA>,
            basic_vector<E, RA>> &&
        unqualified_canonical_msubadd<zero_t, basic_mask<ME, SA>,
            basic_vector<E, LA>, basic_vector<E, RA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    requires (extended_mask<M> || extended_vector<L> || extended_vector<R>) &&
        zmaskable_args<M, L, R> && extended_msubadd<zero_t, M, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(M mask, L lhs, R rhs) noexcept {
        if constexpr (unqualified_extended_msubadd<zero_t, M, L, R>) {
            return subadd(dx::zero, mask, lhs, rhs);
        } else if constexpr (expression_msubadd<zero_t, M, L, R>) {
            return operator()(
    ...
    template <simd_mask M, simd_vector L, simd_vector R>
    requires invocable<subadd_t, M, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    static constexpr basic_vector<E, A> operator()(basic_vector<E, A> src,
        ...
        if constexpr (unqualified_canonical_imsubadd<basic_vector<E, A>, M,
                          basic_vector<E, A>, basic_vector<E, A>>) {
            if consteval {
                return internal::masked<subadd_t>(src, mask, lhs, rhs);
            } else {
    ...
    requires (different_from<SA, common_abi_t<LA, RA>> ||
        ...
        imm_maskable_args<basic_vector<E, SA>, basic_vector<E, LA>,
            basic_vector<E, RA>> &&
        unqualified_canonical_imsubadd<basic_vector<E, SA>, M,
            basic_vector<E, LA>, basic_vector<E, RA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr basic_vector<E, SA> operator()(basic_vector<E, SA> src,
    ...
    requires (extended_vector<S> || extended_vector<L> || extended_vector<R>) &&
        imm_maskable_args<S, L, R> && extended_imsubadd<S, M, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(S src, M mask, L lhs, R rhs) noexcept {
        if constexpr (unqualified_extended_imsubadd<S, M, L, R>) {
            return subadd(src, dx::to_compatible_const_mask<S>(mask), lhs, rhs);
        } else if constexpr (expression_imsubadd<S, M, L, R>) {
            return operator()(
    ...
    static constexpr basic_vector<E, A> operator()(
        M mask, basic_vector<E, A> lhs, basic_vector<E, A> rhs) noexcept {
        if constexpr (unqualified_canonical_imsubadd<zero_t, M,
                          basic_vector<E, A>, basic_vector<E, A>>) {
            if consteval {
                return internal::masked<subadd_t>(mask, lhs, rhs);
            } else {
    ...
    template <simd_vector L, simd_vector R,
        const_mask_for<operation_result_t<subadd_t, L, R>> M>
    requires (extended_vector<L> || extended_vector<R>) &&
        imm_zmaskable_args<L, R> && extended_imsubadd<zero_t, M, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(M mask, L lhs, R rhs) noexcept {
        using S = operation_result_t<subadd_t, L, R>;
        constexpr auto cmask = dx::to_compatible_const_mask<S>(mask);
        if constexpr (unqualified_extended_imsubadd<zero_t, M, L, R>) {
            return subadd(dx::zero, cmask, lhs, rhs);
        } else if constexpr (expression_imsubadd<zero_t, M, L, R>) {
