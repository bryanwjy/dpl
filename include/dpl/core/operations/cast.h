...
concept unqualified_element_castable_to = simd_type<From> &&
...
private:
    ...
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, CONST, NODISCARD)
    static constexpr basic_simd<To, A>
        DPL_VECTORCALL fallback(basic_simd<FromE, A> arg) noexcept {
...
template <typename From, typename To>
concept unqualified_abi_target_castable = simd_type<From> && simd_abi<To> &&
...
template <typename From, typename To>
concept unqualified_abi_source_castable = simd_type<From> && simd_abi<To> &&
    simd_element_for<typename From::value_type, To> && requires(From arg) {
        {
...
private:
    ...
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, CONST, NODISCARD)
    static constexpr basic_simd<E, To>
        DPL_VECTORCALL fallback(basic_simd<E, From> arg) noexcept {
        if constexpr (same_as<To, From>) {
            return arg;
        } else {
            using src_t = basic_simd<E, From>;
            using dst_t = basic_simd<E, To>;
    ...
    template <typename E, typename From>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, CONST, NODISCARD)
    static constexpr basic_simd_mask<E, To>
        DPL_VECTORCALL fallback(basic_simd_mask<E, From> arg) noexcept {
        using src_t = basic_simd_mask<E, From>;
        using dst_t = basic_simd_mask<E, To>;
        if constexpr (same_as<To, From>) {
            return arg;
...
public:
    ...
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr basic_simd<E, To> operator()(
        basic_simd<E, FromA> val) noexcept
    ...
    requires (scalable_abi<To> || scalable_abi<FromA>) &&
        (same_as<To, FromA> ||
            unqualified_abi_castable<basic_simd<E, FromA>, To>)
        DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
        static constexpr basic_simd<E, To> operator()(
            basic_simd<E, FromA> val) noexcept
    ...
    template <simd_element_for<To> E, fixed_width_abi FromA>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr basic_simd_mask<E, To> operator()(
        basic_simd_mask<E, FromA> val) noexcept
    requires fixed_width_abi<To> && common_abi_with<To, FromA>
    {
    ...
    requires (scalable_abi<To> || scalable_abi<FromA>) &&
        (same_as<To, FromA> ||
            unqualified_abi_castable<basic_simd_mask<E, FromA>, To>)
        DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
        static constexpr basic_simd_mask<E, To> operator()(
            basic_simd_mask<E, FromA> val) noexcept
    requires common_abi_with<To, FromA>
...
struct simd_cast_t<basic_simd<ToE, ToA>> {
private:
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(basic_simd<ToE, ToA> from) noexcept {
        return from;
    }

    template <simd_abi A>
    requires regular_invocable<abi_cast_t<ToA>, basic_simd<ToE, A>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(basic_simd<ToE, A> from) noexcept {
        return abi_cast_t<ToA>::operator()(from);
    }

    template <simd_element_for<ToA> E>
    requires regular_invocable<element_cast_t<ToE>, basic_simd<E, ToA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(basic_simd<E, ToA> from) noexcept {
        return element_cast_t<ToE>::operator()(from);
    }

    template <extended_simd From>
    requires explicitly_convertible_to<From, basic_simd<ToE, ToA>> ||
        ...
        (same_as<typename From::abi_type, ToA> &&
            regular_invocable<element_cast_t<ToE>,
                basic_simd<typename From::value_type, ToA>> &&
            same_as<invoke_result_t<element_cast_t<ToE>, From>,
                basic_simd<ToE, ToA>>)
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(From from) noexcept {
        if constexpr (explicitly_convertible_to<From, basic_simd<ToE, ToA>>) {
            return static_cast<basic_simd<ToE, ToA>>(from);
        } else if constexpr (simd_with<From, ToE, ToA>) {
            return dx::to_basic_type(from);
