...
concept unqualified_canonical_mcountr_zero =
    cpo_invocable<countr_zero_t, T> && requires(S src, M mask, T val) {
        {
...
public:
    ...
    template <canonical_vector T>
    requires unqualified_canonical_mcountr_zero<unsigned_canonical_vector_t<T>,
...
concept unqualified_extended_vector_countr_zero = requires {
    {
        countr_zero(internal::declarg<T>())
...
template <typename S, typename M, typename T>
concept unqualified_extended_mcountr_zero_base =
    cpo_invocable<countr_zero_t, T> && requires {
        {
...
concept unqualified_extended_mcountr_zero =
    unqualified_extended_mcountr_zero_base<S, M, T> &&
    equivalent_vector_with<S, unsigned_canonical_vector_t<T>>;

template <typename M, typename T>
concept unqualified_extended_zmcountr_zero =
    unqualified_extended_mcountr_zero_base<dx::zero_t, M, T>;
...
public:
    ...
    requires (extended_mask<M> || extended_vector<T>) &&
        unqualified_extended_zmcountr_zero<M, T>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    template <extended_vector T, result_cmask_for<countr_zero_t, T> M>
    requires unqualified_extended_zmcountr_zero<
