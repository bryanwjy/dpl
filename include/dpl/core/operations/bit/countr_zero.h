...
concept unqualified_extended_countr_zero = requires(T val) {
    { countr_zero(val) } -> vector_with_common_abi<typename T::abi_type>;
};
...
concept unqualified_extended_mcountr_zero = requires(S src, M mask, T val) {
    { countr_zero(src, mask, val) } -> vector_with_common_abi<A>;
};
...
public:
    ...
    static constexpr basic_vector<unsigned_representation_t<E>, A> operator()(
        ...
        if constexpr (unqualified_canonical_mcountr_zero<
            ...
            if consteval {
                return internal::masked<countr_zero_t>(src, mask, val);
            } else {
                return countr_zero(internal::abi<A>, src, mask, val);
            }
        } else {
            return internal::masked<countr_zero_t>(src, mask, val);
        }
    ...
    static constexpr basic_vector<E, MA> operator()(basic_vector<E, MA> src,
        ...
        return countr_zero(internal::abi<MA>, src, mask, val);
    }
    ...
    requires extended_vector_bit<S, T> &&
        ...
        maskable_args<S, M, T> && extended_mcountr_zero<countr_zero_t, S, M, T>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    static constexpr auto operator()(
        basic_mask<ME, MA> mask, basic_vector<E, TA> val) noexcept {
        return countr_zero(internal::abi<MA>, dx::zero, mask, val);
    }

    template <simd_mask M, simd_vector T>
    requires (extended_mask<M> || extended_vector<T>) && zmaskable_args<M, T> &&
        extended_mcountr_zero<countr_zero_t, zero_t, M, T>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    template <simd_mask M, simd_vector T>
    requires requires(M mask, T val) { countr_zero_t::operator()(mask, val); }
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    static constexpr basic_vector<unsigned_representation_t<E>, A> operator()(
        ...
        if constexpr (unqualified_canonical_imcountr_zero<
            ...
            if consteval {
                ...
                return countr_zero(internal::abi<A>, src,
    ...
    static constexpr basic_vector<unsigned_representation_t<E>, SA> operator()(
        ...
        return countr_zero(internal::abi<SA>, src,
    ...
    static constexpr basic_vector<unsigned_representation_t<E>, A> operator()(
        ...
        if constexpr (unqualified_canonical_imcountr_zero<zero_t, M,
                          basic_vector<E, A>>) {
            if consteval {
                return internal::masked<countr_zero_t>(mask, val);
            } else {
