...
public:
    ...
    requires unqualified_canonical_mcountr_one<result_t<T>, cmask_t<T, M>, T>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(
        result_t<T> src, cmask_t<T, M> cmask, T val) noexcept {
        return countr_one(internal::abi<T>, src, cmask, val);
    }
    ...
    template <fixed_width_vector T, imask_t<T> M>
    requires unqualified_canonical_mcountr_one<dx::zero_t, cmask_t<T, M>, T>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(
        dx::zero_t zero, cmask_t<T, M> cmask, T val) noexcept {
        return countr_one(internal::abi<T>, zero, cmask, val);
    }
};
...
public:
    ...
    requires (extended_vector<S> || extended_vector<T>) &&
        unqualified_extended_mcountr_one<S, cmask_t<S, M>, T>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(S&& src, cmask_t<S, M> cmask, T&& val) {
        return countr_one( __DPL forward<S>(src), cmask, __DPL forward<T>(val));
    }
    ...
    requires extended_vector<T> &&
        unqualified_extended_mcountr_one<dx::zero_t, cmask_t<T, M>, T>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(
        dx::zero_t zero, cmask_t<T, M> cmask, T&& val) {
        return countr_one(zero, cmask, __DPL forward<T>(val));
    }
};
