...
struct countr_one_t :
    public bit_manipulation_base<countr_one_t>,
    public maskable_transform_base<countr_one_t> {
    ...
    static consteval size_t operator()(M mask) noexcept {
        // used adl
        return countr_one(dx::to_const_mask(mask));
    }
};
...
struct fallback_impl<countr_one_t> {
    ...
    static constexpr size_t DPL_VECTORCALL operator()(T val) noexcept {
        return __DPL countr_one(dx::to_bitset(val));
    }
};
...
inline constexpr internal::countr_one_t countr_one{};
}
} // namespace datapar
