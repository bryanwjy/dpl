...
concept unqualified_extended_countl_zero = requires(T val) {
    { countl_zero(val) } -> extended_operation_vector<typename T::abi_type>;
};

template <typename T>
concept expression_countl_zero =
    simd_expression<T> && invocable<countl_zero_t, simd_expression_result_t<T>>;

template <typename T>
concept decayable_countl_zero =
    decayable_vector_for<T, operation_category::lane_agnostic> &&
    regular_invocable<countl_zero_t, canonical_type_t<T>>;

template <typename T>
concept extended_countl_zero = unqualified_extended_countl_zero<T> ||
    expression_countl_zero<T> || decayable_countl_zero<T>;

template <typename S, typename M, typename T, typename A = common_abi_t<M, T>>
concept unqualified_canonical_mcountl_zero = requires(S src, M mask, T val) {
    {
...
template <typename S, typename M, typename T, typename A = common_abi_t<M, T>>
concept unqualified_extended_mcountl_zero = requires(S src, M mask, T val) {
    { countl_zero(src, mask, val) } -> extended_operation_vector<A>;
};

template <typename S, typename M, typename T>
concept expression_mcountl_zero =
    (simd_expression<S> || simd_expression<M> || simd_expression<T>) &&
    invocable<countl_zero_t, expression_result_or_zero_t<S>,
...
concept decayable_mcountl_zero =
    ...
    regular_invocable<countl_zero_t, canonical_or_zero_t<S, T, A>,
        canonical_type_t<M>, canonical_type_t<T>>;

template <typename S, typename M, typename T, typename A = common_abi_t<T, M>>
concept extended_mcountl_zero = unqualified_extended_mcountl_zero<S, M, T, A> ||
    expression_mcountl_zero<S, M, T> || decayable_mcountl_zero<S, M, T, A>;
...
template <typename S, typename M, typename T>
concept expression_imcountl_zero = (simd_expression<S> || simd_expression<T>) &&
    invocable<countl_zero_t, expression_result_or_zero_t<S>, M,
...
concept decayable_imcountl_zero =
    decayable_vector_for<canonical_if_zero_t<S, T>,
        operation_category::lane_agnostic> &&
    decayable_vector_for<T, operation_category::lane_agnostic> &&
    regular_invocable<countl_zero_t, canonical_or_zero_t<S, T, A>, M,
        canonical_type_t<T>>;
...
concept extended_imcountl_zero =
    ...
    expression_imcountl_zero<S, M, T> || decayable_imcountl_zero<S, M, T, A>;
...
template <typename T>
concept expression_mask_countl_zero =
    simd_expression<T> && invocable<countl_zero_t, simd_expression_result_t<T>>;

template <typename T>
concept decayable_mask_countl_zero =
    decayable_mask_for<T, operation_category::lane_reduction> &&
    regular_invocable<countl_zero_t, canonical_type_t<T>>;

template <typename T>
concept extended_mask_countl_zero = unqualified_extended_mask_countl_zero<T> ||
    expression_mask_countl_zero<T> || decayable_mask_countl_zero<T>;
///
...
private:
    ...
    template <typename E, typename A>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr size_t fallback(basic_mask<E, A> val) noexcept {
...
public:
    ...
    static constexpr auto operator()(S src, M mask, T val) noexcept {
        ...
        } else if constexpr (expression_mcountl_zero<S, M, T>) {
            return operator()(
    ...
    static constexpr auto operator()(M mask, T val) noexcept {
        if constexpr (unqualified_extended_mcountl_zero<dx::zero_t, M, T>) {
            return countl_zero(dx::zero, mask, val);
        } else if constexpr (expression_mcountl_zero<dx::zero_t, M, T>) {
    ...
    requires extended_vector_bit<S, T> &&
        ...
        extended_imcountl_zero<countl_zero_t, S, M, T>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(S src, M mask, T val) noexcept {
        constexpr auto cmask = dx::to_compatible_const_mask<S>(mask);
        if constexpr (unqualified_extended_imcountl_zero<S, M, T>) {
            return countl_zero(src, cmask, val);
        } else if constexpr (expression_imcountl_zero<S, M, T>) {
    ...
    static constexpr auto operator()(M mask, T val) noexcept {
        ...
        if constexpr (unqualified_extended_imcountl_zero<zero_t, M, T>) {
            return countl_zero(dx::zero, cmask, val);
        } else if constexpr (expression_imcountl_zero<zero_t, M, T>) {
