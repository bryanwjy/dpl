...
struct countl_zero_t :
    public bit_manipulation_base<countl_zero_t>,
    public maskable_transform_base<countl_zero_t> {
    using operation_base<countl_zero_t>::operator();
    using maskable_transform_base<countl_zero_t>::operator();

    template <const_mask_like M>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, NODISCARD)
    static consteval size_t operator()(M mask) noexcept {
        // used adl
        return countl_zero(dx::to_const_mask(mask));
    }
};

template <>
struct operation_signature<countl_zero_t> {
...
struct fallback_impl<countl_zero_t> {
    ...
    static constexpr size_t DPL_VECTORCALL operator()(T val) noexcept {
        return __DPL countl_zero(dx::to_bitset(val));
    }
};
...
concept unqualified_canonical_mcountl_zero =
    cpo_invocable<countl_zero_t, T> && requires(S src, M mask, T val) {
        {
            countl_zero(internal::abi<T>, src, mask, val)
...
public:
    ...
    template <canonical_vector T>
    requires unqualified_canonical_mcountl_zero<unsigned_canonical_vector_t<T>,
...
concept unqualified_extended_vector_countl_zero = requires {
    {
        countl_zero(internal::declarg<T>())
    } -> equivalent_vector_with<unsigned_canonical_vector_t<T>>;
};

template <typename T>
concept unqualified_extended_mask_countl_zero = requires {
    { countl_zero(internal::declarg<T>()) } -> core_convertible_to<size_t>;
};

template <typename S, typename M, typename T>
concept unqualified_extended_mcountl_zero_base =
    cpo_invocable<countl_zero_t, T> && requires {
        {
            countl_zero(internal::declarg<S>(), internal::declarg<M>(),
                internal::declarg<T>())
        } -> equivalent_vector_with<unsigned_canonical_vector_t<T>>;
    };

template <typename S, typename M, typename T>
concept unqualified_extended_mcountl_zero =
    unqualified_extended_mcountl_zero_base<S, M, T> &&
    equivalent_vector_with<S, unsigned_canonical_vector_t<T>>;

template <typename M, typename T>
concept unqualified_extended_zmcountl_zero =
    unqualified_extended_mcountl_zero_base<dx::zero_t, M, T>;

template <>
struct extended_impl<countl_zero_t> {
public:
    template <extended_vector T>
    requires unqualified_extended_vector_countl_zero<T>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    requires unqualified_extended_mask_countl_zero<T>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    static constexpr auto operator()(S&& src, M&& mask, T&& val) {
        return countl_zero( __DPL forward<S>(src), __DPL forward<M>(mask),
            __DPL forward<T>(val));
    }
    ...
    static constexpr auto operator()(S&& src, M cmask, T&& val) {
        return countl_zero( __DPL forward<S>(src), dx::to_const_mask<S>(cmask),
            __DPL forward<T>(val));
    }

    template <simd_vector T, result_mask_for<countl_zero_t, T> M>
    requires (extended_mask<M> || extended_vector<T>) &&
        unqualified_extended_zmcountl_zero<M, T>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    template <extended_vector T, result_cmask_for<countl_zero_t, T> M>
    requires unqualified_extended_zmcountl_zero<
        launder_cmask_t<cpo_result_t<countl_zero_t, T>, M>, T>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(dx::zero_t zero, M cmask, T&& val) {
        return countl_zero(zero,
            dx::to_const_mask<cpo_result_t<countl_zero_t, T>>(cmask),
            __DPL forward<T>(val));
    }
};
...
inline constexpr internal::countl_zero_t countl_zero{};
}
} // namespace datapar
