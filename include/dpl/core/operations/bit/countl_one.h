...
concept unqualified_canonical_mcountl_one = cpo_invocable<countl_one_t, T> &&
    (!simd_type<S> || same_as<S, cpo_result_t<countl_one_t, T>>) &&
    requires(S src, M mask, T val) {
        {
            countl_one(internal::abi<T>, src, mask, val)
        } -> same_as<cpo_result_t<countl_one_t, T>>;
    };
...
public:
    ...
    template <canonical_vector T>
    requires unqualified_canonical_mcountl_one<count_vector_t<T>,
...
concept unqualified_extended_vector_countl_one = requires {
    { countl_one(internal::declarg<T>()) } -> vector_with_common_abi<A>;
};
...
template <typename S, typename M, typename T>
concept unqualified_extended_mcountl_one = cpo_invocable<countl_one_t, T> &&
    (!simd_type<S> ||
        equivalent_vector_with<S, cpo_result_t<countl_one_t, T>>) &&
    requires {
        {
            countl_one(internal::declarg<S>(), internal::declarg<M>(),
                internal::declarg<T>())
        } -> equivalent_vector_with<
            conditional_t<simd_type<S>, S, cpo_result_t<countl_one_t, T>>>;
    };
...
public:
    ...
    requires (extended_mask<M> || extended_vector<T>) &&
        unqualified_extended_mcountl_one<dx::zero_t, M, T>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    template <simd_vector T, result_cmask_for<countl_one_t, T> M>
    requires extended_vector<T> &&
        unqualified_extended_mcountl_one<dx::zero_t,
