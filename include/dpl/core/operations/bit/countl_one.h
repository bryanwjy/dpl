...
template <>
struct operation_signature<countl_one_t> {
    template <simd_type T>
    static consteval void operator()(T&&) noexcept {}
};

template <>
struct fallback_impl<countl_one_t> {
    ...
    static constexpr size_t DPL_VECTORCALL operator()(T val) noexcept {
        return __DPL countl_one(dx::to_bitset(val));
    }
};

template <typename T>
concept unqualified_canonical_countl_one =
    requires(T val) { countl_one(internal::abi<T>, val); };

template <typename S, typename M, typename T>
concept unqualified_canonical_mcountl_one =
    cpo_invocable<countl_one_t, T> && requires(S src, M mask, T val) {
        {
            countl_one(internal::abi<T>, src, mask, val)
        } -> same_as<unsigned_canonical_vector_t<T>>;
    };

template <>
struct canonical_impl<countl_one_t> {
public:
    template <canonical_vector T>
    requires unqualified_canonical_countl_one<T>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr unsigned_canonical_vector_t<T> operator()(T val) noexcept {
        return countl_one(internal::abi<T>, val);
    }
    ...
    template <canonical_vector T>
    requires unqualified_canonical_mcountl_one<unsigned_canonical_vector_t<T>,
    ...
    static constexpr unsigned_canonical_vector_t<T> operator()(
        ...
        T val) noexcept {
        return countl_one(internal::abi<T>, src, mask, val);
    }
    ...
    template <canonical_vector T>
    requires unqualified_canonical_mcountl_one<dx::zero_t, simd_mask_type_t<T>,
        T>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr unsigned_canonical_vector_t<T> operator()(
        dx::zero_t zero, simd_mask_type_t<T> mask, T val) noexcept {
        return countl_one(internal::abi<T>, zero, mask, val);
    }

    template <canonical_vector T, const_mask_for<T> M>
    requires unqualified_canonical_mcountl_one<dx::zero_t,
        launder_cmask_t<T, M>, T>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr unsigned_canonical_vector_t<T> operator()(
...
concept unqualified_extended_vector_countl_one = requires {
    {
        countl_one(internal::declarg<T>())
    } -> equivalent_vector_with<unsigned_canonical_vector_t<T>>;
};

template <typename T>
concept unqualified_extended_mask_countl_one = requires {
    { countl_one(internal::declarg<T>()) } -> core_convertible_to<size_t>;
};

template <typename S, typename M, typename T>
concept unqualified_extended_mcountl_one_base =
    cpo_invocable<countl_one_t, T> && requires {
        {
            countl_one(internal::declarg<S>(), internal::declarg<M>(),
                internal::declarg<T>())
        } -> equivalent_vector_with<unsigned_canonical_vector_t<T>>;
    };

template <typename S, typename M, typename T>
concept unqualified_extended_mcountl_one =
    unqualified_extended_mcountl_one_base<S, M, T> &&
    equivalent_vector_with<S, unsigned_canonical_vector_t<T>>;

template <typename M, typename T>
concept unqualified_extended_zmcountl_one =
    unqualified_extended_mcountl_one_base<dx::zero_t, M, T>;

template <>
struct extended_impl<countl_one_t> {
public:
    template <extended_vector T>
    requires unqualified_extended_vector_countl_one<T>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(T&& val) {
        return countl_one(__DPL forward<T>(val));
    }

    template <extended_mask T>
    requires unqualified_extended_mask_countl_one<T>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    requires (extended_vector<S> || extended_mask<M> || extended_vector<T>) &&
        unqualified_extended_mcountl_one<S, M, T>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(S&& src, M&& mask, T&& val) {
        return countl_one( __DPL forward<S>(src), __DPL forward<M>(mask),
            __DPL forward<T>(val));
    }

    template <simd_vector S, const_mask_for<S> M, vector_subsumed_by<S> T>
    requires (extended_vector<S> || extended_vector<T>) &&
        unqualified_extended_mcountl_one<S, launder_cmask_t<S, M>, T>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(S&& src, M cmask, T&& val) {
        return countl_one( __DPL forward<S>(src), dx::to_const_mask<S>(cmask),
            __DPL forward<T>(val));
    }

    template <simd_vector T, result_mask_for<countl_one_t, T> M>
    requires (extended_mask<M> || extended_vector<T>) &&
        unqualified_extended_zmcountl_one<M, T>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    template <extended_vector T, result_cmask_for<countl_one_t, T> M>
    requires unqualified_extended_zmcountl_one<
        launder_cmask_t<cpo_result_t<countl_one_t, T>, M>, T>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(dx::zero_t zero, M cmask, T&& val) {
        return countl_one(zero,
            dx::to_const_mask<cpo_result_t<countl_one_t, T>>(cmask),
            __DPL forward<T>(val));
    }
};
...
inline constexpr internal::countl_one_t countl_one{};
}
} // namespace datapar
