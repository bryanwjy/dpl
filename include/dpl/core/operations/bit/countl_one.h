...
concept unqualified_extended_countl_one = requires(T val) {
    { countl_one(val) } -> extended_operation_vector<typename T::abi_type>;
};

template <typename T>
concept expression_countl_one =
    simd_expression<T> && invocable<countl_one_t, simd_expression_result_t<T>>;

template <typename T>
concept decayable_countl_one =
    decayable_vector_for<T, operation_category::lane_agnostic> &&
    regular_invocable<countl_one_t, canonical_type_t<T>>;

template <typename T>
concept extended_countl_one = unqualified_extended_countl_one<T> ||
    expression_countl_one<T> || decayable_countl_one<T>;

template <typename S, typename M, typename T, typename A = common_abi_t<M, T>>
concept unqualified_canonical_mcountl_one = requires(S src, M mask, T val) {
    {
...
template <typename S, typename M, typename T, typename A = common_abi_t<M, T>>
concept unqualified_extended_mcountl_one = requires(S src, M mask, T val) {
    { countl_one(src, mask, val) } -> extended_operation_vector<A>;
};

template <typename S, typename M, typename T>
concept expression_mcountl_one =
    (simd_expression<S> || simd_expression<M> || simd_expression<T>) &&
    invocable<countl_one_t, expression_result_or_zero_t<S>,
...
concept decayable_mcountl_one =
    ...
    regular_invocable<countl_one_t, canonical_or_zero_t<S, T, A>,
        canonical_type_t<M>, canonical_type_t<T>>;

template <typename S, typename M, typename T, typename A = common_abi_t<T, M>>
concept extended_mcountl_one = unqualified_extended_mcountl_one<S, M, T, A> ||
    expression_mcountl_one<S, M, T> || decayable_mcountl_one<S, M, T, A>;

template <typename S, typename M, typename T,
    typename A = common_abi_t<canonical_if_zero_t<S, T>, T>>
concept unqualified_canonical_imcountl_one = requires(S src, M mask, T val) {
    {
        countl_one(internal::abi<A>, src,
            internal::to_const_mask<A, countl_one_t, S, T>(mask), val)
    } -> equivalent_simd_as<canonical_if_zero_t<S, T, A>>;
};

template <typename S, typename M, typename T,
    typename A = common_abi_t<canonical_if_zero_t<S, T>, T>>
concept unqualified_extended_imcountl_one = requires(S src, M mask, T val) {
    {
        countl_one(
            src, internal::to_const_mask<A, countl_one_t, S, T>(mask), val)
    } -> extended_operation_vector<A>;
};

template <typename S, typename M, typename T>
concept expression_imcountl_one = (simd_expression<S> || simd_expression<T>) &&
    invocable<countl_one_t, expression_result_or_zero_t<S>, M,
...
concept decayable_imcountl_one = decayable_vector_for<canonical_if_zero_t<S, T>,
                                     operation_category::lane_agnostic> &&
    decayable_vector_for<T, operation_category::lane_agnostic> &&
    regular_invocable<countl_one_t, canonical_or_zero_t<S, T, A>, M,
        canonical_type_t<T>>;

template <typename S, typename M, typename T,
    typename A = common_abi_t<canonical_if_zero_t<S, T>, T>>
concept extended_imcountl_one = unqualified_extended_imcountl_one<S, M, T, A> ||
    expression_imcountl_one<S, M, T> || decayable_imcountl_one<S, M, T, A>;
...
template <typename T>
concept expression_mask_countl_one =
    simd_expression<T> && invocable<countl_one_t, simd_expression_result_t<T>>;

template <typename T>
concept decayable_mask_countl_one =
    decayable_mask_for<T, operation_category::lane_reduction> &&
    regular_invocable<countl_one_t, canonical_type_t<T>>;

template <typename T>
concept extended_mask_countl_one = unqualified_extended_mask_countl_one<T> ||
    expression_mask_countl_one<T> || decayable_mask_countl_one<T>;
///
...
public:
    ...
    static constexpr basic_vector<unsigned_representation_t<E>, A> operator()(
        ...
        if constexpr (unqualified_canonical_mcountl_one<
            ...
            if consteval {
                return internal::masked<countl_one_t>(src, mask, val);
            } else {
                return countl_one(internal::abi<A>, src, mask, val);
            }
        } else {
            return internal::masked<countl_one_t>(src, mask, val);
        }
    ...
    static constexpr basic_vector<E, MA> operator()(basic_vector<E, MA> src,
        ...
        return countl_one(internal::abi<MA>, src, mask, val);
    }
    ...
    requires extended_vector_bit<S, T> &&
        ...
        maskable_args<S, M, T> && extended_mcountl_one<countl_one_t, S, M, T>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(S src, M mask, T val) noexcept {
        if constexpr (unqualified_extended_mcountl_one<S, M, T>) {
            return countl_one(src, mask, val);
        } else if constexpr (expression_mcountl_one<S, M, T>) {
            return operator()(
    ...
    requires (different_from<MA, TA> || scalable_abi<MA> || scalable_abi<TA>) &&
        ...
        unqualified_canonical_mcountl_one<dx::zero_t, basic_mask<ME, MA>,
            basic_vector<E, TA>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(
        basic_mask<ME, MA> mask, basic_vector<E, TA> val) noexcept {
        return countl_one(internal::abi<MA>, dx::zero, mask, val);
    }

    template <simd_mask M, simd_vector T>
    requires (extended_mask<M> || extended_vector<T>) && zmaskable_args<M, T> &&
        extended_mcountl_one<countl_one_t, zero_t, M, T>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(M mask, T val) noexcept {
        if constexpr (unqualified_extended_mcountl_one<dx::zero_t, M, T>) {
            return countl_one(dx::zero, mask, val);
        } else if constexpr (expression_mcountl_one<dx::zero_t, M, T>) {
    ...
    template <simd_mask M, simd_vector T>
    requires requires(M mask, T val) { countl_one_t::operator()(mask, val); }
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    static constexpr basic_vector<unsigned_representation_t<E>, A> operator()(
        ...
        if constexpr (unqualified_canonical_imcountl_one<
            ...
            if consteval {
                ...
                return countl_one(internal::abi<A>, src,
    ...
    static constexpr basic_vector<unsigned_representation_t<E>, SA> operator()(
        ...
        return countl_one(internal::abi<SA>, src,
    ...
    requires extended_vector_bit<S, T> &&
        ...
        extended_imcountl_one<countl_one_t, S, M, T>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(S src, M mask, T val) noexcept {
        constexpr auto cmask = dx::to_compatible_const_mask<S>(mask);
        if constexpr (unqualified_extended_imcountl_one<S, M, T>) {
            return countl_one(src, cmask, val);
        } else if constexpr (expression_imcountl_one<S, M, T>) {
    ...
    static constexpr basic_vector<unsigned_representation_t<E>, A> operator()(
        ...
        if constexpr (unqualified_canonical_imcountl_one<zero_t, M,
                          basic_vector<E, A>>) {
            if consteval {
                return internal::masked<countl_one_t>(mask, val);
            } else {
                return countl_one(internal::abi<A>, dx::zero,
                    dx::to_compatible_const_mask<basic_vector<E, A>>(mask),
                    val);
            }
        } else {
    ...
    requires imm_zmaskable_args<T> &&
        extended_imcountl_one<countl_one_t, zero_t, M, T>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(M mask, T val) noexcept {
        constexpr auto cmask = dx::to_compatible_const_mask<T>(mask);
        if constexpr (unqualified_extended_imcountl_one<zero_t, M, T>) {
            return countl_one(dx::zero, cmask, val);
        } else if constexpr (expression_imcountl_one<zero_t, M, T>) {
