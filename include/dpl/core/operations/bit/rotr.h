...
struct rotr_t :
    private bit_manipulation_base<rotr_t>,
    private maskable_transform_base<rotr_t> {
    ...
    static constexpr auto operator()(M) noexcept {
        return cmask_v<__DPL rotr(M::value)>;
    }
};
...
struct fallback_impl<rotr_t> {
    ...
    static constexpr T DPL_VECTORCALL operator()(T val, size_t size) noexcept {
        ...
        return dx::initialize<T>(__DPL rotr(dx::pack_mask(val), size));
    }
...
concept unqualified_canonical_mrotr = cpo_invocable<rotr_t, T, N> &&
    (!simd_type<S> || same_as<S, cpo_result_t<rotr_t, T, N>>) && requires {
        {
            rotr(internal::abi<cpo_result_t<rotr_t, T, N>>,
                internal::declarg<S>(), internal::declarg<M>(),
                internal::declarg<T>(), internal::declarg<N>())
        } -> same_as<cpo_result_t<rotr_t, T, N>>;
    };
...
public:
    ...
    template <canonical_vector T, const_mask_for<T> M>
    requires unqualified_canonical_mrotr<dx::zero_t, launder_cmask_t<T, M>, T>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr T operator()(
    ...
    requires canonical_vector<T> &&
        unqualified_canonical_mrotr<dx::zero_t, launder_cmask_t<T, M>, T, N>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr T operator()(
    ...
    requires unqualified_canonical_rotr<L, R, common_abi_t<L, R>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr vresult_t<L, R> operator()(L lhs, R rhs) noexcept {
        return rotr(internal::abi<common_abi_t<L, R>>, lhs, rhs);
    }

    template <canonical_vector L, canonical_vrot_vector_for<L> R>
    requires unqualified_canonical_mrotr<vresult_t<L, R>, vmask_t<L, R>, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr vresult_t<L, R> operator()(
        vresult_t<L, R> src, vmask_t<L, R> mask, L lhs, R rhs) noexcept {
        return rotr(internal::abi<common_abi_t<L, R>>, src, mask, lhs, rhs);
    }

    template <canonical_vector L, canonical_vrot_vector_for<L> R,
        const_mask_for<vresult_t<L, R>> M>
    requires unqualified_canonical_mrotr<vresult_t<L, R>,
        launder_cmask_t<vresult_t<L, R>, M>, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr vresult_t<L, R> operator()(
        vresult_t<L, R> src, M cmask, L lhs, R rhs) noexcept {
        return rotr(internal::abi<common_abi_t<L, R>>, src,
            dx::to_const_mask<vresult_t<L, R>>(cmask), lhs, rhs);
    }

    template <canonical_vector L, canonical_vrot_vector_for<L> R>
    requires unqualified_canonical_mrotr<dx::zero_t, vmask_t<L, R>, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr vresult_t<L, R> operator()(
        dx::zero_t zero, vmask_t<L, R> mask, L lhs, R rhs) noexcept {
        return rotr(internal::abi<common_abi_t<L, R>>, zero, mask, lhs, rhs);
    }

    template <canonical_vector L, canonical_vrot_vector_for<L> R,
        const_mask_for<vresult_t<L, R>> M>
    requires unqualified_canonical_mrotr<dx::zero_t,
        launder_cmask_t<vresult_t<L, R>, M>, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr vresult_t<L, R> operator()(
        dx::zero_t zero, M cmask, L lhs, R rhs) noexcept {
        return rotr(internal::abi<common_abi_t<L, R>>, zero,
            dx::to_const_mask<vresult_t<L, R>>(cmask), lhs, rhs);
    }
};
...
template <typename S, typename M, typename T, typename N = size_t>
concept unqualified_extended_mrotr = cpo_invocable<rotr_t, T, N> &&
    (!simd_type<S> || equivalent_vector_with<S, cpo_result_t<rotr_t, T, N>>) &&
    requires {
        {
            rotr(internal::declarg<S>(), internal::declarg<M>(),
                internal::declarg<T>(), internal::declarg<N>())
        } -> equivalent_vector_with<cpo_result_t<rotr_t, T, N>>;
    };
...
public:
    ...
    requires unqualified_extended_mask_rotr<L>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(L&& lhs, size_t shift) {
        return rotr(__DPL forward<L>(lhs), shift);
    }

    template <extended_mask L, integral_constant_like R>
    requires unqualified_extended_mask_rotr<L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    requires (extended_simd_type<L> || extended_simd_type<R>) &&
        unqualified_extended_rotr<L, R, common_abi_t<L, R>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(L&& lhs, R&& rhs) {
        return rotr(__DPL forward<L>(lhs), __DPL forward<L>(rhs));
    }
    ...
    requires (extended_vector<S> || extended_mask<M> || extended_vector<L> ||
                 extended_vector<R>) &&
        unqualified_extended_mrotr<S, M, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    requires (extended_vector<S> || extended_vector<L> || extended_vector<R>) &&
        unqualified_extended_mrotr<S, launder_cmask_t<S, M>, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(S&& src, M cmask, L&& lhs, R&& rhs) {
        return rotr(src, dx::to_const_mask<S>(cmask), __DPL forward<L>(lhs),
            __DPL forward<R>(rhs));
    }

    template <simd_vector L, vrot_vector_for<L> R,
        result_mask_for<rotr_t, L, R> M>
    requires (extended_mask<M> || extended_vector<L> || extended_vector<R>) &&
        unqualified_extended_mrotr<dx::zero_t, M, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(
        dx::zero_t zero, M&& mask, L&& lhs, R&& rhs) {
        return rotr(zero, __DPL forward<M>(mask), __DPL forward<L>(lhs),
            __DPL forward<R>(rhs));
    }

    template <simd_vector L, vrot_vector_for<L> R,
        result_cmask_for<rotr_t, L, R> M>
    requires (extended_vector<L> || extended_vector<R>) &&
        unqualified_extended_mrotr<dx::zero_t,
            launder_cmask_t<cpo_result_t<rotr_t, L, R>, M>, L, R>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(
        dx::zero_t zero, M cmask, L&& lhs, R&& rhs) {
        return rotr(zero, dx::to_const_mask<cpo_result_t<rotr_t, L, R>>(cmask),
            __DPL forward<L>(lhs), __DPL forward<R>(rhs));
    }
    ///
    ...
    template <simd_vector T, result_mask_for<rotr_t, T, size_t> M>
    requires (extended_mask<M> || extended_vector<T>) &&
        unqualified_extended_mrotr<dx::zero_t, M, T>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(
    ...
    template <simd_vector T, result_cmask_for<rotr_t, T, size_t> M>
    requires extended_vector<T> &&
        unqualified_extended_mrotr<dx::zero_t,
            launder_cmask_t<cpo_result_t<rotr_t, T, size_t>, M>, T>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(
        dx::zero_t zero, M cmask, T&& val, size_t count) {
        return rotr(zero,
            dx::to_const_mask<cpo_result_t<rotr_t, T, size_t>>(cmask),
            __DPL forward<T>(val), count);
    }
    ...
    template <simd_vector T, integral_constant_like N,
        result_mask_for<rotr_t, T, N> M>
    requires (extended_mask<M> || extended_vector<T>) &&
        unqualified_extended_mrotr<dx::zero_t, M, T, N>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(
    ...
    template <extended_vector T, integral_constant_like N,
        result_cmask_for<rotr_t, T, N> M>
    requires unqualified_extended_mrotr<dx::zero_t, launder_cmask_t<T, M>, T, N>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(
        dx::zero_t zero, M cmask, T&& val, N count) {
        return rotr(zero, dx::to_const_mask<cpo_result_t<rotr_t, T, N>>(cmask),
            __DPL forward<T>(val), count);
    }
};
...
DPL_EXPORT inline constexpr internal::rotr_t rotr{};
