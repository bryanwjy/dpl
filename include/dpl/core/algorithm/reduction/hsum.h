...
concept unqualified_canonical_hsum = requires {
    { hsum(internal::abi<T>, internal::declarg<T>()) } -> canonical_vector;
};

template <typename S, typename M, typename T>
concept unqualified_canonical_mhsum =
    (!simd_type<S> || equivalent_vector_with<S, cpo_result_t<hsum_t, T>>) &&
    requires {
        {
            hsum(internal::abi<conditional_t<simd_type<S>, S, T>>,
                internal::declarg<S>(), internal::declarg<M>(),
                internal::declarg<T>())
        } -> equivalent_vector_with<cpo_result_t<hsum_t, T>>;
    };
...
private:
    template <typename T>
    using source_t DPL_NODEBUG = canonical_type_t<cpo_result_t<hsum_t, T>>;
...
public:
    ...
    template <canonical_vector T>
    requires unqualified_canonical_mhsum<source_t<T>, mask_t<T>, T>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr T operator()(
        source_t<T> src, mask_t<T> mask, T val) noexcept {
        return hsum(internal::abi<T>, src, mask, val);
    }

    template <canonical_vector T, imask_t<T> M>
    requires unqualified_canonical_mhsum<source_t<T>, cmask_t<T, M>, T>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr T operator()(
        source_t<T> src, cmask_t<T, M> cmask, T val) noexcept {
        return hsum(internal::abi<T>, src, cmask, val);
    }

    template <canonical_vector T>
    requires unqualified_canonical_mhsum<dx::zero_t, mask_t<T>, T>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr T operator()(
        dx::zero_t zero, mask_t<T> mask, T val) noexcept {
        return hsum(internal::abi<T>, zero, mask, val);
    }

    template <canonical_vector T, imask_t<T> M>
    requires unqualified_canonical_mhsum<dx::zero_t, cmask_t<T, M>, T>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr T operator()(
        dx::zero_t zero, cmask_t<T, M> cmask, T val) noexcept {
        return hsum(internal::abi<T>, zero, cmask, val);
    }
};

template <typename T>
concept unqualified_extended_hsum = requires {
    { hsum(internal::declarg<T>()) } -> equivalent_vector_with<T>;
};

template <typename S, typename M, typename T>
concept unqualified_extended_mhsum =
    (!simd_type<S> || equivalent_vector_with<S, cpo_result_t<hsum_t, T>>) &&
    requires {
        {
            hsum(internal::declarg<S>(), internal::declarg<M>(),
                internal::declarg<T>())
        } -> equivalent_vector_with<cpo_result_t<hsum_t, T>>;
    };
...
private:
    ...
    using mask_t DPL_NODEBUG =
        basic_mask<simd_element_type_t<cpo_result_t<hsum_t, T>>,
            simd_abi_type_t<cpo_result_t<hsum_t, T>>>;

    template <typename T>
    using imask_t DPL_NODEBUG = simask_t<cpo_result_t<hsum_t, T>>;

    template <typename T, imask_t<T> M>
    using cmask_t DPL_NODEBUG = scmask_t<cpo_result_t<hsum_t, T>, M>;
...
public:
    ...
    requires (extended_vector<S> || extended_vector<T> || extended_mask<M>) &&
        unqualified_extended_mhsum<S, M, T>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(S&& src, M&& mask, T&& val) {
        return hsum(__DPL forward<S>(src), __DPL forward<M>(mask),
            __DPL forward<T>(val));
    }
...
public:
    ...
    static constexpr auto operator()(T&& val) noexcept(canonical_vector<T>) {
        return internal::reduction( __DPL forward<T>(val), dx::add);
    }

    template <simd_vector S, simd_mask M, simd_vector T>
    requires cpo_invocable<reduce_t, S, M, T, add_t>
    ...
    requires cpo_invocable<reduce_t, S, scmask_t<S, M>, T, add_t>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, CONST, NODISCARD)
    ...
    requires cpo_invocable<reduce_t, dx::zero_t, M, T, add_t>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, CONST, NODISCARD)
    ...
    requires cpo_invocable<reduce_t, dx::zero_t, scmask_t<T, M>, T, add_t>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, CONST, NODISCARD)
    static constexpr auto DPL_VECTORCALL operator()(dx::zero_t zero,
        scmask_t<T, M> cmask, T&& val) noexcept(canonical_vector<T>) {
        return dx::reduce(zero, cmask, __DPL forward<T>(val), dx::add);
    }
};
