...
struct scan_sum_t : public inclusive_scan_base<scan_sum_t> {
...
concept unqualified_canonical_mscan_sum =
    cpo_invocable<scan_sum_t, T> && requires {
        {
            scan_sum(internal::abi<T>, internal::declarg<T>(),
                internal::declarg<M>())
        } -> same_as<T>;
    };
...
public:
    ...
    template <canonical_vector T>
    requires unqualified_canonical_mscan_sum<T, simd_mask_type_t<T>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
...
concept unqualified_extended_mscan_sum =
    cpo_invocable<scan_sum_t, T> && requires {
        {
            scan_sum(internal::declarg<T>(), internal::declarg<M>())
...
public:
    ...
    static constexpr auto operator()(T&& val, M&& mask) {
        return scan_sum( __DPL forward<T>(val), __DPL forward<M>(mask));
    }
