...
struct exscan_min_t :
    ...
    public maskable_operation_base<exscan_min_t> {
...
concept unqualified_canonical_mexscan_min =
    cpo_invocable<exscan_min_t, T, V> && requires {
        {
            exscan_min(internal::abi<T>, internal::declarg<M>(),
...
public:
    ...
    template <canonical_vector T, broadcastable_to<T> V>
    requires unqualified_canonical_mexscan_min<simd_mask_type_t<T>, T, V>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr T operator()(
        simd_mask_type_t<T> mask, T val, V&& init) noexcept {
        return exscan_min(internal::abi<T>, mask, val, __DPL forward<V>(init));
    }

    template <canonical_vector T, broadcastable_to<T> V, const_mask_for<T> M>
    requires unqualified_canonical_mexscan_min<launder_cmask_t<T, M>, T, V>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr T operator()(M mask, T val, V&& init) noexcept {
        return exscan_min(internal::abi<T>, dx::to_const_mask<T>(mask), val,
...
concept unqualified_extended_mexscan_min =
    cpo_invocable<exscan_min_t, T, V> && requires {
        {
            exscan_min(internal::declarg<M>(), internal::declarg<T>(),
                internal::declarg<V>())
        } -> equivalent_vector_with<T>;
    };
...
public:
    ...
    requires (extended_vector<T> || extended_mask<M>) &&
        unqualified_extended_mexscan_min<M, T, V>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(M&& mask, T&& val, V&& init) {
        return exscan_min( __DPL forward<M>(mask), __DPL forward<T>(val),
            __DPL forward<V>(init));
    }

    template <extended_vector T, broadcastable_to<T> V, const_mask_for<T> M>
    requires unqualified_extended_mexscan_min<launder_cmask_t<T, M>, T, V>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(M mask, T&& val, V&& init) {
        return exscan_min(dx::to_const_mask<T>(mask), __DPL forward<T>(val),
            __DPL forward<V>(init));
    }
};
