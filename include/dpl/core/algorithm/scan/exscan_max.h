...
struct exscan_max_t : public exclusive_scan_base<exscan_max_t> {
...
concept unqualified_canonical_mexscan_max =
    cpo_invocable<exscan_max_t, T, V> && requires {
        {
            exscan_max(internal::abi<T>, internal::declarg<T>(),
...
struct canonical_impl<exscan_max_t> {
    ...
    template <canonical_vector T, broadcastable_to<T> V>
    requires unqualified_canonical_mexscan_max<T, simd_mask_type_t<T>, V>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr T operator()(
        T val, simd_mask_type_t<T> mask, V&& init) noexcept {
        return exscan_max(internal::abi<T>, val, mask, __DPL forward<V>(init));
    }

    template <canonical_vector T, broadcastable_to<T> V, const_mask_for<T> M>
    requires unqualified_canonical_mexscan_max<T, launder_cmask_t<T, M>, V>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr T operator()(T val, M mask, V&& init) noexcept {
        return exscan_max(internal::abi<T>, val, dx::to_const_mask<T>(mask),
...
concept unqualified_extended_mexscan_max =
    cpo_invocable<exscan_max_t, T, V> && requires {
        {
            exscan_max(internal::declarg<T>(), internal::declarg<M>(),
                internal::declarg<V>())
        } -> equivalent_vector_with<T>;
    };
...
struct extended_impl<exscan_max_t> {
    ...
    requires (extended_vector<T> || extended_mask<M>) &&
        unqualified_extended_mexscan_max<T, M, V>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(T&& val, M&& mask, V&& init) {
        return exscan_max( __DPL forward<T>(val), __DPL forward<M>(mask),
            __DPL forward<V>(init));
    }

    template <extended_vector T, const_mask_for<T> M, broadcastable_to<T> V>
    requires unqualified_extended_mexscan_max<T, launder_cmask_t<T, M>, V>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(T&& val, M mask, V&& init) {
        return exscan_max(__DPL forward<T>(val), dx::to_const_mask<T>(mask),
            __DPL forward<V>(init));
    }
};
