...
struct scan_min_t :
    ...
    public maskable_transform_base<scan_min_t> {
...
concept unqualified_canonical_mscan_min = cpo_invocable<scan_min_t, T> &&
    (!simd_type<S> || equivalent_vector_with<S, cpo_result_t<scan_min_t, T>>) &&
    requires {
        {
            scan_min(internal::abi<cpo_result_t<scan_min_t, T>>,
...
private:
    template <typename T>
    using result_t DPL_NODEBUG = canonical_type_t<cpo_result_t<scan_min_t, T>>;
...
public:
    ...
    template <canonical_vector T>
    requires unqualified_canonical_mscan_min<result_t<T>, mask_t<T>, T>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    template <canonical_vector T, result_cmask_for<scan_min_t, T> M>
    requires unqualified_canonical_mscan_min<result_t<T>,
        launder_cmask_t<cpo_result_t<scan_min_t, T>, M>, T>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr T operator()(result_t<T> src, M cmask, T val) noexcept {
        return scan_min(internal::abi<T>, src,
            dx::to_const_mask<cpo_result_t<scan_min_t, T>>(cmask), val);
    }
    ...
    template <canonical_vector T, result_cmask_for<scan_min_t, T> M>
    requires unqualified_canonical_mscan_min<dx::zero_t,
        launder_cmask_t<cpo_result_t<scan_min_t, T>, M>, T>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr T operator()(dx::zero_t zero, M cmask, T val) noexcept {
        return scan_min(internal::abi<T>, zero,
            dx::to_const_mask<cpo_result_t<scan_min_t, T>>(cmask), val);
    }
};
...
template <typename S, typename M, typename T>
concept unqualified_extended_mscan_min = cpo_invocable<scan_min_t, T> &&
    (!simd_type<S> || equivalent_vector_with<S, cpo_result_t<scan_min_t, T>>) &&
    requires {
        {
...
public:
    ...
    requires (extended_vector<S> || extended_vector<T>) &&
        unqualified_extended_mscan_min<S, launder_cmask_t<S, M>, T>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(S&& src, M cmask, T&& val) {
        return scan_min(__DPL forward<S>(src), dx::to_const_mask<S>(cmask),
            __DPL forward<T>(val));
    }

    template <simd_vector T, result_mask_for<scan_min_t, T> M>
    ...
    template <fixed_width_vector T, result_cmask_for<scan_min_t, T> M>
    requires extended_vector<T> &&
        unqualified_extended_mscan_min<dx::zero_t,
            launder_cmask_t<cpo_result_t<scan_min_t, T>, M>, T>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(dx::zero_t zero, M cmask, T&& val) {
        return scan_min(zero,
            dx::to_const_mask<cpo_result_t<scan_min_t, T>>(cmask),
            __DPL forward<T>(val));
    }
};
...
public:
    ...
    requires cpo_invocable<scan_t, S, launder_cmask_t<S, M>, T, min_t>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, CONST, NODISCARD)
    ...
    template <simd_vector T, result_cmask_for<scan_t, T, min_t> M>
    requires cpo_invocable<scan_t, dx::zero_t, M, T, min_t>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, CONST, NODISCARD)
    static constexpr auto DPL_VECTORCALL operator()(
