...
DPL_EXPORT template <simd_element E1, simd_abi A1, simd_element E2, simd_abi A2>
requires requires { typename common_order_type_t<E1, E2>; }
struct common_order_simd<simd_mask<E1, A1>, simd_mask<E2, A2>> {
    using type DPL_NODEBUG =
        simd_mask<common_order_type_t<E1, E2>, common_abi_t<A1, A2>>;
};
...
struct common_order_simd<T1, T2> {
    using type DPL_NODEBUG =
        simd_mask<common_order_type_t<T1, T2>, common_abi_t<T1, T2>>;
};
} // namespace datapar
