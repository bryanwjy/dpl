...
concept unqualified_canonical_misnans = canonical_mask<S> &&
...
public:
    ...
    requires canonical_vector<T> &&
        unqualified_canonical_misnans<cmask_t<T, M>, T>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr mask_t<T> operator()(cmask_t<T, M> cmask, T val) noexcept {
        return isnans(internal::abi<T>, cmask, val);
    }
};
...
template <typename S, typename T>
concept unqualified_extended_misnans =
...
public:
    ...
    requires extended_vector<T> &&
        unqualified_extended_misnans<cmask_t<T, M>, T>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(cmask_t<T, M> cmask, T&& arg) {
        return isnans(cmask, __DPL forward<T>(arg));
    }
};
