...
struct DPL_EMPTY_BASES fmaddsub_t :
    public math_operation_base<fmaddsub_t>,
...
template <>
struct operation_signature<fmaddsub_t> {
...
template <>
struct fallback_impl<fmaddsub_t> : fm_canonical_broadcaster<fmaddsub_t> {
...
concept unqualified_canonical_mfmaddsub =
    cpo_invocable<fmaddsub_t, AT, BT, CT> && requires {
        {
            fmaddsub(internal::abi<cpo_result_t<fmaddsub_t, AT, BT, CT>>,
                internal::declarg<AT>(), internal::declarg<M>(),
                internal::declarg<BT>(), internal::declarg<CT>())
        } -> same_as<cpo_result_t<fmaddsub_t, AT, BT, CT>>;
    };
...
private:
    template <typename AT, typename BT, typename CT>
    using result_t DPL_NODEBUG = cpo_result_t<fmaddsub_t, AT, BT, CT>;
...
public:
    ...
    requires canonical_vector<BT> && canonical_vector<CT> &&
        unqualified_canonical_fmaddsub<AT, BT, CT, AT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr AT operator()(AT aval, BT bval, CT cval) noexcept {
        return fmaddsub(internal::abi<AT>, aval, bval, cval);
    }
    ...
    requires canonical_vector<CT> &&
        unqualified_canonical_fmaddsub<AT, BT, CT, vector_t<BT, CT>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr vector_t<BT, CT> operator()(
        AT&& aval, BT bval, CT cval) noexcept {
        return fmaddsub(internal::abi<common_abi_t<BT, CT>>,
            __DPL forward<AT>(aval), bval, cval);
    }

    template <canonical_vector AT, broadcastable_to<AT> BT,
        broadcastable_to<AT> CT>
    requires unqualified_canonical_fmaddsub<AT, BT, CT, AT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr AT operator()(AT aval, BT&& bval, CT&& cval) noexcept {
        return fmaddsub(internal::abi<AT>, aval, __DPL forward<BT>(bval),
            __DPL forward<CT>(cval));
    }

    template <canonical_vector BT, broadcastable_to<BT> AT,
        broadcastable_to<BT> CT>
    requires unqualified_canonical_fmaddsub<AT, BT, CT, BT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr BT operator()(AT&& aval, BT bval, CT&& cval) noexcept {
        return fmaddsub(internal::abi<BT>, __DPL forward<AT>(aval), bval,
            __DPL forward<CT>(cval));
    }

    template <unextended_type AT, unextended_type BT, unextended_type CT>
    requires unqualified_canonical_mfmaddsub<AT, mask_t<AT, BT, CT>, BT, CT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    template <unextended_type AT, unextended_type BT, unextended_type CT,
        result_cmask_for<fmaddsub_t, AT, BT, CT> M>
    requires unqualified_canonical_mfmaddsub<AT,
        launder_cmask_t<result_t<AT, BT, CT>, M>, BT, CT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr result_t<AT, BT, CT> operator()(
        AT aval, M mask, BT bval, CT cval) noexcept {
        return fmaddsub(internal::abi<result_t<AT, BT, CT>>, aval,
            dx::to_const_mask<result_t<AT, BT, CT>>(mask), bval, cval);
    }

    template <unextended_type AT, unextended_type BT, unextended_type CT>
    requires unqualified_canonical_zmfmaddsub<mask_t<AT, BT, CT>, AT, BT, CT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    template <unextended_type AT, unextended_type BT, unextended_type CT,
        result_cmask_for<fmaddsub_t, AT, BT, CT> M>
    requires unqualified_canonical_zmfmaddsub<
        launder_cmask_t<result_t<AT, BT, CT>, M>, AT, BT, CT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr result_t<AT, BT, CT> operator()(
        dx::zero_t zero, M mask, AT aval, BT bval, CT cval) noexcept {
        return fmaddsub(internal::abi<result_t<AT, BT, CT>>, zero,
            dx::to_const_mask<result_t<AT, BT, CT>>(mask), aval, bval, cval);
    }
};

template <typename AT, typename BT, typename CT, typename T>
concept unqualified_extended_fmaddsub = requires {
    {
        fmaddsub(internal::declarg<AT>(), internal::declarg<BT>(),
            internal::declarg<CT>())
...
public:
    ...
    requires (extended_vector<AT> || extended_vector<BT> ||
                 extended_vector<CT>) &&
        unqualified_extended_fmaddsub<AT, BT, CT, AT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(AT&& aval, BT&& bval, CT&& cval) {
        return fmaddsub(__DPL forward<AT>(aval), __DPL forward<BT>(bval),
            __DPL forward<CT>(cval));
    }
    ...
    requires (extended_vector<BT> || extended_vector<CT>) &&
        unqualified_extended_fmaddsub<AT, BT, CT, vector_t<BT, CT>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(AT&& aval, BT&& bval, CT&& cval) {
        return fmaddsub(__DPL forward<AT>(aval), __DPL forward<BT>(bval),
            __DPL forward<CT>(cval));
    ...
    template <extended_vector AT, broadcastable_to<AT> BT,
        broadcastable_to<AT> CT>
    requires unqualified_extended_fmaddsub<AT, BT, CT, AT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(AT&& aval, BT&& bval, CT&& cval) {
        return fmaddsub(__DPL forward<AT>(aval), __DPL forward<BT>(bval),
            __DPL forward<CT>(cval));
    ...
    template <extended_vector BT, broadcastable_to<BT> AT,
        broadcastable_to<BT> CT>
    requires unqualified_extended_fmaddsub<AT, BT, CT, BT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(AT&& aval, BT&& bval, CT&& cval) {
        return fmaddsub(__DPL forward<AT>(aval), __DPL forward<BT>(bval),
            __DPL forward<CT>(cval));
    ...
    requires unqualified_extended_fmaddsub<AT, BT, CT, CT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(AT&& aval, BT&& bval, CT&& cval) {
        return fmaddsub(__DPL forward<AT>(aval), __DPL forward<BT>(bval),
            __DPL forward<CT>(cval));
    ...
    template <typename AT, typename BT, typename CT,
        result_mask_for<fmaddsub_t, AT, BT, CT> M>
    ...
    requires (extended_vector<AT> || extended_vector<BT> ||
                 extended_vector<CT>) &&
        unqualified_extended_mfmaddsub<AT,
            launder_cmask_t<cpo_result_t<fmaddsub_t, AT, BT, CT>, M>, BT, CT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(AT&& aval, M mask, BT&& bval, CT&& cval) {
        return fmaddsub(__DPL forward<AT>(aval),
            dx::to_const_mask<cpo_result_t<fmaddsub_t, AT, BT, CT>>(mask),
            __DPL forward<BT>(bval), __DPL forward<CT>(cval));
    }
    ...
    static constexpr auto operator()(
        ...
        return fmaddsub(zero, __DPL forward<M>(mask), __DPL forward<AT>(aval),
            __DPL forward<BT>(bval), __DPL forward<CT>(cval));
    }
    ...
    requires (extended_vector<AT> || extended_vector<BT> ||
                 extended_vector<CT>) &&
        unqualified_extended_zmfmaddsub<
