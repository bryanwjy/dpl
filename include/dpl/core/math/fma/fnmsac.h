...
struct DPL_EMPTY_BASES fnmsac_t :
    public math_operation_base<fnmsac_t>,
    public maskable_accumulation_base<fnmsac_t> {
    using operation_base<fnmsac_t>::operator();
    using maskable_accumulation_base<fnmsac_t>::operator();
};

template <>
struct operation_signature<fnmsac_t> {
...
template <>
struct fallback_impl<fnmsac_t> : fm_canonical_broadcaster<fnmsac_t> {
    ...
    requires floating_point_like<simd_element_type_t<CT>> &&
        ...
        cpo_invocable<fmsac_t, CT, AT, BT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, CONST, NODISCARD)
    static constexpr auto DPL_VECTORCALL operator()(
        CT cval, AT aval, BT bval) noexcept {
        return dx::negate(dx::fmsac(cval, aval, bval));
    }

    using fm_canonical_broadcaster<fnmsac_t>::operator();
};

template <typename AT, typename BT, typename CT, typename T>
concept unqualified_canonical_fnmsac =
    unqualified_canonical_fnmsub<BT, CT, AT, T>;

template <typename AT, typename M, typename BT, typename CT>
concept unqualified_canonical_mfnmsac =
    cpo_invocable<fnmsac_t, AT, BT, CT> && requires {
        {
            fnmsub(internal::abi<cpo_result_t<fnmsac_t, AT, BT, CT>>,
                internal::declarg<BT>(), internal::declarg<CT>(),
                internal::declarg<AT>(), internal::declarg<M>())
        } -> same_as<cpo_result_t<fnmsac_t, AT, BT, CT>>;
    };

template <typename M, typename AT, typename BT, typename CT>
concept unqualified_canonical_zmfnmsac =
    cpo_invocable<fnmsac_t, AT, BT, CT> && requires {
        {
            fnmsub(internal::abi<cpo_result_t<fnmsac_t, AT, BT, CT>>, dx::zero,
...
template <>
struct canonical_impl<fnmsac_t> {
private:
    template <typename AT, typename BT, typename CT>
    using result_t DPL_NODEBUG = cpo_result_t<fnmsac_t, AT, BT, CT>;
...
public:
    ...
    requires canonical_vector<BT> && canonical_vector<CT> &&
        unqualified_canonical_fnmsac<AT, BT, CT, AT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr AT operator()(AT aval, BT bval, CT cval) noexcept {
        return fnmsub(internal::abi<AT>, bval, cval, aval);
    }
    ...
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr AT operator()(AT aval, BT bval, CT&& cval) noexcept {
        return fnmsub(internal::abi<AT>, bval, __DPL forward<CT>(cval), aval);
    }
    ...
    template <unextended_type AT, unextended_type BT, unextended_type CT,
        result_cmask_for<fnmsac_t, AT, BT, CT> M>
    requires unqualified_canonical_mfnmsac<AT,
        launder_cmask_t<result_t<AT, BT, CT>, M>, BT, CT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr result_t<AT, BT, CT> operator()(
        AT aval, M mask, BT bval, CT cval) noexcept {
        return fnmsub(internal::abi<result_t<AT, BT, CT>>, bval, cval, aval,
            dx::to_const_mask<result_t<AT, BT, CT>>(mask));
    }

    template <unextended_type AT, unextended_type BT, unextended_type CT>
    requires unqualified_canonical_zmfnmsac<mask_t<AT, BT, CT>, AT, BT, CT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    template <unextended_type AT, unextended_type BT, unextended_type CT,
        result_cmask_for<fnmsac_t, AT, BT, CT> M>
    requires unqualified_canonical_zmfnmsac<
        launder_cmask_t<result_t<AT, BT, CT>, M>, AT, BT, CT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr result_t<AT, BT, CT> operator()(
        dx::zero_t zero, M mask, AT aval, BT bval, CT cval) noexcept {
        return fnmsub(internal::abi<result_t<AT, BT, CT>>, zero,
            dx::to_const_mask<result_t<AT, BT, CT>>(mask), bval, cval, aval);
    }
};

template <typename AT, typename BT, typename CT, typename T>
concept unqualified_extended_fnmsac =
    unqualified_extended_fnmsub<BT, CT, AT, T>;

template <typename AT, typename M, typename BT, typename CT>
concept unqualified_extended_mfnmsac =
    cpo_invocable<fnmsac_t, AT, BT, CT> && requires {
        {
            fnmsub(internal::declarg<BT>(), internal::declarg<CT>(),
                internal::declarg<AT>(), internal::declarg<M>())
        } -> equivalent_vector_with<cpo_result_t<fnmsac_t, AT, BT, CT>>;
    };

template <typename M, typename AT, typename BT, typename CT>
concept unqualified_extended_zmfnmsac =
    cpo_invocable<fnmsac_t, AT, BT, CT> && requires {
        {
            fnmsub(dx::zero, internal::declarg<M>(), internal::declarg<BT>(),
                internal::declarg<CT>(), internal::declarg<AT>())
        } -> equivalent_vector_with<cpo_result_t<fnmsac_t, AT, BT, CT>>;
    };

template <>
struct extended_impl<fnmsac_t> {
private:
    template <typename L, typename R>
    using vector_t DPL_NODEBUG = common_canonical_simd_t<L, R>;
...
public:
    ...
    requires (extended_vector<AT> || extended_vector<BT> ||
                 extended_vector<CT>) &&
        unqualified_extended_fnmsac<AT, BT, CT, AT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(AT&& aval, BT&& bval, CT&& cval) {
        return fnmsub(__DPL forward<BT>(bval), __DPL forward<CT>(cval),
            __DPL forward<AT>(aval));
    }
    ...
    requires (extended_vector<BT> || extended_vector<CT>) &&
        unqualified_extended_fnmsac<AT, BT, CT, vector_t<BT, CT>>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(AT&& aval, BT&& bval, CT&& cval) {
        return fnmsub(__DPL forward<BT>(bval), __DPL forward<CT>(cval),
            __DPL forward<AT>(aval));
    ...
    template <extended_vector AT, broadcastable_to<AT> BT,
        broadcastable_to<AT> CT>
    requires unqualified_extended_fnmsac<AT, BT, CT, AT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(AT&& aval, BT&& bval, CT&& cval) {
        return fnmsub(__DPL forward<BT>(bval), __DPL forward<CT>(cval),
            __DPL forward<AT>(aval));
    ...
    template <extended_vector BT, broadcastable_to<BT> AT,
        broadcastable_to<BT> CT>
    requires unqualified_extended_fnmsac<AT, BT, CT, BT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(AT&& aval, BT&& bval, CT&& cval) {
        return fnmsub(__DPL forward<BT>(bval), __DPL forward<CT>(cval),
            __DPL forward<AT>(aval));
    ...
    template <extended_vector CT, broadcastable_to<CT> AT,
        broadcastable_to<CT> BT>
    requires unqualified_extended_fnmsac<AT, BT, CT, CT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(AT&& aval, BT&& bval, CT&& cval) {
        return fnmsub(__DPL forward<BT>(bval), __DPL forward<CT>(cval),
            __DPL forward<AT>(aval));
    ...
    template <typename AT, typename BT, typename CT,
        result_mask_for<fnmsac_t, AT, BT, CT> M>
    requires (extended_mask<M> || extended_vector<AT> || extended_vector<BT> ||
                 extended_vector<CT>) &&
        unqualified_extended_mfnmsac<AT, M, BT, CT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    requires (extended_vector<AT> || extended_vector<BT> ||
                 extended_vector<CT>) &&
        unqualified_extended_mfnmsac<AT,
            launder_cmask_t<cpo_result_t<fnmsac_t, AT, BT, CT>, M>, BT, CT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(AT&& aval, M mask, BT&& bval, CT&& cval) {
        return fnmsub( __DPL forward<BT>(bval), __DPL forward<CT>(cval),
            __DPL forward<AT>(aval),
            dx::to_const_mask<cpo_result_t<fnmsac_t, AT, BT, CT>>(mask));
    }
    ...
    requires (extended_mask<M> || extended_vector<AT> || extended_vector<BT> ||
                 extended_vector<CT>) &&
        unqualified_extended_zmfnmsac<M, AT, BT, CT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(
        dx::zero_t zero, M&& mask, AT&& aval, BT&& bval, CT&& cval) {
        return fnmsub(zero, __DPL forward<M>(mask), __DPL forward<BT>(bval),
    ...
    requires (extended_vector<AT> || extended_vector<BT> ||
                 extended_vector<CT>) &&
        unqualified_extended_zmfnmsac<
            launder_cmask_t<cpo_result_t<fnmsac_t, AT, BT, CT>, M>, AT, BT, CT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(
        dx::zero_t zero, M mask, AT&& aval, BT&& bval, CT&& cval) {
        return fnmsub(zero,
...
inline constexpr internal::fnmsac_t fnmsac{};
} // namespace cpo
} // namespace datapar
