...
template <>
struct fallback_impl<mulsac_t> : fm_canonical_broadcaster<mulsac_t> {
    ...
    requires cpo_invocable<mulsac_t, AT, BT, CT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr cpo_result_t<mulsac_t, AT, BT, CT> operator()(AT a,
        simd_mask_type_t<cpo_result_t<mulsac_t, AT, BT, CT>> mask, BT b,
        CT c) noexcept {
        if constexpr (dx::is_simd_invocable<AT, BT, CT>(dx::fmsac)) {
    ...
    template <typename AT, typename BT, typename CT,
        result_mask_for<mulsac_t, AT, BT, CT> M>
    ...
    template <typename AT, typename BT, typename CT,
        result_cmask_for<mulsac_t, AT, BT, CT> M>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    requires cpo_invocable<mulsac_t, AT, BT, CT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr cpo_result_t<mulsac_t, AT, BT, CT> operator()(
        dx::zero_t zero,
        simd_mask_type_t<cpo_result_t<mulsac_t, AT, BT, CT>> mask, AT a, BT b,
        CT c) noexcept {
        if constexpr (dx::is_simd_invocable<AT, BT, CT>(dx::fmsac)) {
    ...
    using fm_canonical_broadcaster<mulsac_t>::operator();
};
