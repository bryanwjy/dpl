...
struct DPL_EMPTY_BASES mulsub_t :
    public math_operation_base<mulsub_t>,
...
template <>
struct fallback_impl<mulsub_t> : fm_canonical_broadcaster<mulsub_t> {
    ...
    static constexpr auto operator()(AT&& a, BT&& b, CT&& c) noexcept(
        ...
        if constexpr (dx::is_simd_invocable<AT, BT, CT>(dx::fmsub)) {
            return dx::fmsub(__DPL forward<AT>(a), __DPL forward<BT>(b),
                __DPL forward<CT>(c));
        } else {
            return dx::subtract(
    ...
    requires cpo_invocable<mulsub_t, AT, BT, CT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr cpo_result_t<mulsub_t, AT, BT, CT> operator()(AT a,
        simd_mask_type_t<cpo_result_t<mulsub_t, AT, BT, CT>> mask, BT b,
        CT c) noexcept {
        if constexpr (dx::is_simd_invocable<AT, BT, CT>(dx::fmsub)) {
    ...
    template <typename AT, typename BT, typename CT,
        result_cmask_for<mulsub_t, AT, BT, CT> M>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(AT&& a, M mask, BT&& b, CT&& c) noexcept(
        canonical_vector<AT> && canonical_vector<BT> && canonical_vector<CT>) {
        if constexpr (dx::is_simd_invocable<AT, BT, CT>(dx::fmsub)) {
            return dx::fmsub( __DPL forward<AT>(a), mask, __DPL forward<BT>(b),
                __DPL forward<CT>(c));
        } else {
            return dx::subtract(a, mask, dx::multiply(a, __DPL forward<BT>(b)),
                __DPL forward<CT>(c));
        }
    }
    ...
    requires cpo_invocable<mulsub_t, AT, BT, CT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr cpo_result_t<mulsub_t, AT, BT, CT> operator()(
        dx::zero_t zero,
        simd_mask_type_t<cpo_result_t<mulsub_t, AT, BT, CT>> mask, AT a, BT b,
        CT c) noexcept {
        if constexpr (dx::is_simd_invocable<AT, BT, CT>(dx::fmsub)) {
    ...
    static constexpr auto operator()(
        ...
        if constexpr (dx::is_simd_invocable<AT, BT, CT>(dx::fmsub)) {
            return dx::fmsub(zero, __DPL forward<M>(mask), __DPL forward<AT>(a),
                __DPL forward<BT>(b), __DPL forward<CT>(c));
        } else {
    ...
    static constexpr auto operator()(dx::zero_t zero, M mask, AT&& a, BT&& b,
        ...
        canonical_vector<CT>) {
        if constexpr (dx::is_simd_invocable<AT, BT, CT>(dx::fmsub)) {
            return dx::fmsub(zero, mask, __DPL forward<AT>(a),
                __DPL forward<BT>(b), __DPL forward<CT>(c));
        } else {
    ...
    using fm_canonical_broadcaster<mulsub_t>::operator();
};
} // namespace datapar::internal
