...
#include "dpl/core/math/fma/fmaddsac.h"
#if !DPL_MODULES
...
struct DPL_EMPTY_BASES muladdsac_t :
    public math_operation_base<muladdsac_t>,
...
template <>
struct fallback_impl<muladdsac_t> : fm_canonical_broadcaster<muladdsac_t> {
    ...
    static constexpr auto operator()(AT&& a, BT&& b, CT&& c) noexcept(
        ...
        if constexpr (dx::is_simd_invocable<AT, BT, CT>(dx::fmaddsac)) {
            return dx::fmaddsac(__DPL forward<AT>(a), __DPL forward<BT>(b),
                __DPL forward<CT>(c));
        } else {
            return dx::addsub(
    ...
    requires cpo_invocable<muladdsac_t, AT, BT, CT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr cpo_result_t<muladdsac_t, AT, BT, CT> operator()(AT a,
        simd_mask_type_t<cpo_result_t<muladdsac_t, AT, BT, CT>> mask, BT b,
        CT c) noexcept {
        if constexpr (dx::is_simd_invocable<AT, BT, CT>(dx::fmaddsac)) {
    ...
    template <typename AT, typename BT, typename CT,
        result_cmask_for<muladdsac_t, AT, BT, CT> M>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(AT&& a, M mask, BT&& b, CT&& c) noexcept(
        canonical_vector<AT> && canonical_vector<BT> && canonical_vector<CT>) {
        if constexpr (dx::is_simd_invocable<AT, BT, CT>(dx::fmaddsac)) {
            return dx::fmaddsac( __DPL forward<AT>(a), mask,
                __DPL forward<BT>(b), __DPL forward<CT>(c));
        } else {
            return dx::addsub(a, mask,
                dx::multiply(__DPL forward<BT>(b), __DPL forward<CT>(c)), a);
        }
    }
    ...
    requires cpo_invocable<muladdsac_t, AT, BT, CT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr cpo_result_t<muladdsac_t, AT, BT, CT> operator()(
        dx::zero_t zero,
        simd_mask_type_t<cpo_result_t<muladdsac_t, AT, BT, CT>> mask, AT a,
        BT b, CT c) noexcept {
        if constexpr (dx::is_simd_invocable<AT, BT, CT>(dx::fmaddsac)) {
    ...
    static constexpr auto operator()(
        ...
        if constexpr (dx::is_simd_invocable<AT, BT, CT>(dx::fmaddsac)) {
            return dx::fmaddsac(zero, __DPL forward<M>(mask),
    ...
    static constexpr auto operator()(dx::zero_t zero, M mask, AT&& a, BT&& b,
        ...
        canonical_vector<CT>) {
        if constexpr (dx::is_simd_invocable<AT, BT, CT>(dx::fmaddsac)) {
            return dx::fmaddsac(zero, mask, __DPL forward<AT>(a),
                __DPL forward<BT>(b), __DPL forward<CT>(c));
        } else {
    ...
    using fm_canonical_broadcaster<muladdsac_t>::operator();
};
