...
template <>
struct fallback_impl<nmulsub_t> : fm_canonical_broadcaster<nmulsub_t> {
    ...
    static constexpr auto operator()(AT&& a, BT&& b, CT&& c) noexcept(
        ...
        if constexpr (dx::is_simd_invocable<AT, BT, CT>(dx::fnmsub)) {
            return dx::fnmsub(__DPL forward<AT>(a), __DPL forward<BT>(b),
                __DPL forward<CT>(c));
        } else {
            return dx::subtract(__DPL forward<CT>(c),
                dx::multiply(__DPL forward<AT>(a), __DPL forward<BT>(b)));
        }
    }

    template <unextended_type AT, unextended_type BT, unextended_type CT>
    requires cpo_invocable<nmulsub_t, AT, BT, CT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr cpo_result_t<nmulsub_t, AT, BT, CT> operator()(AT a,
        simd_mask_type_t<cpo_result_t<nmulsub_t, AT, BT, CT>> mask, BT b,
        CT c) noexcept {
        if constexpr (dx::is_simd_invocable<AT, BT, CT>(dx::fnmsub)) {
    ...
    template <typename AT, typename BT, typename CT,
        result_mask_for<nmulsub_t, AT, BT, CT> M>
    requires (extended_mask<M> || extended_vector<AT> || extended_vector<BT> ||
        extended_vector<CT>)
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(AT&& a, M&& mask, BT&& b, CT&& c) {
        if constexpr (dx::is_simd_invocable<AT, BT, CT>(dx::fnmsub)) {
            return dx::fnmsub( __DPL forward<AT>(a), __DPL forward<M>(mask),
                __DPL forward<BT>(b), __DPL forward<CT>(c));
        } else {
            return dx::subtract(a, __DPL forward<M>(mask),
                __DPL forward<CT>(c), dx::multiply(a, __DPL forward<BT>(b)));
        }
    }

    template <typename AT, typename BT, typename CT,
        result_cmask_for<nmulsub_t, AT, BT, CT> M>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(AT&& a, M mask, BT&& b, CT&& c) noexcept(
        canonical_vector<AT> && canonical_vector<BT> && canonical_vector<CT>) {
        if constexpr (dx::is_simd_invocable<AT, BT, CT>(dx::fnmsub)) {
            return dx::fnmsub( __DPL forward<AT>(a), mask, __DPL forward<BT>(b),
                __DPL forward<CT>(c));
        } else {
            return dx::subtract(a, mask, __DPL forward<CT>(c),
    ...
    requires cpo_invocable<nmulsub_t, AT, BT, CT>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr cpo_result_t<nmulsub_t, AT, BT, CT> operator()(
        dx::zero_t zero,
        simd_mask_type_t<cpo_result_t<nmulsub_t, AT, BT, CT>> mask, AT a, BT b,
        CT c) noexcept {
        if constexpr (dx::is_simd_invocable<AT, BT, CT>(dx::fnmsub)) {
    ...
    static constexpr auto operator()(
        ...
        if constexpr (dx::is_simd_invocable<AT, BT, CT>(dx::fnmsub)) {
            return dx::fnmsub(zero, __DPL forward<M>(mask),
    ...
    static constexpr auto operator()(dx::zero_t zero, M mask, AT&& a, BT&& b,
        ...
        canonical_vector<CT>) {
        if constexpr (dx::is_simd_invocable<AT, BT, CT>(dx::fnmsub)) {
            return dx::fnmsub(zero, mask, __DPL forward<AT>(a),
                __DPL forward<BT>(b), __DPL forward<CT>(c));
        } else {
            return dx::subtract(zero, mask, __DPL forward<CT>(c),
                dx::multiply(__DPL forward<AT>(a), __DPL forward<BT>(b)));
        }
    }

    using fm_canonical_broadcaster<nmulsub_t>::operator();
};
