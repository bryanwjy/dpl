...
public:
    ...
    static constexpr auto operator()(AT start, BT end, CT scale) noexcept {
    ...
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(source_t<AT, BT, CT> src,
        cmask_t<AT, BT, CT, M> cmask, AT start, BT end, CT scale) noexcept {
        return lerp(internal::abi<A>, src, cmask, start, end, scale);
    }
    ...
    static constexpr auto operator()(dx::zero_t zero,
        cmask_t<AT, BT, CT, M> cmask, AT start, BT end, CT scale) noexcept {
        return lerp(internal::abi<A>, zero, cmask, start, end, scale);
    }
};
...
public:
    ...
    requires (extended_vector<S> || extended_mask<M> || extended_vector<AT> ||
        ...
        unqualified_extended_mlerp<S, M, AT, BT, CT, A>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    ...
    requires (extended_vector<S> || extended_vector<AT> ||
                 extended_vector<BT> || extended_vector<CT>) &&
        unqualified_extended_mlerp<S, scmask_t<S, M>, AT, BT, CT, A>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(
        S&& src, scmask_t<S, M> cmask, AT&& start, BT&& end, CT&& scale) {
        return lerp(__DPL forward<S>(src), cmask, __DPL forward<AT>(start),
    ...
    requires (extended_mask<M> || extended_vector<AT> || extended_vector<BT> ||
                 extended_vector<CT>) &&
        unqualified_extended_mlerp<dx::zero_t, M, AT, BT, CT, A>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(
    ...
    requires (extended_vector<AT> || extended_vector<BT> ||
                 extended_vector<CT>) &&
        unqualified_extended_mlerp<dx::zero_t, cmask_t<AT, BT, CT, M>, AT, BT,
            CT, A>
    DPL_ATTRIBUTES(_HIDE_FROM_ABI, ALWAYS_INLINE, NODISCARD)
    static constexpr auto operator()(dx::zero_t zero,
        cmask_t<AT, BT, CT, M> cmask, AT&& start, BT&& end, CT&& scale) {
        return lerp(zero, cmask, __DPL forward<AT>(start),
            __DPL forward<BT>(end), __DPL forward<CT>(scale));
    }
};
} // namespace datapar::internal
