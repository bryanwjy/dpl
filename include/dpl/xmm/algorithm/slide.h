...
inline vector<E>
    ...
    if constexpr (!integral<E>) {
        ...
        vector<int8> const idx = [count]() {
            constexpr auto size = xmm::vector_size_v<E>;
...
inline vector<E>
    ...
    if constexpr (N == 0) {
        return lhs;
    } else if constexpr (N >= xmm::vector_size_v<E>) {
        return rhs;
...
#  else
        constexpr auto size = static_cast<int>(xmm::vector_size_v<E>);
        return _mm_or_si128(
...
inline vector<E>
    ...
    if constexpr (N::value == 0) {
        return xmm::select(mask, lhs, src);
    } else if constexpr (N::value >= xmm::vector_size_v<E> || N::value < 0) {
        return xmm::select(mask, rhs, src);
    } else if constexpr (!integral<E>) {
        using int_t = signed_representation_t<E>;
        ...
        return _mm_mask_alignr_epi32(
            +src, M, +rhs, +lhs, static_cast<int>(count));
    }
}
...
inline vector<E>
    DPL_VECTORCALL slide_left(dx::zero_t zero, cmask_t<int32, M> mask,
        vector<E> lhs, vector<E> rhs, N count) noexcept {
    if constexpr (N::value == 0) {
        return xmm::select(mask, lhs, zero);
    } else if constexpr (N::value >= xmm::vector_size_v<E> || N::value < 0) {
        return xmm::select(mask, rhs, zero);
    } else if constexpr (!integral<E>) {
        using int_t = signed_representation_t<E>;
        return xmm::reinterpret<E>(xmm::slide_left(zero, mask,
            xmm::reinterpret<int_t>(lhs), xmm::reinterpret<int_t>(rhs), count));
    } else {
        return _mm_maskz_alignr_epi32(M, +rhs, +lhs, static_cast<int>(count));
    }
}
...
inline vector<E>
    ...
    if constexpr (N::value == 0) {
        ...
        return _mm_maskz_alignr_epi64(M, +rhs, +lhs, static_cast<int>(count));
    }
}
...
inline vector<E>
    ...
    if constexpr (N::value == 0) {
        ...
        return _mm_maskz_alignr_epi8(M, +rhs, +lhs, static_cast<int>(count));
    }
}
...
inline vector<E>
    ...
    if constexpr (N::value == 0) {
        ...
        return _mm_maskz_alignr_epi8(
            details::widen_bits16(M), +rhs, +lhs, static_cast<int>(count) * 2);
    }
}
...
inline vector<E> slide_right(
    abi_tag, vector<E> lhs, vector<E> rhs, N count) noexcept
requires requires {
    xmm::slide_left<(xmm::vector_size_v<E> - N::value)>(lhs, rhs);
}
{
    static_assert(N::value <= xmm::vector_size_v<E>);
    return xmm::slide_left<(xmm::vector_size_v<E> - N::value)>(lhs, rhs);
}
...
requires requires {
    xmm::slide_left(src, mask, lhs, rhs, imm<xmm::vector_size_v<E> - N::value>);
}
{
    static_assert(N::value <= xmm::vector_size_v<E>);
    return xmm::slide_left(
        src, mask, lhs, rhs, imm<xmm::vector_size_v<E> - N::value>);
}
...
requires requires {
    xmm::slide_left(
        zero, mask, lhs, rhs, imm<xmm::vector_size_v<E> - N::value>);
}
{
    static_assert(N::value <= xmm::vector_size_v<E>);
    return xmm::slide_left(
...
inline vector<E> slide_right(abi_tag, type_identity_t<vector<E>> src,
    cmask_t<E, M> mask, vector<E> lhs, vector<E> rhs, size_t count) noexcept
requires requires {
    xmm::slide_left(src, mask, lhs, rhs, xmm::vector_size_v<E> - count);
}
{
    return xmm::slide_left(src, mask, lhs, rhs, xmm::vector_size_v<E> - count);
}
...
inline vector<E> slide_left(abi_tag, dx::zero_t zero, cmask_t<E, M> mask,
    vector<E> lhs, vector<E> rhs, size_t count) noexcept
requires requires {
    xmm::slide_left(zero, mask, lhs, rhs, xmm::vector_size_v<E> - count);
}
{
    return xmm::slide_left(zero, mask, lhs, rhs, xmm::vector_size_v<E> - count);
}
