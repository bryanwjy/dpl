...
DPL_ATTRIBUTES(_HIDE_FROM_ABI, VECTORCALL, CONST, NODISCARD) inline vector<T> min(
...
DPL_ATTRIBUTES(_HIDE_FROM_ABI, VECTORCALL, CONST, NODISCARD) inline vector<T> min(
    ...
    if constexpr (sizeof(T) == 1) {
        return _mm_min_epi8(+left, +right);
    } else if constexpr (sizeof(T) == 2) {
        return _mm_min_epi16(+left, +right);
    } else if constexpr (sizeof(T) == 4) {
        return _mm_min_epi32(+left, +right);
    } else {
        static_assert(sizeof(T) == 8, "unsupported operation");
#if DPL_SIMD_X86_AVX512 & DPL_SIMD_X86_AVX512DQ
        return _mm_min_epi64(+left, +right);
#else
...
DPL_ATTRIBUTES(_HIDE_FROM_ABI, VECTORCALL, CONST, NODISCARD) inline vector<T> min(
    ...
    if constexpr (sizeof(T) == 1) {
        ...
        return _mm_min_epu16(+left, +right);
    } else if constexpr (sizeof(T) == 4) {
        return _mm_min_epu32(+left, +right);
    } else {
        static_assert(sizeof(T) == 8, "unsupported operation");
#if DPL_SIMD_X86_AVX512 & DPL_SIMD_X86_AVX512DQ
        return _mm_min_epu64(+left, +right);
#else
