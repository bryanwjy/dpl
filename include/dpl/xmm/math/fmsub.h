...
inline vector<float> DPL_VECTORCALL fmsub(
    vector<float> lhs, vector<float> mid, vector<float> rhs) noexcept {
    return _mm_fmsub_ps(+lhs, +mid, +rhs);
}

DPL_ATTRIBUTES(_HIDE_FROM_ABI, CONST, NODISCARD)
inline vector<double> DPL_VECTORCALL fmsub(
    vector<double> lhs, vector<double> mid, vector<double> rhs) noexcept {
    return _mm_fmsub_pd(+lhs, +mid, +rhs);
}

#  if DPL_SIMD_X86_AVX512FP16 & DPL_SIMD_X86_AVX512VL
DPL_ATTRIBUTES(_HIDE_FROM_ABI, CONST, NODISCARD)
inline vector<ext::float16> DPL_VECTORCALL fmsub(vector<ext::float16> lhs,
    vector<ext::float16> mid, vector<ext::float16> rhs) noexcept {
    return _mm_fmsub_ph(+lhs, +mid, +rhs);
}
#  endif

DPL_ATTRIBUTES(_HIDE_FROM_ABI, CONST, NODISCARD)
inline vector<ext::bfloat16> DPL_VECTORCALL fmsub(vector<ext::bfloat16> lhs,
    ...
    auto const lo = xmm::fmsub(xmm::element_cast<float>(lhs),
        xmm::element_cast<float>(mid), xmm::element_cast<float>(rhs));
    auto const hi = xmm::fmsub(xmm::element_cast<float>(hlhs),
...
constexpr vector<E> fmsub(
    abi_tag, vector<E> lhs, vector<E> mid, vector<E> rhs) noexcept
requires requires { xmm::fmsub(lhs, mid, rhs); }
{
    return xmm::fmsub(lhs, mid, rhs);
}
...
DPL_ATTRIBUTES(_HIDE_FROM_ABI, CONST, NODISCARD)
inline vector<float>
    DPL_VECTORCALL fmsub(vector<float> lhs, cmask_t<float, M> mask,
        vector<float> mid, vector<float> rhs) noexcept {
    return _mm_mask_fmsub_ps(+lhs, M, +mid, +rhs);
}

template <imask_t<float> M>
DPL_ATTRIBUTES(_HIDE_FROM_ABI, CONST, NODISCARD)
inline vector<float>
    DPL_VECTORCALL fmsub(dx::zero_t, cmask_t<float, M> mask, vector<float> lhs,
        vector<float> mid, vector<float> rhs) noexcept {
    return _mm_maskz_fmsub_ps(M, +lhs, +mid, +rhs);
}

template <imask_t<float> M>
DPL_ATTRIBUTES(_HIDE_FROM_ABI, CONST, NODISCARD)
inline vector<float>
    DPL_VECTORCALL fmsub(vector<float> lhs, vector<float> mid,
        vector<float> rhs, cmask_t<float, M> mask) noexcept {
    return _mm_mask3_fmsub_ps(+lhs, +mid, +rhs, M);
}

template <imask_t<ext::bfloat16> M>
DPL_ATTRIBUTES(_HIDE_FROM_ABI, CONST, NODISCARD)
inline vector<ext::bfloat16>
    DPL_VECTORCALL fmsub(vector<ext::bfloat16> lhs,
    ...
    auto const lo = xmm::fmsub(xmm::element_cast<float>(lhs), lmask,
        xmm::element_cast<float>(mid), xmm::element_cast<float>(rhs));
    auto const hi = xmm::fmsub(xmm::element_cast<float>(hlhs), hmask,
        xmm::element_cast<float>(hmid), xmm::element_cast<float>(hrhs));
...
DPL_ATTRIBUTES(_HIDE_FROM_ABI, CONST, NODISCARD)
inline vector<ext::bfloat16>
    DPL_VECTORCALL fmsub(dx::zero_t zero, cmask_t<ext::bfloat16, M> mask,
    ...
    constexpr cmask_t<float, (M & 0xf)> lmask;
    constexpr cmask_t<float, (M >> 4)> hmask;
    auto const lo = xmm::fmsub(zero, lmask, xmm::element_cast<float>(lhs),
        xmm::element_cast<float>(mid), xmm::element_cast<float>(rhs));
    auto const hi = xmm::fmsub(zero, hmask, xmm::element_cast<float>(hlhs),
        xmm::element_cast<float>(hmid), xmm::element_cast<float>(hrhs));
...
DPL_ATTRIBUTES(_HIDE_FROM_ABI, CONST, NODISCARD)
inline vector<ext::bfloat16>
    DPL_VECTORCALL fmsub(vector<ext::bfloat16> lhs, vector<ext::bfloat16> mid,
...
inline vector<double>
    DPL_VECTORCALL fmsub(vector<double> lhs, cmask_t<double, M> mask,
        vector<double> mid, vector<double> rhs) noexcept {
    return _mm_mask_fmsub_pd(+lhs, M, +mid, +rhs);
}

template <imask_t<double> M>
DPL_ATTRIBUTES(_HIDE_FROM_ABI, CONST, NODISCARD)
inline vector<double>
    DPL_VECTORCALL fmsub(dx::zero_t, cmask_t<double, M> mask,
        vector<double> lhs, vector<double> mid, vector<double> rhs) noexcept {
    return _mm_maskz_fmsub_pd(M, +lhs, +mid, +rhs);
}
template <imask_t<double> M>
DPL_ATTRIBUTES(_HIDE_FROM_ABI, CONST, NODISCARD)
inline vector<double>
    DPL_VECTORCALL fmsub(vector<double> lhs, vector<double> mid,
        vector<double> rhs, cmask_t<double, M> mask) noexcept {
    return _mm_mask3_fmsub_pd(+lhs, +mid, +rhs, M);
}
#    endif
...
inline vector<ext::float16>
    DPL_VECTORCALL fmsub(vector<ext::float16> lhs,
        cmask_t<ext::float16, M> mask, vector<ext::float16> mid,
        vector<ext::float16> rhs) noexcept {
    return _mm_mask_fmsub_ph(+lhs, M, +mid, +rhs);
}

template <imask_t<ext::float16> M>
DPL_ATTRIBUTES(_HIDE_FROM_ABI, CONST, NODISCARD)
inline vector<ext::float16>
    DPL_VECTORCALL fmsub(dx::zero_t, cmask_t<ext::float16, M> mask,
        vector<ext::float16> lhs, vector<ext::float16> mid,
        vector<ext::float16> rhs) noexcept {
    return _mm_maskz_fmsub_ph(M, +lhs, +mid, +rhs);
}

template <imask_t<ext::float16> M>
DPL_ATTRIBUTES(_HIDE_FROM_ABI, CONST, NODISCARD)
inline vector<ext::float16>
    DPL_VECTORCALL fmsub(vector<ext::float16> lhs, vector<ext::float16> mid,
        vector<ext::float16> rhs, cmask_t<ext::float16, M> mask) noexcept {
    return _mm_mask3_fmsub_ph(+lhs, +mid, +rhs, M);
}
#    endif
