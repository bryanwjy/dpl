...
inline mask<int8> DPL_VECTORCALL cmpgt(
    vector<int8> lhs, vector<int8> rhs) noexcept {
    return _mm_cmpgt_epi8(+lhs, +rhs);
}

DPL_ATTRIBUTES(_HIDE_FROM_ABI, CONST, NODISCARD)
inline mask<uint8> DPL_VECTORCALL cmpgt(
...
DPL_ATTRIBUTES(_HIDE_FROM_ABI, CONST, NODISCARD)
inline mask<int16> DPL_VECTORCALL cmpgt(
    vector<int16> lhs, vector<int16> rhs) noexcept {
    return _mm_cmpgt_epi16(+lhs, +rhs);
}

DPL_ATTRIBUTES(_HIDE_FROM_ABI, CONST, NODISCARD)
inline mask<uint16> DPL_VECTORCALL cmpgt(
...
DPL_ATTRIBUTES(_HIDE_FROM_ABI, CONST, NODISCARD)
inline mask<int32> DPL_VECTORCALL cmpgt(
    vector<int32> lhs, vector<int32> rhs) noexcept {
    return _mm_cmpgt_epi32(+lhs, +rhs);
}

DPL_ATTRIBUTES(_HIDE_FROM_ABI, CONST, NODISCARD)
inline mask<uint32> DPL_VECTORCALL cmpgt(
...
inline mask<int64> DPL_VECTORCALL cmpgt(
    vector<int64> lhs, vector<int64> rhs) noexcept {
    auto r =
        _mm_and_si128(_mm_cmpeq_epi32(+lhs, +rhs), _mm_sub_epi64(+rhs, +lhs));
    r = _mm_or_si128(r, _mm_cmpgt_epi32(+rhs, +lhs));
...
inline mask<uint64> DPL_VECTORCALL cmpgt(
    vector<uint64> lhs, vector<uint64> rhs) noexcept {
    auto const bias = +xmm::broadcast<int64>(dx::msb);
    return xmm::cmpgt(vector<int64>(_mm_xor_si128(+lhs, bias)),
        vector<int64>(_mm_xor_si128(+rhs, bias)));
}

#  endif

inline mask<float> DPL_VECTORCALL cmpgt(
    vector<float> lhs, vector<float> rhs) noexcept {
    return _mm_cmpgt_ps(+lhs, +rhs);
}

inline mask<double> DPL_VECTORCALL cmpgt(
...
inline mask<ext::float16> DPL_VECTORCALL cmpgt(
    vector<ext::float16> lhs, vector<ext::float16> rhs) noexcept {
    return _mm_movm_epi16(_mm_cmp_ph_mask(+lhs, +rhs, _CMP_GT_OQ));
}

template <imask_t<ext::float16> M>
DPL_ATTRIBUTES(_HIDE_FROM_ABI, CONST, NODISCARD)
inline mask<ext::float16>
    DPL_VECTORCALL cmpgt(cmask_t<ext::float16, M>, vector<ext::float16> lhs,
        vector<ext::float16> rhs) noexcept {
    return _mm_movm_epi16(_mm_mask_cmp_ph_mask(M, +lhs, +rhs, _CMP_GT_OQ));
}

#  endif

DPL_ATTRIBUTES(_HIDE_FROM_ABI, CONST, NODISCARD)
inline mask<ext::bfloat16> DPL_VECTORCALL cmpgt(
    ...
    auto const lo = _mm_castps_si128(_mm_cmpgt_ps(llhs, lrhs));
    auto const hi = _mm_castps_si128(_mm_cmpgt_ps(hlhs, hrhs));
...
inline mask<uint32>
    ...
    return _mm_movm_epi32(
        _mm_mask_cmp_epu32_mask(M, +lhs, +rhs, _MM_CMPINT_NLE));
}
...
inline mask<uint16>
    ...
    return _mm_movm_epi16(
        _mm_mask_cmp_epu16_mask(M, +lhs, +rhs, _MM_CMPINT_NLE));
}
...
inline mask<uint8>
    ...
    return _mm_movm_epi8(_mm_mask_cmp_epu8_mask(M, +lhs, +rhs, _MM_CMPINT_NLE));
}
...
inline mask<E> cmpgt(abi_tag, vector<E> lhs, vector<E> rhs) noexcept
requires requires { xmm::cmpgt(lhs, rhs); }
{
    return xmm::cmpgt(lhs, rhs);
}
...
requires requires { xmm::cmpgt(mask, lhs, rhs); }
{
    return xmm::cmpgt(mask, lhs, rhs);
}
