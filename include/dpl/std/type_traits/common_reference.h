...
struct copy_cvref_from {
    template <typename U>
    using apply DPL_NODEBUG = copy_cvref_t<T, U>;
};

template <typename Target, typename... Ts>
using merge_const_t DPL_NODEBUG =
    conditional_t<(... || is_const_v<Ts>), add_const_t<Target>, Target>;
template <typename Target, typename... Ts>
using merge_volatile_t DPL_NODEBUG =
    conditional_t<(... || is_volatile_v<Ts>), add_volatile_t<Target>, Target>;

template <typename Target, typename... Ts>
using merge_cv_t DPL_NODEBUG =
    merge_volatile_t<merge_const_t<Target, Ts...>, Ts...>;

template <typename T0, typename T1>
struct simple_common_ref {};

template <typename T0, typename T1>
using simple_common_ref_t DPL_NODEBUG =
    typename simple_common_ref<T0, T1>::type;
...
template <typename T>
using rvalue_ref DPL_NODEBUG = add_rvalue_reference<remove_reference_t<T>>;
template <typename T>
using rvalue_ref_t DPL_NODEBUG = typename rvalue_ref<T>::type;
...
using basic_common_ref DPL_NODEBUG =
...
template <typename T, typename U>
using common_reference DPL_NODEBUG = first_type<simple_common_ref<T, U>,
    basic_common_reference<remove_cvref_t<T>, remove_cvref_t<U>,
        copy_cvref_from<T>::template apply, copy_cvref_from<U>::template apply>,
